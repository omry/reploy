...
A startup failure may deliver `terminated` before `ready`, without
`terminating` or `workload-outputs-finalized`. In that branch, do not wait for
those events or send `complete`; acknowledge `terminated` immediately.
...
The controller may write these requests at their documented lifecycle points:
...
`resize` supports headless orchestration. After `ready`, `resize` is valid until
termination begins, and `terminate` requests workload termination. Send
`complete` exactly once only after both `terminating` and
`workload-outputs-finalized`; it says controller artifacts are finalized. After
reading `terminated`, send `acknowledge-terminated` exactly once and continue
reading until the broker closes cleanly.
