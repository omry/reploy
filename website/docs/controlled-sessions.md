...
The initial controller runtime is a Linux container under Docker. Reploy
supports Linux `amd64` and `arm64` controller images and installs the matching
`reploy-session-client` executable into a derived controller image. The
workload image is not modified.
...
The workload blueprint must declare its persistent workload command. Declare
any service the controller needs as a named workload endpoint; see
...
in the controller; the workload always runs its declared persistent shell.
...
Initial limitations include Linux controller containers under Docker only;
...
filtering, or content inspection. The current private endpoint network has the
coarse reachability described above. A backend that cannot establish the
required Linux sandbox fails closed instead of silently degrading.
...
`broker-ready(terminal_socket)`, it records the byte-only attachment with an
unmodified asciinema 3.x command:

```bash
asciinema record --quiet --return --command \
  "reploy-session-client attach --socket '$terminal_socket'" \
  "$REPLOY_OUTPUT_DIR/session.cast"
```

Use a safely quoted socket value obtained from the decoded event; do not parse
it from terminal output. OmegaFlow can use the granted endpoint coordinates
for Playwright or Chromium while the same workload shell stays alive. After
the attachment exits, it finalizes the cast, screenshots, diagnostics, and
rendered media, sends `complete`, reads and stores `terminated`, then
acknowledges it.
