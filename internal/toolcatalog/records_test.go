...
const recordOtherDigest canonical.Digest = "sha256:1111111111111111111111111111111111111111111111111111111111111111"
...
func TestValidateToolReleaseIndexV1(t *testing.T) {
	...
	baseRecords := map[string]loadedRecordV1{
		baseManifest.ID: {ID: baseManifest.ID, Schema: baseManifest.Schema, Digest: recordTestDigest, Value: &baseManifest},
	}
	if err := validateToolReleaseIndexV1(&baseTool, baseRecords); err != nil {
		t.Fatal(err)
	}
	for _, test := range []struct {
		name   string
		mutate func(*ToolRecordV1, map[string]loadedRecordV1)
		want   string
	}{
		{name: "missing manifest", mutate: func(_ *ToolRecordV1, records map[string]loadedRecordV1) { delete(records, baseManifest.ID) }, want: "does not resolve"},
		{name: "wrong manifest digest", mutate: func(tool *ToolRecordV1, _ map[string]loadedRecordV1) { tool.Releases[0].Digest = recordOtherDigest }, want: "does not resolve"},
		{name: "wrong record kind", mutate: func(_ *ToolRecordV1, records map[string]loadedRecordV1) {
			record := records[baseManifest.ID]
			record.Value = validRecordValuesV1()[2]
			records[baseManifest.ID] = record
		}, want: "non-manifest"},
	} {
		t.Run("references/"+test.name, func(t *testing.T) {
			tool := baseTool
			tool.Releases = append([]RecordReferenceV1{}, baseTool.Releases...)
			records := make(map[string]loadedRecordV1, len(baseRecords))
			for id, record := range baseRecords {
				records[id] = record
			}
			test.mutate(&tool, records)
			err := validateToolReleaseIndexV1(&tool, records)
			if err == nil || !strings.Contains(err.Error(), test.want) {
				t.Fatalf("error = %v, want substring %q", err, test.want)
			}
		})
	}
	if err := validateToolReleaseManifestsV1(&baseTool, []*ReleaseManifestV1{&baseManifest}); err != nil {
		t.Fatal(err)
	}
	pepTool := baseTool
	...
	if err := validateToolReleaseManifestsV1(&pepTool, []*ReleaseManifestV1{&pepManifest}); err != nil {
		t.Fatal(err)
	}
	opaqueTool := baseTool
	...
	if err := validateToolReleaseManifestsV1(&opaqueTool, []*ReleaseManifestV1{&opaqueManifest}); err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range []struct {
		...
		{name: "invalid semver alias", scheme: "semver", version: "1.2.3", aliases: []string{"banana"}, wantError: "invalid under SemVer"},
		{name: "noncanonical integer alias", scheme: "integer", version: "21", aliases: []string{"01"}, wantError: "canonical decimal"},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			err := validateToolReleaseManifestsV1(&tool, manifests)
...
func TestValidateTargetAgainstContractV1(t *testing.T) {
	newRecords := func() (ReleaseContractV1, TargetRecordV1) {
		contract := *(validRecordValuesV1()[2].(*ReleaseContractV1))
		target := *(validRecordValuesV1()[3].(*TargetRecordV1))
		release := "tool:demo/releases/1.2.3"
		contract.Binding = BindingRequestV1{Options: []string{"python"}, Required: true, Default: "python"}
		contract.Selections = SelectionRequestV1{Options: []string{"chromium"}, Minimum: "0", Maximum: "1", Defaults: []string{"chromium"}, CompatibilityGroups: [][]string{{"chromium"}}}
		colorDefault, featureDefault, workerDefault := "blue", "true", "2"
		contract.Parameters = []ParameterSchemaV1{
			{Name: "color", Type: "enum", Default: &colorDefault, Values: []string{"blue", "green"}},
			{Name: "feature", Type: "boolean", Default: &featureDefault, Values: []string{}},
			{Name: "workers", Type: "integer", Default: &workerDefault, Values: []string{}, Minimum: "0", Maximum: "4"},
		}
		target.Bindings = []TargetBindingV1{{
			Name: "python", Contract: recordTestReference(release + "/bindings/python/contract"),
			Artifacts:   []RecordReferenceV1{recordTestReference(release + "/bindings/python/artifacts/linux-amd64")},
			PackageSets: []RecordReferenceV1{}, Exports: []ToolExportV1{}, Probes: []RecordProbeV1{},
		}}
		target.Selections = []TargetSelectionV1{{
			Name: "chromium", Payloads: []RecordReferenceV1{recordTestReference(release + "/payloads/chromium/chromium-linux-amd64")},
			PackageSets: []RecordReferenceV1{}, Exports: []ToolExportV1{}, Probes: []RecordProbeV1{},
		}}
		target.Parameters = []TargetParameterConstraintV1{
			{Name: "color", Values: []string{"blue"}},
			{Name: "feature", Values: []string{"true"}},
			{Name: "workers", Values: []string{}, Minimum: "1", Maximum: "3"},
		}
		return contract, target
	}

	contract, target := newRecords()
	if err := validateTargetAgainstContractV1(&contract, &target); err != nil {
		t.Fatalf("valid target contract relationship: %v", err)
	}
	for _, test := range []struct {
		name   string
		...
		{name: "missing binding mapping", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Bindings = []TargetBindingV1{} }, want: "every contract binding option"},
		{name: "undeclared binding mapping", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) {
			target.Bindings = append([]TargetBindingV1{{Name: "node"}}, target.Bindings...)
		}, want: "every contract binding option"},
		{name: "missing selection mapping", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Selections = []TargetSelectionV1{} }, want: "every contract selection option"},
		{name: "undeclared parameter", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Parameters[0].Name = "architecture" }, want: "not declared"},
		{name: "enum outside contract", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Parameters[0].Values = []string{"red"} }, want: "outside the contract domain"},
		{name: "enum excludes default", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Parameters[0].Values = []string{"green"} }, want: "excludes the contract default"},
		{name: "integer widens contract", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Parameters[2].Maximum = "5" }, want: "widens the contract domain"},
		{name: "integer excludes default", mutate: func(_ *ReleaseContractV1, target *TargetRecordV1) { target.Parameters[2].Minimum = "3" }, want: "excludes the contract default"},
	} {
		t.Run(test.name, func(t *testing.T) {
			contract, target := newRecords()
...
func TestValidateManifestResolvedGraphV1(t *testing.T) {
	newGraph := func() (*ReleaseManifestV1, map[string]loadedRecordV1) {
		values := validRecordValuesV1()
		...
		contract.Binding = BindingRequestV1{Options: []string{"python"}, Required: true, Default: "python"}
		target.Bindings = []TargetBindingV1{{
			Name: "python", Contract: recordTestReference(bindingContract.ID),
			Artifacts: []RecordReferenceV1{recordTestReference(artifact.ID)}, PackageSets: []RecordReferenceV1{},
			Exports: []ToolExportV1{}, Probes: []RecordProbeV1{},
		}}
		target.Payloads = []RecordReferenceV1{}
		target.Selections = []TargetSelectionV1{}
		manifest.Contract = recordTestReference(contract.ID)
		manifest.Targets = []RecordReferenceV1{recordTestReference(target.ID)}
		manifest.ValidationProfile = recordTestReference(profile.ID)
		manifest.ArtifactSources = []ArtifactSourceMappingV1{{
			ArtifactSHA256: recordTestDigest,
			Artifact:       recordTestReference(artifact.ID),
			Source:         recordTestReference(source.ID),
		}}
		return &manifest, map[string]loadedRecordV1{
			contract.ID:        {ID: contract.ID, Schema: contract.Schema, Digest: recordTestDigest, Value: &contract},
			target.ID:          {ID: target.ID, Schema: target.Schema, Digest: recordTestDigest, Value: &target},
			bindingContract.ID: {ID: bindingContract.ID, Schema: bindingContract.Schema, Digest: recordTestDigest, Value: &bindingContract},
			artifact.ID:        {ID: artifact.ID, Schema: artifact.Schema, Digest: recordTestDigest, Value: &artifact},
			source.ID:          {ID: source.ID, Schema: source.Schema, Digest: recordTestDigest, Value: &source},
			fixture.ID:         {ID: fixture.ID, Schema: fixture.Schema, Digest: recordTestDigest, Value: &fixture},
			profile.ID:         {ID: profile.ID, Schema: profile.Schema, Digest: recordTestDigest, Value: &profile},
		}
	}

	manifest, records := newGraph()
	if err := validateManifestResolvedGraphV1(manifest, records); err != nil {
		t.Fatalf("valid artifact-source mapping: %v", err)
	}
	for _, test := range []struct {
		name   string
		mutate func(*ReleaseManifestV1, map[string]loadedRecordV1)
		want   string
	}{
		{name: "missing artifact", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			delete(records, manifest.ArtifactSources[0].Artifact.ID)
		}, want: "does not resolve"},
		{name: "missing source mapping", mutate: func(manifest *ReleaseManifestV1, _ map[string]loadedRecordV1) {
			manifest.ArtifactSources = []ArtifactSourceMappingV1{}
		}, want: "has no source mapping"},
		{name: "cross-platform binding artifact", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Artifact.ID]
			value := *(record.Value.(*BindingArtifactRecordV1))
			value.Platform = "linux/arm64"
			record.Value = &value
			records[record.ID] = record
		}, want: "incompatible with target"},
		{name: "fixture for another target", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			value.Target.VersionID = "13"
			record.Value = &value
			records[record.ID] = record
		}, want: "describes a different target"},
		{name: "artifact digest mismatch", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Artifact.ID]
			value := *(record.Value.(*BindingArtifactRecordV1))
			value.SHA256 = recordOtherDigest
			record.Value = &value
			records[record.ID] = record
		}, want: "content digests disagree"},
		{name: "source digest mismatch", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Source.ID]
			value := *(record.Value.(*ArtifactSourceRecordV1))
			value.SHA256 = recordOtherDigest
			record.Value = &value
			records[record.ID] = record
		}, want: "content digests disagree"},
		{name: "source size mismatch", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Source.ID]
			value := *(record.Value.(*ArtifactSourceRecordV1))
			value.Size = "43"
			record.Value = &value
			records[record.ID] = record
		}, want: "content sizes disagree"},
		{name: "missing validation profile", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			delete(records, manifest.ValidationProfile.ID)
		}, want: "release validation profile"},
		{name: "profile for another release", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ValidationProfile.ID]
			value := *(record.Value.(*ValidationProfileRecordV1))
			value.Version = "2.0.0"
			record.Value = &value
			records[record.ID] = record
		}, want: "incompatible with the release"},
		{name: "missing target package set", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			target.PackageSets = []RecordReferenceV1{recordTestReference("tool:demo/releases/1.2.3/package-sets/missing")}
		}, want: "package set"},
		{name: "missing binding package set", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			target.Bindings[0].PackageSets = []RecordReferenceV1{recordTestReference("tool:demo/releases/1.2.3/package-sets/missing")}
		}, want: "package set"},
		{name: "missing selection package set", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			contract := records[manifest.Contract.ID].Value.(*ReleaseContractV1)
			contract.Selections = SelectionRequestV1{Options: []string{"chromium"}, Minimum: "0", Maximum: "1", Defaults: []string{}, CompatibilityGroups: [][]string{{"chromium"}}}
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			target.Selections = []TargetSelectionV1{{Name: "chromium", Payloads: []RecordReferenceV1{}, PackageSets: []RecordReferenceV1{recordTestReference("tool:demo/releases/1.2.3/package-sets/missing")}, Exports: []ToolExportV1{}, Probes: []RecordProbeV1{}}}
		}, want: "package set"},
		{name: "wrong package manager", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			packageSet := *(validRecordValuesV1()[8].(*NativePackageSetV1))
			packageSet.Manager = "rpm"
			records[packageSet.ID] = loadedRecordV1{ID: packageSet.ID, Schema: packageSet.Schema, Digest: recordTestDigest, Value: &packageSet}
			target.PackageSets = []RecordReferenceV1{recordTestReference(packageSet.ID)}
		}, want: "incompatible package manager"},
		{name: "wrong wheel distribution", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Artifact.ID]
			value := *(record.Value.(*BindingArtifactRecordV1))
			value.Filename = "other-1.2.3-py3-none-manylinux1_x86_64.whl"
			record.Value = &value
			records[record.ID] = record
		}, want: "does not match contract package"},
		{name: "wrong wheel version", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Artifact.ID]
			value := *(record.Value.(*BindingArtifactRecordV1))
			value.Filename = "demo-9.9-py3-none-manylinux1_x86_64.whl"
			record.Value = &value
			records[record.ID] = record
		}, want: "does not satisfy contract requirement"},
		{name: "unsupported wheel interpreter", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Artifact.ID]
			value := *(record.Value.(*BindingArtifactRecordV1))
			value.RequiresPython = ">=4"
			record.Value = &value
			records[record.ID] = record
		}, want: "excludes every contract interpreter"},
		{name: "fixture context outside contract", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			...
			record.Value = &value
			records[record.ID] = record
		}, want: "not declared by the release contract"},
		{name: "fixture context coverage", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			contract := records[manifest.Contract.ID].Value.(*ReleaseContractV1)
			contract.Contexts = []string{"build", "runtime"}
		}, want: "do not cover support tuple"},
		{name: "fixture undeclared binding", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			value.Binding = "node"
		...
		{name: "fixture incompatible selections", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			value.Selections = []string{"chromium"}
			record.Value = &value
			records[record.ID] = record
		}, want: "selections do not satisfy"},
		{name: "fixture selection coverage", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			contract := records[manifest.Contract.ID].Value.(*ReleaseContractV1)
			contract.Selections = SelectionRequestV1{Options: []string{"chromium", "webkit"}, Minimum: "1", Maximum: "1", Defaults: []string{"chromium"}, CompatibilityGroups: [][]string{{"chromium"}, {"webkit"}}}
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			target.Selections = []TargetSelectionV1{
				{Name: "chromium", Payloads: []RecordReferenceV1{}, PackageSets: []RecordReferenceV1{}, Exports: []ToolExportV1{}, Probes: []RecordProbeV1{}},
				{Name: "webkit", Payloads: []RecordReferenceV1{}, PackageSets: []RecordReferenceV1{}, Exports: []ToolExportV1{}, Probes: []RecordProbeV1{}},
			}
		}, want: "do not cover support tuple"},
		{name: "fixture parameter outside contract", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			value.Parameters = []ParameterValueV1{{Name: "color", Value: "red"}}
			record.Value = &value
			records[record.ID] = record
		}, want: "outside the contract or target domain"},
		{name: "fixture parameter outside target", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			contract := records[manifest.Contract.ID].Value.(*ReleaseContractV1)
			contract.Parameters = []ParameterSchemaV1{{Name: "color", Type: "enum", Values: []string{"blue", "red"}}}
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			target.Parameters = []TargetParameterConstraintV1{{Name: "color", Values: []string{"blue"}}}
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			value.Parameters = []ParameterValueV1{{Name: "color", Value: "red"}}
			record.Value = &value
		...
		{name: "fixture missing required parameter", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			contract := records[manifest.Contract.ID].Value.(*ReleaseContractV1)
			contract.Parameters = []ParameterSchemaV1{{Name: "color", Type: "enum", Required: true, Values: []string{"blue"}}}
		}, want: "required parameter"},
		{name: "fixture parameter coverage", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			contract := records[manifest.Contract.ID].Value.(*ReleaseContractV1)
			contract.Parameters = []ParameterSchemaV1{{Name: "feature", Type: "boolean", Required: true, Values: []string{}}}
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			record := records[target.IntegrationFixtures[0].ID]
			value := *(record.Value.(*IntegrationFixtureRecordV1))
			value.Parameters = []ParameterValueV1{{Name: "feature", Value: "false"}}
			record.Value = &value
			records[record.ID] = record
		}, want: "do not cover support tuple"},
		{name: "conflicting selected package sets", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			first := &NativePackageSetV1{Schema: NativePackageSetSchemaV1, ID: "tool:demo/releases/1.2.3/package-sets/first", Manager: "apt", Requirements: []string{"libfoo=1"}}
			second := &NativePackageSetV1{Schema: NativePackageSetSchemaV1, ID: "tool:demo/releases/1.2.3/package-sets/second", Manager: "apt", Requirements: []string{"libfoo=2"}}
			...
			target.PackageSets = []RecordReferenceV1{recordTestReference(first.ID)}
			target.Bindings[0].PackageSets = []RecordReferenceV1{recordTestReference(second.ID)}
		}, want: "conflict on package"},
		{name: "conflicting selected exports", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			target := records[manifest.Targets[0].ID].Value.(*TargetRecordV1)
			target.Exports = []ToolExportV1{{Name: "demo", Path: "/opt/other/bin/demo"}}
		}, want: "conflict on export"},
		{name: "resolver outside contract", mutate: func(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) {
			record := records[manifest.ArtifactSources[0].Source.ID]
			value := *(record.Value.(*ArtifactSourceRecordV1))
			value.Resolver = "other"
			record.Value = &value
			records[record.ID] = record
		}, want: "outside the release contract"},
	} {
		t.Run(test.name, func(t *testing.T) {
			manifest, records := newGraph()
			test.mutate(manifest, records)
...
func TestTargetBindingV1PreservesAllScopedContributions(t *testing.T) {
	value := *(validRecordValuesV1()[3].(*TargetRecordV1))
	release := "tool:demo/releases/1.2.3"
	value.Bindings = []TargetBindingV1{{
		Name:        "python",
		Contract:    recordTestReference(release + "/bindings/python/contract"),
		Artifacts:   []RecordReferenceV1{recordTestReference(release + "/bindings/python/artifacts/linux-amd64")},
		PackageSets: []RecordReferenceV1{recordTestReference(release + "/package-sets/python-runtime-amd64")},
		Exports:     []ToolExportV1{{Name: "python", Path: "/opt/demo/bin/python"}},
		Probes:      []RecordProbeV1{{Path: "/opt/demo/bin/python", Args: []string{"--version"}, Network: "none"}},
	}}
	if err := validateLoadedRecordV1(loadedRecordV1{ID: value.ID, Schema: value.Schema, Value: &value}); err != nil {
		t.Fatal(err)
	}
	...
	target := decoded.Value.(*TargetRecordV1)
	if len(target.Bindings[0].Artifacts) != 1 || len(target.Bindings[0].PackageSets) != 1 || len(target.Bindings[0].Exports) != 1 || len(target.Bindings[0].Probes) != 1 {
		t.Fatalf("decoded target binding lost scoped contributions: %#v", target.Bindings[0])
	}
}

func TestTargetSupportsMultipleParameterizedIntegrationFixturesV1(t *testing.T) {
	value := *(validRecordValuesV1()[3].(*TargetRecordV1))
	release := "tool:demo/releases/1.2.3/validation/fixtures/"
	value.IntegrationFixtures = []RecordReferenceV1{
		recordTestReference(release + "debian-12-amd64-build-blue"),
		recordTestReference(release + "debian-12-amd64-build-green"),
	}
	if err := validateLoadedRecordV1(loadedRecordV1{ID: value.ID, Schema: value.Schema, Value: &value}); err != nil {
		t.Fatal(err)
	}
	fixture := *(validRecordValuesV1()[9].(*IntegrationFixtureRecordV1))
	fixture.Name = "debian-12-amd64-build-blue"
	fixture.ID = release + fixture.Name
	fixture.Parameters = []ParameterValueV1{{Name: "color", Value: "blue"}}
	if err := validateLoadedRecordV1(loadedRecordV1{ID: fixture.ID, Schema: fixture.Schema, Value: &fixture}); err != nil {
		t.Fatal(err)
	}
}
...
func TestDecodeValidationEvidenceV1(t *testing.T) {
	evidence := ValidationEvidenceV1{
		...
		BaseImageDigest: recordTestDigest, Binding: "python", Selections: []string{"chromium"}, Parameters: []ParameterValueV1{},
		Fixture: "tool:demo/releases/1.2.3/validation/fixtures/demo-debian-12-amd64", ValidatorVersion: "validator-v1", Result: "pass",
...
func TestValidateValidationEvidenceV1RejectsUnboundClaims(t *testing.T) {
	valid := ValidationEvidenceV1{
		...
		BaseImageDigest: recordTestDigest, Binding: "python", Selections: []string{"chromium"}, Parameters: []ParameterValueV1{},
		Fixture: "tool:demo/releases/1.2.3/validation/fixtures/demo", ValidatorVersion: "validator-v1", Result: "pass", ProbeDigests: []canonical.Digest{recordTestDigest},
	}
	for _, test := range []struct {
		name   string
		...
		{name: "parameters", mutate: func(value *ValidationEvidenceV1) { value.Parameters = nil }, want: "must use a bounded array"},
		{name: "fixture", mutate: func(value *ValidationEvidenceV1) { value.Fixture = "anything" }, want: "canonical ID"},
		{name: "fixture wrong version", mutate: func(value *ValidationEvidenceV1) {
			value.Fixture = "tool:demo/releases/2.0.0/validation/fixtures/demo"
		}, want: "evidence tool and version namespace"},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			value.Parameters = append([]ParameterValueV1{}, valid.Parameters...)
...
func TestValidationEvidenceAcceptsSchemeNativeVersion(t *testing.T) {
	value := ValidationEvidenceV1{
		...
		BaseImageDigest: recordTestDigest, Binding: "python", Selections: []string{"chromium"}, Parameters: []ParameterValueV1{},
		Fixture: "tool:demo/releases/1%212/validation/fixtures/demo", ValidatorVersion: "validator-v1", Result: "pass", ProbeDigests: []canonical.Digest{recordTestDigest},
	}
