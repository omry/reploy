...
func TestValidateManifestArtifactContentV1(t *testing.T) {
	...
	if err := validateManifestArtifactContentV1(manifest, records); err != nil {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			err := validateManifestArtifactContentV1(manifest, records)
			if err == nil || !strings.Contains(err.Error(), test.want) {
				t.Fatalf("error = %v, want substring %q", err, test.want)
			}
		})
