...
func validValidationEvidenceV1() *ValidationEvidenceV1 {
	return &ValidationEvidenceV1{
		...
		BaseImageDigest: recordTestDigest, Bindings: []string{"python"},
...
func TestReleaseContractAndFixtureClonesAreIndependent(t *testing.T) {
	contract := &ReleaseContractV1{
		...
		Selections: SelectionSchemaV1{
			...
			Combinations: []SelectionCombinationV1{{Values: [][]string{{"chromium", "firefox"}}}},
		},
	}
	...
	contractClone.Selections.Combinations[0].Values[0][0] = "webkit"
	...
	if contract.Selections.Combinations[0].Values[0][0] != "chromium" {
...
func TestRecordModelUsesFinalBindingSelectionAndValidationShape(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			value: ReleaseContractV1{
				...
				Selections: SelectionSchemaV1{
					...
					Combinations: []SelectionCombinationV1{{Values: [][]string{{"chromium"}}}},
				},
				CompatibilityConstraints: []string{"portable-archive-v1"},
			},
			required:  []string{`"binding":{"options":["python"]}`, `"dimensions"`, `"combinations"`, `"compatibility_constraints":["portable-archive-v1"]`},
			forbidden: []string{`"required"`, `"default"`, `"parameters"`, `"probes"`, `"compatibility_groups"`},
		},
		{
		...
		},
		{
			...
			required:  []string{`"bindings":["python"]`, `"selections":{"browser":["chromium"]}`, `"validator_output_digest"`},
