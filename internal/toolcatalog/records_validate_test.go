	...
	for _, testCase := range []struct {
		...
		{name: "contract selection tuple", index: 2, wantSub: "not advertised", mutate: func(value any) {
			value.(*ReleaseContractV1).Selections = SelectionSchemaV1{
				...
				Combinations: []SelectionCombinationV1{{Values: [][]string{{"webkit"}}}},
			}
		}},
...
func TestValidateBindingAndSelectionSchemasV1(t *testing.T) {
	...
	validSelections := SelectionSchemaV1{
		...
		Combinations: []SelectionCombinationV1{
			{Values: [][]string{{"chromium"}}},
			{Values: [][]string{{"webkit"}}},
		},
	}
	...
	for index := range tooManyCombinations {
		tooManyCombinations[index] = SelectionCombinationV1{Values: [][]string{{"chromium"}}}
	}
	for _, test := range []struct {
		name       string
		...
		{name: "invalid option", selections: SelectionSchemaV1{Dimensions: []SelectionDimensionV1{{Name: "browser", Options: []string{"Chromium"}}}, Combinations: []SelectionCombinationV1{{Values: [][]string{{"Chromium"}}}}}, want: "canonical identifiers"},
		...
		{name: "wrong arity", selections: SelectionSchemaV1{Dimensions: validSelections.Dimensions, Combinations: []SelectionCombinationV1{{Values: [][]string{}}}}, want: "one value set"},
		{name: "undeclared value", selections: SelectionSchemaV1{Dimensions: validSelections.Dimensions, Combinations: []SelectionCombinationV1{{Values: [][]string{{"firefox"}}}}}, want: "not advertised"},
		{name: "duplicate combinations", selections: SelectionSchemaV1{Dimensions: validSelections.Dimensions, Combinations: []SelectionCombinationV1{{Values: [][]string{{"chromium"}}}, {Values: [][]string{{"chromium"}}}}}, want: "unique and sorted"},
