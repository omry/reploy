...
func TestSelectedClosureIgnoresUnrelatedTargetAndSourceLocatorChanges(t *testing.T) {
	...
	newFixtureDigest := addCatalogRecordV1(t, files, "definitions/java/21.0.12+8/validation/fixtures/debian-14-amd64.json", &newFixture)
	...
	newTargetDigest := addCatalogRecordV1(t, files, "definitions/java/21.0.12+8/targets/debian/14/amd64.json", &newTarget)
...
func TestCatalogRejectsMissingDuplicateAndUnmappedRecords(t *testing.T) {
	...
	t.Run("duplicate ID", func(t *testing.T) {
		...
		files["definitions/java/21.0.12+8/duplicate.json"] = &fstest.MapFile{Data: append([]byte{}, files[filename].Data...)}
	...
	t.Run("orphaned source mapping", func(t *testing.T) {
		...
		payload.Size = "42"
		payloadDigest := addCatalogRecordV1(t, files, "definitions/java/21.0.12+8/payloads/orphan-linux-amd64.json", &payload)
		...
		sourceDigest := addCatalogRecordV1(t, files, "definitions/java/21.0.12+8/revisions/1/sources/orphan-linux-amd64.json", &source)
		manifestDigest := rewriteCatalogRecordV1(t, files, "tool:java/releases/21.0.12+8/revisions/1/manifest", func(value any) {
			manifest := value.(*ReleaseManifestV1)
	...
	t.Run("shared digest size mismatch", func(t *testing.T) {
		...
		payload.Size = "1"
		payloadDigest := addCatalogRecordV1(t, files, "definitions/java/21.0.12+8/payloads/jdk-copy-linux-amd64.json", &payload)
		targetID := "tool:java/releases/21.0.12+8/targets/debian/12/amd64"
		targetDigest := rewriteCatalogRecordV1(t, files, targetID, func(value any) {
