...
func TestCompareToolVersionsAndExactLookupV1(t *testing.T) {
	...
	catalog, tool := enumerationTestCatalogV1(t, "semver", []enumerationReleaseV1{
		{Version: "1.0.0+first", Revision: "1"}, {Version: "1.0.0+second", Revision: "2"},
	})
	ordered, err := catalog.enumerateReleaseCandidatesV1(tool, nil, "")
	if err != nil || strings.Join(enumeratedVersionsV1(t, catalog, ordered), ",") !=
		"1.0.0+second,1.0.0+first" {
		t.Errorf("equal-precedence revision ordering = %v, %v",
			enumeratedVersionsV1(t, catalog, ordered), err)
	}
}
...
func TestReleaseSatisfiesConstraintV1(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{upstream: "1.2.3", want: true},
		{upstream: "1.3.0", want: false},
	} {
		requested := requestedVersionForTestV1(testCase.upstream, nil, "==1.2.*")
		got, err := releaseSatisfiesConstraintV1("pep440", testCase.upstream, nil, requested)
		if err != nil || got != testCase.want {
			t.Errorf("PEP 440 prefix equality for %q = %v, %v", testCase.upstream, got, err)
		}
	}
	catalog, tool := enumerationTestCatalogV1(t, "pep440", []enumerationReleaseV1{
		{Version: "1.2.3", Revision: "1"},
		{Version: "1.2.4", Revision: "1"},
		{Version: "1.3.0", Revision: "1"},
	})
	ordered, err := catalog.enumerateReleaseCandidatesV1(tool, []string{"==1.2.*"}, "")
	if err != nil || strings.Join(enumeratedVersionsV1(t, catalog, ordered), ",") != "1.2.4,1.2.3" {
		t.Errorf("PEP 440 prefix-equality enumeration = %v, %v",
			enumeratedVersionsV1(t, catalog, ordered), err)
	}
}
