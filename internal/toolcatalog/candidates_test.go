...
func TestReleaseSatisfiesConstraintV1(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			got, err := releaseSatisfiesConstraintV1(testCase.scheme, testCase.upstream, testCase.aliases, testCase.constraint)
			if err != nil {
	...
	if _, err := releaseSatisfiesConstraintV1("opaque", "vetted", nil, ">=1.0.0"); err == nil {
		t.Error("a comparison constraint was evaluated against an opaque scheme")
	}
	if got, err := releaseSatisfiesConstraintV1("opaque", "vetted", nil, "vetted"); err != nil || !got {
		t.Errorf("an exact opaque coordinate did not match: %v, %v", got, err)
	}
	if _, err := releaseSatisfiesConstraintV1("semver", "1.2.3", nil, ">=not-a-version"); err == nil {
...
func TestEqualityConstraintsResolveThroughTheLookupMapV1(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			got, err := releaseSatisfiesConstraintV1(testCase.scheme, testCase.upstream,
				testCase.aliases, testCase.constraint)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if got != testCase.want {
	...
	if _, err := releaseSatisfiesConstraintV1("opaque", "vetted", nil, "==other"); err != nil {
