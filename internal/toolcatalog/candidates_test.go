...
func TestCompactRevisionSuffixUsesTheLoadedVersionSchemeV1(t *testing.T) {
	for _, scheme := range []string{"semver", "pep440", "integer"} {
		constraints, revision, err := normalizedVersionDemandV1(
			&ToolRecordV1{Name: "demo", VersionScheme: scheme}, []string{"1~2"}, "")
		if err != nil || strings.Join(constraints, ",") != "1" || revision != "2" {
			t.Errorf("%s compact suffix = %v revision %q, %v", scheme, constraints, revision, err)
		}
	}
	constraints, revision, err := normalizedVersionDemandV1(
	...
	if _, _, err := normalizedVersionDemandV1(
		&ToolRecordV1{Name: "demo", VersionScheme: "semver"}, []string{"1.0.0~2"}, "3"); err == nil ||
