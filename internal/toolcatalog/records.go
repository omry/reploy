...
func validateSourceURLV1(raw string) error {
	...
	if err != nil || parsed.Scheme != "https" || parsed.Opaque != "" || parsed.Host == "" || parsed.Hostname() == "" || parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" || parsed.Host != strings.ToLower(parsed.Host) || strings.HasSuffix(parsed.Hostname(), ".") || parsed.Port() == "443" || !asciiURLHostV1(parsed.Hostname()) || !canonicalPercentEscapesV1(parsed.EscapedPath()) || hasURLDotSegmentV1(parsed.Path) {
		return fmt.Errorf("source URL must be a credential-free HTTPS URL without query or fragment")
	}
	return nil
}
