...
// validateManifestResolvedGraphV1 verifies relationships that require the
...
func validateManifestResolvedGraphV1(manifest *ReleaseManifestV1, records map[string]loadedRecordV1) error {
	...
	addArtifact := func(reference RecordReferenceV1, target *TargetRecordV1, binding string, selection string, bindingContract *BindingContractV1) error {
		...
		case *BindingArtifactRecordV1:
			...
			if err := validateBindingArtifactAgainstContractV1(bindingContract, value); err != nil {
				return fmt.Errorf("binding artifact %q: %w", value.ID, err)
			}
			digest = value.SHA256
		case *PayloadRecordV1:
	...
	for _, targetReference := range manifest.Targets {
		...
		for _, binding := range target.Bindings {
			...
			for _, reference := range binding.Artifacts {
				if err := addArtifact(reference, target, binding.Name, "", bindingContract); err != nil {
					return err
				}
			}
		}
		for _, reference := range target.Payloads {
			if err := addArtifact(reference, target, "", "", nil); err != nil {
				return err
			}
		}
		for _, selection := range target.Selections {
			...
			for _, reference := range selection.Payloads {
				if err := addArtifact(reference, target, "", selection.Name, nil); err != nil {
					return err
				}
			}
		}
