...
func validateLoadedRecordV1(record loadedRecordV1) error {
	...
	case *BindingContractV1:
		...
		for _, requirement := range value.Requirements {
			if _, err := pythonprovider.CanonicalPackageRequestV1(requirement); err != nil {
				return fmt.Errorf("binding requirement %q: %w", requirement, err)
			}
			if _, err := pythonprovider.RequirementDistributionName(requirement); err != nil {
				return fmt.Errorf("binding requirement %q: %w", requirement, err)
			}
		}
	...
	case *NativePackageSetV1:
		...
		for _, requirement := range value.Requirements {
			if _, err := blueprint.ParseAPTPackageRequest(requirement); err != nil {
...
func wheelFilenameTagsV1(filename string) ([]string, error) {
	...
	tags := make([]string, 0, len(pythonTags)*len(abiTags)*len(platformTags))
...
func wheelPlatformTagCompatibleV1(tag string, platform string) bool {
	...
	for _, prefix := range []string{"manylinux_", "musllinux_"} {
		if components, found := strings.CutPrefix(policy, prefix); found {
