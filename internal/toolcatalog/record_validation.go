...
func validateLoadedRecordV1(record loadedRecordV1) error {
	...
	case *ReleaseManifestV1:
		if record.Schema != ReleaseManifestSchemaV1 || value.Schema != ReleaseManifestSchemaV1 || value.ID != record.ID || !validRecordIdentifierV1(value.Tool) || !validRecordSegmentV1(value.Version) {
		...
		for index, raw := range value.Provenance {
			...
			if index > 0 && value.Provenance[index-1] >= raw {
	...
	case *ArtifactSourceRecordV1:
		...
		for index, provenance := range value.Provenance {
			...
			if index > 0 && value.Provenance[index-1] >= provenance {
...
func validateSelectionRequestV1(selections SelectionRequestV1) error {
	...
	if minimum > uint64(len(selections.Options)) {
		return fmt.Errorf("minimum selections exceeds the declared options")
	}
	...
	if len(selections.Defaults) != 0 && uint64(len(selections.Defaults)) < minimum {
		return fmt.Errorf("default selections do not satisfy the minimum")
	}
	return nil
}
...
func validateValidationEvidenceV1(evidence ValidationEvidenceV1) error {
	if evidence.Schema != ValidationEvidenceSchemaV1 || !validRecordIdentifierV1(evidence.Tool) || !validRecordSegmentV1(evidence.Version) {
