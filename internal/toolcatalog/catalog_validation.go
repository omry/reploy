...
func (catalog *CatalogV1) validateCatalogGraphV1() error {
	for _, id := range catalog.sortedRecordIDsV1() {
		if err := catalog.validateRecordReferencesV1(catalog.records[id]); err != nil {
			return err
		}
	}
...
func (catalog *CatalogV1) validateRecordReferencesV1(record loadedRecordV1) error {
	...
	for _, edge := range catalogReferencesV1(record.Value) {
		...
		if target.Digest != edge.Reference.Digest {
			return fmt.Errorf("record %q reference %q has digest %s, want %s",
				record.ID, edge.Reference.ID, edge.Reference.Digest, target.Digest)
		}
...
func (catalog *CatalogV1) validateReachabilityV1() error {
	...
	reachable := make(map[string]struct{}, len(catalog.records))
	...
	visit = func(id string, depth int) error {
		...
		if state[id] == visiting {
			return fmt.Errorf("catalog records form a cycle at %q", id)
		}
		reachable[id] = struct{}{}
		if state[id] == settled {
			return nil
	...
	for _, id := range catalog.sortedRecordIDsV1() {
		if _, found := reachable[id]; !found {
			return fmt.Errorf("catalog record %q is unreachable from any tool record", id)
		}
	}
	return nil
}
		...
		if digest, size, ok := artifactContentV1(record.Value); ok {
			if previous, exists := sizes[digest]; exists && previous != size {
				return fmt.Errorf("catalog artifacts %q and %q share content digest %s but declare sizes %q and %q",
					owners[digest], id, digest, previous, size)
			}
			sizes[digest] = size
			owners[digest] = id
		}
...
// overlappingRecordPathsV1 reports whether two owned trees overlap. A shared
// unowned parent is allowed; owning a path inside another's tree is not.
func overlappingRecordPathsV1(left string, right string) bool {
	return recordPathOverlapsV1(left, right)
}

// packageRequirementNameV1 extracts the package name a native requirement pins.
func packageRequirementNameV1(requirement string) (string, error) {
	name, _, _ := strings.Cut(requirement, "=")
	if name == "" {
		return "", fmt.Errorf("native package requirement %q declares no package name", requirement)
	}
	return name, nil
}
