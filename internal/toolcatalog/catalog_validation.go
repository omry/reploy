...
func (catalog *CatalogV1) validateAcquisitionMappingsV1() error {
	...
	for _, key := range keys {
		digest, _, ok := artifactContentV1(catalog.records[key].Value)
		if !ok {
			continue
		}
		if _, found := mapped[digest]; !found {
			return fmt.Errorf("catalog artifact %q has content digest %s with no acquisition source mapping", key.ID, digest)
		}
	}
	return nil
}
