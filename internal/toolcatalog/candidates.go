...
// normalizedVersionDemandV1 gives an opaque group an exact coordinate and
// extracts a compact revision suffix only for schemes whose exact coordinates
// cannot contain '~'. Public parsing cannot make that distinction before the
// immutable tool version scheme is loaded.
func normalizedVersionDemandV1(tool *ToolRecordV1, constraints []string,
	...
	if tool.VersionScheme != "opaque" {
		normalized := make([]string, 0, len(constraints))
		for _, constraint := range constraints {
			...
			if separator > 0 {
				...
				if validateCanonicalDecimalV1("compact definition revision", compactRevision, true) == nil {
					...
					constraint = constraint[:separator]
				}
			}
			normalized = append(normalized, constraint)
		}
		sort.Strings(normalized)
		normalized = compactStringsV1(normalized)
...
func classifyRequestedVersionV1(releases []releaseEntryV1, constraint string) requestedVersionV1 {
	...
	requested := requestedVersionV1{Constrained: true, Token: constraint,
...
func validateRequestedVersionConstraintV1(scheme string, requested requestedVersionV1) error {
	if !requested.Constrained || requested.Exact || requested.Equality {
		return nil
	}
	switch scheme {
	case "semver":
		if _, err := semver.NewConstraints(requested.Token); err != nil {
			return fmt.Errorf("version constraint %q is invalid under %s", requested.Token, scheme)
		}
	case "integer":
		if _, err := parseIntegerConstraintV1(requested.Token); err != nil {
			return err
		}
	case "pep440":
		if _, err := pep440.NewSpecifiers(requested.Token); err != nil {
			return fmt.Errorf("version constraint %q is invalid under PEP 440", requested.Token)
		}
	case "opaque":
