...
func (catalog *CatalogV1) enumerateReleaseCandidatesV1(tool *ToolRecordV1, version string,
	...
	entries := make([]entry, 0, len(tool.Releases))
	for _, reference := range tool.Releases {
		...
		satisfies, err := releaseSatisfiesConstraintV1(tool.VersionScheme, manifest.Version, manifest.Aliases, version)
		if err != nil {
			return nil, err
		}
		if !satisfies {
		...
		entries = append(entries, entry{key: key, version: manifest.Version, revision: parsed})
	}
	...
	// version comparison is inert and revisions decide.
	sort.SliceStable(entries, func(left int, right int) bool {
		if entries[left].version != entries[right].version {
			return compareToolVersionsV1(tool.VersionScheme, entries[left].version, entries[right].version) > 0
		}
		return entries[left].revision > entries[right].revision
	})
	keys := make([]recordKeyV1, 0, len(entries))
...
// releaseSatisfiesConstraintV1 tests one release against a request's version
// constraint. An empty constraint accepts every release. An ordered scheme
// accepts a comparison constraint such as ">=1.55.0,<1.56.0" as well as an exact
// coordinate, because restricting enumeration to exact equality would make a
// range request select nothing. An opaque scheme has no ordering, so only exact
// coordinates and aliases are meaningful there.
func releaseSatisfiesConstraintV1(scheme string, upstream string, aliases []string,
	constraint string) (bool, error) {
	if constraint == "" {
		return true, nil
	}
	// The design gives each tool one collision-free lookup map of exact versions
	// and aliases, and every normalized input token resolves through it. An
	// equality constraint names such a token, so it is normalized to that token
	// before lookup. Comparing the whole constraint would make `==21` miss the
	// coordinate `21` and would push an opaque request down the ordering path,
	// which an opaque scheme has no answer for.
	normalized := strings.TrimSpace(strings.TrimPrefix(constraint, "=="))
	if matchesRequestedVersionV1(upstream, aliases, normalized) {
		return true, nil
	}
	if !strings.ContainsAny(constraint, "<>=!~,") || strings.HasPrefix(constraint, "==") {
		return false, nil
	}
	switch scheme {
	case "semver", "integer":
		constraints, err := semver.NewConstraints(constraint)
		if err != nil {
			return false, fmt.Errorf("version constraint %q is invalid under %s", constraint, scheme)
		}
	...
	case "pep440":
		...
		if err != nil {
			return false, fmt.Errorf("version constraint %q is invalid under PEP 440", constraint)
		}
...
func selectExactTargetV1(view map[string]loadedRecordV1, manifest *ReleaseManifestV1,
	...
	for _, reference := range manifest.Targets {
		...
		if selected != nil {
			return nil, fmt.Errorf("release targets %q and %q both match the observed base", selected.ID, target.ID)
		}
		selected = target
	}
	if selected == nil {
