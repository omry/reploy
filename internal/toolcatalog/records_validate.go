...
func validateLoadedRecordV1(record loadedRecordV1) error {
	...
	case *ReleaseManifestV1:
		...
		if err := validateProfileReferenceV1("release validation profile", value.ValidationProfile, releasePrefix); err != nil {
			return err
		}
	...
	case *TargetRecordV1:
		...
		if err := validateProfileReferenceV1("target validation profile", value.ValidationProfile, releasePrefix); err != nil {
			return err
		}
...
// Mirrors the validation profile ID rule, whose only published leaf is "default".
func validateProfileReferenceV1(field string, reference RecordReferenceV1, releasePrefix string) error {
	if reference.ID != releasePrefix+"/validation/profiles/default" {
		return fmt.Errorf("%s %q must be %q", field, reference.ID, releasePrefix+"/validation/profiles/default")
	}
	return nil
}
