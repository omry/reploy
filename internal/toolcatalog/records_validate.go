...
func validateSelectionSchemaV1(selections SelectionSchemaV1) error {
	...
	for index, combination := range selections.Combinations {
		if len(combination.Values) != len(selections.Dimensions) {
			return fmt.Errorf("selection combination %d must have one value set per dimension", index)
		}
		for dimensionIndex, values := range combination.Values {
			if err := requireNonemptySortedStringsV1("selection combination values", values); err != nil {
				return err
			}
			for _, value := range values {
				if !containsRecordValueV1(selections.Dimensions[dimensionIndex].Options, value) {
					return fmt.Errorf("selection combination value %q is not advertised for dimension %q", value, selections.Dimensions[dimensionIndex].Name)
				}
			}
		}
		if index > 0 && compareSelectionCombinationV1(selections.Combinations[index-1], combination) >= 0 {
...
func compareSelectionCombinationV1(left SelectionCombinationV1, right SelectionCombinationV1) int {
	for index := 0; index < len(left.Values) && index < len(right.Values); index++ {
		if compared := compareRecordStringSlicesV1(left.Values[index], right.Values[index]); compared != 0 {
			return compared
		}
	}
	switch {
	case len(left.Values) < len(right.Values):
		return -1
	case len(left.Values) > len(right.Values):
		return 1
	default:
