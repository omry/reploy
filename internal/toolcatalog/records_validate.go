...
func validateLoadedRecordV1(record loadedRecordV1) error {
	...
	case *ReleaseManifestV1:
		...
		if len(value.Aliases) > maxDefinitionReferences {
			return fmt.Errorf("release aliases must use at most %d entries", maxDefinitionReferences)
		}
	...
	case *ArtifactSourceRecordV1:
		...
		if len(value.Diagnostics) > maxDefinitionReferences {
			return fmt.Errorf("artifact source diagnostics must use at most %d entries", maxDefinitionReferences)
		}
	...
	case *NativePackageSetV1:
		...
		if len(value.Repositories) > maxDefinitionReferences {
			return fmt.Errorf("native package repositories must use at most %d entries", maxDefinitionReferences)
		}
		if err := validateSortedUniqueStringsV1("native package validation metadata", value.ValidationMetadata, false); err != nil {
			return err
		}
		if len(value.ValidationMetadata) > maxDefinitionReferences {
			return fmt.Errorf("native package validation metadata must use at most %d entries", maxDefinitionReferences)
		}
...
// binding artifacts. Every other in-release namespace is rejected.
func validateArtifactSourceTargetV1(reference RecordReferenceV1, releasePrefix string) error {
	if strings.HasPrefix(reference.ID, releasePrefix+"/payloads/") {
		return nil
	}
	segments := strings.Split(reference.ID, "/")
	if len(segments) >= 7 && strings.Join(segments[:4], "/") == releasePrefix+"/bindings" &&
		validRecordIdentifierV1(segments[4]) && segments[5] == "artifacts" {
		return nil
	}
...
func validateSelectionRequestV1(selections SelectionRequestV1) error {
	...
	if minimum > 0 {
		feasible := false
		for _, group := range selections.CompatibilityGroups {
			feasible = feasible || uint64(len(group)) >= minimum
		}
		if !feasible {
			return fmt.Errorf("minimum selections cannot be satisfied by any compatibility group")
		}
	}
...
func validateParameterSchemasV1(parameters []ParameterSchemaV1) error {
	...
	for index, parameter := range parameters {
		...
		case "enum":
			...
			domainSize = uint64(len(parameter.Values))
		case "integer":
			...
			domainSize = uint64(maximum-minimum) + 1
		default:
		...
		if domainSize == 0 || coverage > uint64(maxDefinitionValidationCases)/domainSize {
			return fmt.Errorf("contract parameter Cartesian coverage exceeds the validation case limit")
		}
		coverage *= domainSize
	}
	return nil
