...
// The per-artifact contract check proves one wheel agrees with the binding
// contract that consumes it: same distribution, a version its requirement
// admits, and an interpreter the contract advertises.
func TestBindingArtifactAgainstContractV1(t *testing.T) {
	contract := func() *BindingContractV1 {
		return &BindingContractV1{Name: "python", Package: "demo",
			Requirements:    []string{"demo==1.2.3", "support>=1,<2"},
			SupportedPython: []string{"3.11", "3.12"}}
	}
	artifact := func() *BindingArtifactRecordV1 {
		return &BindingArtifactRecordV1{ID: "artifact",
			Filename: "demo-1.2.3-py3-none-manylinux1_x86_64.whl", RequiresPython: ">=3.11"}
	}

	if err := validateBindingArtifactAgainstContractV1(contract(), artifact()); err != nil {
		t.Errorf("an agreeing wheel was rejected: %v", err)
	}

	for _, testCase := range []struct {
		name    string
		mutate  func(*BindingContractV1, *BindingArtifactRecordV1)
		wantSub string
	}{
		{name: "wheel identity has too few parts", wantSub: "wheel identity is invalid",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) { a.Filename = "demo-1.2.3.whl" }},
		{name: "distribution does not match the contract package", wantSub: "does not match contract package",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) {
				a.Filename = "other-1.2.3-py3-none-manylinux1_x86_64.whl"
			}},
		{name: "wheel version is not PEP 440", wantSub: "wheel version is invalid",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) {
				a.Filename = "demo-banana-py3-none-manylinux1_x86_64.whl"
			}},
		{name: "wheel version violates the contract requirement", wantSub: "does not satisfy contract requirement",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) {
				a.Filename = "demo-2.0.0-py3-none-manylinux1_x86_64.whl"
			}},
		{name: "contract package has no requirement", wantSub: "has no binding requirement",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) {
				c.Requirements = []string{"support>=1,<2"}
			}},
		{name: "requires_python is malformed", wantSub: "requires_python is invalid",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) { a.RequiresPython = "not-a-specifier" }},
		{name: "requires_python excludes every advertised interpreter", wantSub: "excludes every contract interpreter",
			mutate: func(c *BindingContractV1, a *BindingArtifactRecordV1) { a.RequiresPython = ">=4.0" }},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			c, a := contract(), artifact()
			testCase.mutate(c, a)
			err := validateBindingArtifactAgainstContractV1(c, a)
			if err == nil || !strings.Contains(err.Error(), testCase.wantSub) {
				t.Errorf("error = %v, want substring %q", err, testCase.wantSub)
			}
		})
	...
	// PTD-01 rejects extras in a package root requirement and record-local
	// validation rejects a contract carrying one, so a requirement that fails to
	// parse here is a defect and must surface rather than be skipped over.
	c, a := contract(), artifact()
	c.Requirements = []string{"demo[http]==1.2.3"}
	err := validateBindingArtifactAgainstContractV1(c, a)
	if err == nil || !strings.Contains(err.Error(), "must not request extras") {
		t.Errorf("unparseable contract requirement error = %v, want it surfaced", err)
	}
}

// Contributions selected together must agree: two package sets cannot pin one
// package differently, and two exports cannot claim one name for two paths.
func TestTupleContributionsRejectPackageAndExportConflictsV1(t *testing.T) {
	const release = "tool:demo/releases/1.2.3"
	packageSet := func(id string, requirements ...string) *NativePackageSetV1 {
		return &NativePackageSetV1{Schema: NativePackageSetSchemaV1, ID: id, Manager: "apt",
			Requirements: append([]string{}, requirements...)}
	}
	base := packageSet(release+"/package-sets/base", "libdemo=1.0")
	conflicting := packageSet(release+"/package-sets/conflicting", "libdemo=2.0")
	agreeing := packageSet(release+"/package-sets/agreeing", "libdemo=1.0")
	records := composeTestRecordsV1(base, conflicting, agreeing)
	...
	target.Selections = []TargetSelectionV1{
		{Name: "conflicting", PackageSets: []RecordReferenceV1{recordTestReference(conflicting.ID)}},
		{Name: "agreeing", PackageSets: []RecordReferenceV1{recordTestReference(agreeing.ID)}},
	}
	tuple := func(selection string) supportTupleV1 {
		return supportTupleV1{Context: "build", Selections: []string{selection}, Parameters: []ParameterValueV1{}}
	}

	if err := validateTupleContributionsV1(records, contract, target, tuple("agreeing")); err != nil {
		t.Errorf("package sets pinning the same version conflicted: %v", err)
	}
	err := validateTupleContributionsV1(records, contract, target, tuple("conflicting"))
	if err == nil || !strings.Contains(err.Error(), "conflict on package") {
		t.Errorf("package pin conflict error = %v", err)
	}

	// A package set reference that resolves to another record type fails.
	mistyped := composeTestTargetV1()
	mistyped.Payloads = []RecordReferenceV1{}
	mistyped.Selections = []TargetSelectionV1{}
	mistyped.PackageSets = []RecordReferenceV1{recordTestReference(contract.ID)}
	if err := validateTupleContributionsV1(records, contract, mistyped, tuple("")); err == nil {
		t.Error("a release contract was accepted as a package set")
	}

	// Two contributions claiming one export name with different paths conflict.
	exporting := composeTestTargetV1()
	exporting.Payloads = []RecordReferenceV1{}
	exporting.PackageSets = []RecordReferenceV1{}
	exporting.Exports = []ToolExportV1{{Name: "demo", Path: "/opt/demo/bin/other"}}
	exporting.Selections = []TargetSelectionV1{}
	err = validateTupleContributionsV1(records, contract, exporting,
		supportTupleV1{Context: "build", Selections: []string{}, Parameters: []ParameterValueV1{}})
	if err == nil || !strings.Contains(err.Error(), "conflict on export") {
		t.Errorf("export path conflict error = %v", err)
	}
}

// A request that omits selections normalizes to the contract defaults, so a
// target that advertises a subset excluding a default has no mapping to
// traverse for the request every consumer makes by default.
func TestTargetMustAdvertiseContractDefaultSelectionsV1(t *testing.T) {
	contract := composeRichContractV1()
	contract.Selections = SelectionRequestV1{
		Options: []string{"chromium", "firefox"}, Minimum: "1", Maximum: "1",
	...
	if err := validateTargetAgainstContractV1(contract, target); err != nil {
		t.Errorf("a target advertising both selections was rejected: %v", err)
	}

	// Advertising only the default is a legal subset.
	narrowed := composeRichTargetV1()
	narrowed.Selections = narrowed.Selections[:1]
	if err := validateTargetAgainstContractV1(contract, narrowed); err != nil {
		t.Errorf("a target advertising only the default was rejected: %v", err)
	}

	// Dropping the default is not, even though the remaining selection is valid.
	withoutDefault := composeRichTargetV1()
	withoutDefault.Selections = withoutDefault.Selections[1:]
	err := validateTargetAgainstContractV1(contract, withoutDefault)
	if err == nil || !strings.Contains(err.Error(), "default selection") {
		t.Errorf("target omitting the contract default selection error = %v", err)
	}
}

// The binding contract publishes constituent metadata to consumers, so an
// artifact must bundle what the contract advertises at the advertised version
// and path.
func TestBindingArtifactBundledComponentsMatchTheContractV1(t *testing.T) {
	contract := func() *BindingContractV1 {
		return &BindingContractV1{Name: "python", Package: "demo",
			Requirements: []string{"demo==1.2.3"}, SupportedPython: []string{"3.11"},
			BundledComponents: []BundledComponentV1{{Name: "nodejs", Version: "24.0.0", Path: "node"}}}
	}
	artifact := func(components ...BundledComponentV1) *BindingArtifactRecordV1 {
		return &BindingArtifactRecordV1{ID: "artifact",
			Filename: "demo-1.2.3-py3-none-manylinux1_x86_64.whl", RequiresPython: ">=3.11",
			BundledComponents: components}
	}

	if err := validateBindingArtifactAgainstContractV1(contract(),
		artifact(BundledComponentV1{Name: "nodejs", Version: "24.0.0", Path: "node"})); err != nil {
		t.Errorf("an artifact bundling exactly what the contract advertises was rejected: %v", err)
	}

	err := validateBindingArtifactAgainstContractV1(contract(), artifact())
	if err == nil || !strings.Contains(err.Error(), "does not bundle") {
		t.Errorf("missing bundled component error = %v", err)
	}
	err = validateBindingArtifactAgainstContractV1(contract(),
		artifact(BundledComponentV1{Name: "nodejs", Version: "23.0.0", Path: "node"}))
	if err == nil || !strings.Contains(err.Error(), "contract advertises") {
		t.Errorf("bundled component version mismatch error = %v", err)
	}
	err = validateBindingArtifactAgainstContractV1(contract(),
		artifact(BundledComponentV1{Name: "nodejs", Version: "24.0.0", Path: "vendor/node"}))
	if err == nil || !strings.Contains(err.Error(), "contract advertises") {
		t.Errorf("bundled component path mismatch error = %v", err)
	}

	// An artifact may bundle more than the contract advertises: the contract
	// publishes a floor of constituent metadata, not an exhaustive inventory.
	if err := validateBindingArtifactAgainstContractV1(contract(),
		artifact(BundledComponentV1{Name: "nodejs", Version: "24.0.0", Path: "node"},
			BundledComponentV1{Name: "playwright-core", Version: "1.2.3", Path: "package"})); err != nil {
		t.Errorf("an artifact bundling an extra component was rejected: %v", err)
	}
}

// With optional selections and a declared default, a fixture that omits its
// selections normalizes to the default, so the empty set is not a reachable
// tuple and must not be enumerated as one.
func TestOptionalSelectionsWithDefaultsAreCoverableV1(t *testing.T) {
	contract := composeRichContractV1()
	contract.Selections = SelectionRequestV1{
		Options: []string{"chromium", "firefox"}, Minimum: "0", Maximum: "1",
		Defaults: []string{"chromium"}, CompatibilityGroups: [][]string{{"chromium", "firefox"}},
	}
	target := composeRichTargetV1()
	records := composeTestRecordsV1()
	...
	for _, tuple := range tuples {
		if len(tuple.Selections) == 0 {
			t.Errorf("enumerated an empty selection tuple no fixture can normalize to: %+v", tuple)
		}
	}

	// Both advertised selections are coverable, and coverage now succeeds.
	chromium := composeFixtureV1("debian-12-amd64-chromium", "python", "chromium")
	firefox := composeFixtureV1("debian-12-amd64-firefox", "python", "firefox")
	if err := validateTargetFixtureCoverageV1(records, contract, target,
		[]*IntegrationFixtureRecordV1{chromium, firefox}); err != nil {
		t.Errorf("optional selections with a default were uncoverable: %v", err)
	}

	// A fixture that omits its selections normalizes to the default and so
	// covers the chromium tuple rather than a distinct empty one.
	implicit := composeFixtureV1("debian-12-amd64-default", "python")
	if err := validateTargetFixtureCoverageV1(records, contract, target,
		[]*IntegrationFixtureRecordV1{implicit, firefox}); err != nil {
		t.Errorf("a fixture relying on the default selection did not cover its tuple: %v", err)
	}

	// With no defaults declared, the empty set is reachable and must be covered.
	noDefaults := composeRichContractV1()
	noDefaults.Selections = SelectionRequestV1{
		Options: []string{"chromium", "firefox"}, Minimum: "0", Maximum: "1",
		Defaults: []string{}, CompatibilityGroups: [][]string{{"chromium", "firefox"}},
	}
	tuples, err = targetSupportTuplesV1(noDefaults, target)
	if err != nil {
		t.Fatal(err)
	}
	empty := false
	for _, tuple := range tuples {
		empty = empty || len(tuple.Selections) == 0
	}
	if !empty {
		t.Error("no defaults declared, so the empty selection set should be enumerated")
	}
}
