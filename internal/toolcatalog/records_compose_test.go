...
func composeTargetV1() *TargetRecordV1 {
	...
	return &TargetRecordV1{
		...
		SupportCases: []TargetSupportCaseV1{
			{Context: "runtime", Bindings: []string{"node"}, Selections: map[string][]string{"browser": {"firefox"}, "mode": {"full"}}},
			{Context: "build", Bindings: []string{"python"}, Selections: map[string][]string{"browser": {"chromium"}, "mode": {"full", "headless"}}},
		},
...
func TestSupportTuplesUseBindingSetsAndNamedSelectionCombinationsV1(t *testing.T) {
	...
	if len(tuples) != 2 {
		t.Fatalf("tuple count = %d, want 2 declared cases", len(tuples))
	}
	for _, tuple := range tuples {
		if tuple.Context == "runtime" && containsRecordValueV1(tuple.Bindings, "python") || tuple.Context == "build" && containsRecordValueV1(tuple.Bindings, "node") {
			t.Errorf("inferred cross-product tuple: %+v", tuple)
		}
	}
...
func TestSupportCaseEnumerationIsBoundedV1(t *testing.T) {
	contract, target := composeContractV1(), composeTargetV1()
	target.SupportCases = make([]TargetSupportCaseV1, maxDefinitionValidationCases+1)
	for index := range target.SupportCases {
		target.SupportCases[index] = TargetSupportCaseV1{Context: "build", Bindings: []string{"python"}, Selections: map[string][]string{"browser": {"chromium"}, "mode": {"full", "headless"}}}
	}
	if _, err := targetSupportTuplesV1(contract, target); err == nil || !strings.Contains(err.Error(), "limit") {
		t.Fatalf("unbounded support-case error = %v", err)
	}
}

func TestSupportCasePreservesOmittedOptionalDimensionsV1(t *testing.T) {
	contract, target := composeContractV1(), composeTargetV1()
	contract.Selections.Combinations = []SelectionCombinationV1{{"browser": {"chromium"}}}
	target.Selections = target.Selections[:1]
	target.SupportCases = []TargetSupportCaseV1{{
		Context: "build", Bindings: []string{"python"},
	...
	if len(tuples) != 1 || tuples[0].Selections["mode"] != nil {
		t.Fatalf("optional dimension omission was not preserved: %+v", tuples)
	}
}
...
func TestTargetAgainstContractRequiresCompleteAdvertisedCombinationsV1(t *testing.T) {
	...
	if err := validateTargetAgainstContractV1(contract, target); err == nil || !strings.Contains(err.Error(), "combination") {
...
func TestFixtureCoverageRequiresEveryTupleExactlyOnceV1(t *testing.T) {
	...
	target.SupportCases = []TargetSupportCaseV1{
		{Context: "build", Bindings: []string{"python"}, Selections: map[string][]string{"browser": {"chromium"}, "mode": {"full", "headless"}}},
		{Context: "build", Bindings: []string{"python"}, Selections: map[string][]string{"browser": {"firefox"}, "mode": {"full"}}},
	}
...
func TestBindingArtifactsAgreeWithContractAndExactReferencesV1(t *testing.T) {
	...
	missingRoot := *contract
	missingRoot.Requirements = []string{"support>=1,<2"}
	missingRootTarget := composeTargetV1()
	missingRootTarget.Bindings = missingRootTarget.Bindings[1:]
	if err := validateTargetBindingsAgainstContractsV1(composeTestRecordsV1(&missingRoot, artifact), missingRootTarget); err == nil || !strings.Contains(err.Error(), "no binding requirement") {
		t.Fatalf("missing binding-package requirement error = %v", err)
	}
}
