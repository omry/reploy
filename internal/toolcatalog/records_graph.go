...
func validateValidationEvidenceV1(evidence ValidationEvidenceV1) error {
	...
	// Sorted and unique is not enough: a selection name must be one the record
	// model can express, or the evidence claims support for a tuple no target
	// could ever advertise. This is the same rule the integration fixture
	// validator applies to its own selections.
	for _, selection := range evidence.Selections {
