...
func TestValidateExactJSONMembersV1SortsMapKeys(t *testing.T) {
	target := reflect.TypeOf(map[string]struct {
		Value string `json:"value"`
	}{})
	payload := json.RawMessage(`{"z":{"value":"ok","z_extra":true},"a":{"value":"ok","a_extra":true}}`)
	err := validateExactJSONMembersV1(payload, target)
	if err == nil || !strings.Contains(err.Error(), `unknown field "a_extra"`) {
		t.Fatalf("error = %v, want deterministic first map-key error", err)
	}
}
...
func TestValidateStrictJSONV1RejectsAmbiguousAndOversizedInput(t *testing.T) {
	...
	tooDeepEmptyArray := strings.Repeat("[", maxDefinitionJSONDepth+1) + strings.Repeat("]", maxDefinitionJSONDepth+1)
	tooDeepEmptyObject := strings.Repeat(`{"value":`, maxDefinitionJSONDepth) + `{}` + strings.Repeat("}", maxDefinitionJSONDepth)
	tests := []struct {
		name    string
		payload []byte
		want    string
		...
		{name: "too deep empty array", payload: []byte(tooDeepEmptyArray), want: "JSON nesting exceeds"},
		{name: "too deep empty object", payload: []byte(tooDeepEmptyObject), want: "JSON nesting exceeds"},
	...
	atDepthLimit := strings.Repeat("[", maxDefinitionJSONDepth) + strings.Repeat("]", maxDefinitionJSONDepth)
	if err := validateStrictJSONV1([]byte(atDepthLimit)); err != nil {
		t.Fatalf("empty containers at depth limit: %v", err)
	}
}
