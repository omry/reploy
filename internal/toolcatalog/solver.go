...
func addBindingRequirementClaimV1(claims *assignmentClaimsV1, domain string,
	...
	if !exists {
		claims.bindingRequirements[claimKey] = bindingRequirementClaimV1{
			owner: owner, requirements: []string{requirement},
		}
		return "", nil
	}
	...
	if !compatible {
		return fmt.Sprintf("binding requirement conflict in domain %q on %q between %s and %s",
			domain, key, previous.owner, owner), nil
	}
...
func addPythonInterpreterClaimV1(claims *assignmentClaimsV1, domain string,
	...
	if !exists {
		claims.pythonInterpreters[domain] = pythonInterpreterClaimV1{
			owner: owner, supported: append([]string{}, supported...),
		}
		return ""
	}
	...
	if len(intersection) == 0 {
		return fmt.Sprintf("Python interpreter conflict in domain %q between %s and %s",
			domain, previous.owner, owner)
	}
