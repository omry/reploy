...
func (catalog *CatalogV1) addCandidateClaimsV1(claims *assignmentClaimsV1,
	...
	if candidate.Contract.Runtime != nil {
		runtimeValue, err := canonical.Marshal(candidate.Contract.Runtime)
		if err != nil {
			return "", fmt.Errorf("candidate runtime constraint: %w", err)
		}
		if candidate.Contract.Runtime.InstallRoot != "" {
			if conflict := addOwnedPathClaimV1(claims, domains.Filesystem,
				candidate.Contract.Runtime.InstallRoot, string(runtimeValue), owner); conflict != "" {
				return conflict, nil
			}
		}
