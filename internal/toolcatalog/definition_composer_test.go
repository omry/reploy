...
func TestComposePortableToolCatalogV1RejectsInvalidComposition(t *testing.T) {
	...
	t.Run("nil authoring field", func(t *testing.T) {
		definitions := composerTestDefinitionsV1(t)
		manifest := composerRecordV1[*ReleaseManifestV1](t, definitions)
		manifest.ArtifactSources = nil
		assertComposerErrorV1(t, definitions, "artifact source mappings must use a bounded array")
	})
	...
	t.Run("exact digest matches duplicate definitions", func(t *testing.T) {
		definitions := composerTestDefinitionsV1(t)
		manifest := composerRecordV1[*ReleaseManifestV1](t, definitions)
		contract := composerRecordV1[*ReleaseContractV1](t, definitions)
		digest, err := canonical.Sum("portable-tool-record", portableToolRecordIdentityV1, contract)
		if err != nil {
			t.Fatal(err)
		}
		manifest.Contract.Digest = digest
		duplicate := cloneReleaseContractV1(contract)
		definitions = append(definitions, PortableToolDefinitionRecordV1{
			Path:   "demo/releases/1.2.3/contract-duplicate.json",
			Record: &duplicate,
		})
		assertComposerErrorV1(t, definitions, "matches conflicting definitions")
	})
