...
func validSelectionSetsForCoverageV1(request SelectionRequestV1) ([][]string, error) {
	...
	// A request that omits selections normalizes to the contract defaults, so
	// when defaults exist the empty set is not a reachable tuple: it is the
	// defaults tuple under another name. Enumerating it anyway would demand a
	// fixture that normalizedFixtureTupleV1 can never produce.
	if minimum == 0 && len(request.Defaults) == 0 {
...
func targetSupportTuplesV1(contract *ReleaseContractV1, target *TargetRecordV1) ([]supportTupleV1, error) {
	// Enumerate what this target advertises, not what the contract declares:
	// a target that supports one binding but not another advertises only the
	// tuples it can actually satisfy.
	bindings := make([]string, 0, len(target.Bindings)+1)
	...
	selections, err := validSelectionSetsForCoverageV1(targetAdvertisedSelectionsV1(contract, target))
	if err != nil {
		return nil, err
	}
...
// targetAdvertisesBindingV1 reports whether the target maps the named binding.
...
// targetAdvertisedSelectionsV1 narrows a contract's selection request to the
// symbols this target advertises, so coverage enumerates only satisfiable sets.
func targetAdvertisedSelectionsV1(contract *ReleaseContractV1, target *TargetRecordV1) SelectionRequestV1 {
	...
	narrowed := contract.Selections
	narrowed.Options = advertised
	narrowed.Defaults = intersectRecordValuesV1(contract.Selections.Defaults, advertised)
	groups := make([][]string, 0, len(contract.Selections.CompatibilityGroups))
	for _, group := range contract.Selections.CompatibilityGroups {
		if narrowedGroup := intersectRecordValuesV1(group, advertised); len(narrowedGroup) != 0 {
			groups = append(groups, narrowedGroup)
		}
	}
	narrowed.CompatibilityGroups = groups
	return narrowed
}

func intersectRecordValuesV1(values []string, allowed []string) []string {
	result := make([]string, 0, len(values))
	for _, value := range values {
		if containsRecordValueV1(allowed, value) {
			result = append(result, value)
		}
	}
	return result
...
func validateTargetAgainstContractV1(contract *ReleaseContractV1, target *TargetRecordV1) error {
	// A target advertises the subset of contract symbols that this exact OS
	// generation and architecture supports. Every advertised symbol has exactly
	// one entry and an unadvertised symbol cannot have one, but a target is not
	// required to advertise every option the contract declares: support is
	// derived from valid target leaves rather than promised contract-wide.
	for _, binding := range target.Bindings {
	...
	if contract.Binding.Required && len(target.Bindings) == 0 {
		return fmt.Errorf("release contract requires a binding but the target advertises none")
	}
	if contract.Binding.Default != "" && len(target.Bindings) != 0 &&
		!targetAdvertisesBindingV1(target, contract.Binding.Default) {
		return fmt.Errorf("target does not advertise the contract default binding %q", contract.Binding.Default)
	}
	for _, selection := range target.Selections {
		if !containsRecordValueV1(contract.Selections.Options, selection.Name) {
			return fmt.Errorf("target selection mapping %q is not declared by the release contract", selection.Name)
		}
	}
	minimumSelections, _ := strconv.ParseUint(contract.Selections.Minimum, 10, 63)
	if uint64(len(target.Selections)) < minimumSelections {
		return fmt.Errorf("release contract requires %d selections but the target advertises %d",
			minimumSelections, len(target.Selections))
	}
	// A request that omits selections normalizes to the contract defaults, so a
	// target that does not advertise a default has no contribution mapping to
	// traverse for the request every consumer makes by default. Narrowing
	// coverage around the gap would hide it.
	advertisedSelections := make([]string, 0, len(target.Selections))
	for _, selection := range target.Selections {
		advertisedSelections = append(advertisedSelections, selection.Name)
	}
	for _, defaulted := range contract.Selections.Defaults {
		if !containsRecordValueV1(advertisedSelections, defaulted) {
			return fmt.Errorf("target does not advertise the contract default selection %q", defaulted)
		}
	}
...
func validateBindingArtifactAgainstContractV1(contract *BindingContractV1, artifact *BindingArtifactRecordV1) error {
	...
	for _, requirement := range contract.Requirements {
		// Record-local validation already proved every contract requirement
		// parses, so a failure here is a defect rather than a requirement to
		// skip over.
		...
		// Extras cannot appear: PackageRootDistributionNameV1 rejects them, so
		// the specifier set begins at the first comparison operator.
		remainder := ""
	...
	// The binding contract publishes its constituent metadata to consumers, so
	// an artifact must actually bundle what the contract advertises, at the
	// version and path it advertises.
	...
	for _, declared := range contract.BundledComponents {
		...
		if present.Version != declared.Version {
			return fmt.Errorf("bundled component %q is version %q but the contract advertises %q",
				declared.Name, present.Version, declared.Version)
		}
		if present.Path != declared.Path {
			return fmt.Errorf("bundled component %q is at %q but the contract advertises %q",
				declared.Name, present.Path, declared.Path)
		}
	}
...
func validateTupleContributionsV1(records map[string]loadedRecordV1, contract *ReleaseContractV1, target *TargetRecordV1, tuple supportTupleV1) error {
	...
	// Probe identity is the complete canonical probe value, matching what
	// record-local validation accepts. Byte-identical probes deduplicate; the
	// same executable invoked with different arguments is two probes, not a
	// conflict, so both run.
	seenProbes := make(map[string]struct{}, len(probes))
	for _, probe := range probes {
		key, err := canonical.Marshal(probe)
		if err != nil {
			return fmt.Errorf("probe canonical form: %w", err)
		}
		seenProbes[string(key)] = struct{}{}
	}
	return nil
}
