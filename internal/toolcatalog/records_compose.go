...
func validateTupleContributionsV1(records map[string]loadedRecordV1, contract *ReleaseContractV1, target *TargetRecordV1, tuple supportTupleV1) error {
	...
	payloadReferences := append([]RecordReferenceV1{}, target.Payloads...)
	...
	for _, selected := range tuple.Selections {
		for _, selection := range target.Selections {
			if selection.Name == selected {
				...
				payloadReferences = append(payloadReferences, selection.Payloads...)
	...
	if err := validateTuplePayloadsV1(records, payloadReferences); err != nil {
		return err
	}
...
func validateTuplePayloadsV1(records map[string]loadedRecordV1, references []RecordReferenceV1) error {
	type owned struct {
		id        string
		directory string
	}
