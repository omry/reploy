...
// References are exact: ID and digest together. An artifact naming its binding's
// contract ID at a different digest is a dangling reference the graph must reject.
func TestBindingArtifactContractReferenceMustBeExactV1(t *testing.T) {
	...
	// The artifact agrees with its binding's contract reference exactly.
	if err := validateManifestResolvedGraphV1(manifest, records); err != nil &&
		strings.Contains(err.Error(), "names contract") {
		t.Fatalf("an exact contract reference was rejected: %v", err)
	}

	// Same contract ID, different digest: record-local validation still passes
	// because it only compares the ID.
	artifact.Contract = RecordReferenceV1{ID: artifact.Contract.ID,
		Digest: canonical.Digest("sha256:" + strings.Repeat("c", 64))}
	err := validateManifestResolvedGraphV1(manifest, records)
	if err == nil || !strings.Contains(err.Error(), "names contract") {
		t.Errorf("error = %v, want a contract digest disagreement", err)
	}
}

// Evidence selections must be names the record model can express. Sorted and
// unique alone lets evidence claim a tuple no target could advertise. This is
// the same gap that was fixed for integration fixture selections in PTD-04.
func TestValidationEvidenceSelectionsMustBeIdentifiersV1(t *testing.T) {
	for _, selection := range []string{"Chromium", "not/a-selection", "chromium.1", "1chromium"} {
		evidence := *validValidationEvidenceV1()
		evidence.Selections = []string{selection}
		if err := validateValidationEvidenceV1(evidence); err == nil {
			t.Errorf("evidence selection %q was accepted", selection)
		}
	}
	evidence := *validValidationEvidenceV1()
	evidence.Selections = []string{"chromium", "firefox"}
	if err := validateValidationEvidenceV1(evidence); err != nil {
