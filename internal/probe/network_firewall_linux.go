...
func installApplicationNetworkPolicyV1(policy applicationNetworkPolicyV1) error {
	...
	for _, family := range []struct {
		...
		if err := replaceApplicationNetworkTableV1(connection, family.tableFamily, family.local, family.publicExceptions, family.ambiguous, family.addressSize, policy, ports); err != nil {
			return err
		}
	}
...
func replaceApplicationNetworkTableV1(connection *nftables.Conn, family nftables.TableFamily, local []string, publicExceptions []string, ambiguous []string, addressSize uint32, policy applicationNetworkPolicyV1, ports []uint16) error {
