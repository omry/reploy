...
func Main(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) int {
	return mainWithHold(args, stdin, stdout, stderr, waitForHoldSignal)
}
...
func mainWithHold(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer, hold func() error) int {
	...
	if len(args) != 0 {
		_, _ = fmt.Fprintln(stderr, "reploy-probe accepts either no arguments for one canonical stdin request or the fixed hold mode")
		return 2
	}
