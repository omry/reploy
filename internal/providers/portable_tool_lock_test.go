...
func TestValidatePortableToolLockV1RejectsIncompleteOrAmbiguousAcquisitionEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nested source record name", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolTestRecordDigest(&lock.Releases[0].Manifest.Reference, lock.Releases[0].Manifest.Record)
			lock.Plan.PortableToolPlan.Tools[0].Provenance.ManifestDigest = lock.Releases[0].Manifest.Reference.Digest
		}, want: "must name an artifact source record in revision"},
		...
		{name: "empty manifest validation profiles", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockManifestDigestV1(lock)
		}, want: "release validation profiles must not be empty"},
		{name: "manifest omits selected validation profile", mutate: func(lock *PortableToolLockV1) {
			lock.Releases[0].Manifest.Record.Value["validation_profiles"] = []any{canonical.Object{
				"id": "tool:demo/releases/1.2.3/validation/profiles/other", "digest": string(portableToolTestDigest),
			}}
			refreshPortableToolLockManifestDigestV1(lock)
		}, want: "does not authorize selected validation profile"},
		{name: "manifest alias repeats exact version", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockManifestDigestV1(lock)
		}, want: "redundantly equals its exact version"},
		{name: "manifest contract outside release", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockManifestDigestV1(lock)
		}, want: "must identify the current release contract"},
		...
		{name: "manifest mapping to impossible artifact record", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockManifestDigestV1(lock)
		}, want: "must reference a payload or binding artifact record"},
		...
		}, want: "artifact source provenance must use a nonempty bounded array"},
		{name: "unsorted source provenance", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockSourceDigestV1(lock, 1)
		}, want: "artifact source provenance must be unique and sorted"},
		...
		{name: "missing source diagnostics", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockSourceDigestV1(lock, 1)
		}, want: "exactly the canonical fields"},
		...
		{name: "unsorted source diagnostics", mutate: func(lock *PortableToolLockV1) {
			...
			refreshPortableToolLockSourceDigestV1(lock, 1)
		}, want: "artifact source diagnostics must contain unique sorted canonical values"},
		...
		{name: "relative validation-profile probe", mutate: func(lock *PortableToolLockV1) {
			profile := &lock.Plan.PortableToolPlan.Tools[0].ValidationProfiles[0]
			profile.Record.Value["probes"].([]any)[0].(canonical.Object)["path"] = "bin/demo"
			refreshPortableToolTestRecordDigest(&profile.Reference, profile.Record)
		}, want: "probe must use an absolute path"},
	}
...
func TestValidatePortableToolLockV1RejectsArtifactAtCoreByteCapForEveryOutcome(t *testing.T) {
	tests := []struct {
		name    string
		outcome string
	}{
		{name: "network", outcome: providerstore.AcquisitionOutcomeNetwork},
		{name: "cache hit", outcome: providerstore.AcquisitionOutcomeCacheHit},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			dag, releases, inputs := portableToolLockFixtureV1(t)
			...
			plan := clonePortableToolPlanForPortableToolDAGV1(lock.Plan.PortableToolPlan)
			artifact := &plan.Tools[0].Responsibilities.BindingArtifacts[0]
			size := strconv.FormatInt(providerstore.CoreMaxArtifactBytesPerAttempt, 10)
			artifact.Record.Value["size"] = size
			refreshPortableToolTestRecordDigest(&artifact.Reference, artifact.Record)

			lock.Acquisitions[0].Artifact = artifact.Reference
			lock.Acquisitions[0].Descriptor.Size = size
			lock.Acquisitions[0].Outcome.Kind = test.outcome
			if test.outcome == providerstore.AcquisitionOutcomeCacheHit {
				lock.Acquisitions[0].Outcome.SuccessfulDeclaredLocator = ""
				lock.Acquisitions[0].Outcome.RedirectHops = "0"
			}
			mapping := lock.Releases[0].Manifest.Record.Value["artifact_sources"].([]any)[0].(canonical.Object)
			mapping["artifact"].(canonical.Object)["digest"] = string(artifact.Reference.Digest)
			refreshPortableToolLockManifestDigestV1(&lock)
			plan.Tools[0].Provenance.ManifestDigest = lock.Releases[0].Manifest.Reference.Digest
			lock.Plan, err = BuildPortableToolProviderDAGV1(lock.Plan.ProviderPlan, plan, lock.Plan.Domains)
			if err != nil {
				t.Fatal(err)
			}

			err = ValidatePortableToolLockV1(lock)
			if err == nil || !strings.Contains(err.Error(), "less than core per-attempt cap") {
				t.Fatalf("validation error = %v, want %s byte-cap rejection", err, test.outcome)
			}
		})
	}
}

func TestValidatePortableToolLockedCatalogRecordsV1RejectsIncompleteSelectedRecords(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing payload resolver", mutate: func(plan *PortableToolPlanV1) {
			delete(plan.Tools[0].Responsibilities.Payloads[0].Record.Value, "resolver")
		...
		{name: "missing package-set validation metadata", mutate: func(plan *PortableToolPlanV1) {
			delete(plan.Tools[0].Responsibilities.NativePackageSets[0].Record.Value, "validation_metadata")
		}, want: "portable-tool-package-set-v1 record must contain exactly the canonical fields"},
		{name: "relative validation-profile probe", mutate: func(plan *PortableToolPlanV1) {
			plan.Tools[0].ValidationProfiles[0].Record.Value["probes"].([]any)[0].(canonical.Object)["path"] = "bin/demo"
		}, want: "probe must use an absolute path"},
		{name: "nested validation-profile probe field", mutate: func(plan *PortableToolPlanV1) {
			plan.Tools[0].ValidationProfiles[0].Record.Value["probes"].([]any)[0].(canonical.Object)["unexpected"] = "value"
		}, want: "unknown field \"unexpected\""},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			candidate := clonePortableToolLockPlanV1(dag).PortableToolPlan
			test.mutate(&candidate)
			if err := validatePortableToolLockedCatalogRecordsV1(candidate); err == nil || !strings.Contains(err.Error(), test.want) {
				t.Fatalf("validation error = %v, want %q", err, test.want)
			}
		})
	}
...
func TestBuildPortableToolLockV1AuthorizesSelectedArtifactsByContentGroup(t *testing.T) {
	...
	manifest := portableToolLockManifestV1("demo", "1.2.3", "1", inputs[:1], plan.Tools[0].ValidationProfiles[0].Reference)
...
func TestPortableToolLockDistinguishesReleaseRevisionsSharingAClosure(t *testing.T) {
	...
	secondManifest := portableToolLockManifestV1("demo", "1.2.3", "2", secondInputs, secondPlan.Tools[0].ValidationProfiles[0].Reference)
...
func portableToolLockFixtureV1(t *testing.T) (PortableToolProviderDAGV1, []PortableToolReleaseManifestInputV1, []PortableToolArtifactAcquisitionInputV1) {
	...
	binding.Record.Value = canonical.Object{
		"schema": portableToolBindingArtifactSchemaV1, "id": binding.Reference.ID,
		"binding": "demo", "contract": canonical.Object{
			"id": contract.Reference.ID, "digest": string(contract.Reference.Digest),
		},
		"name": "demo", "ecosystem_version": "1.2.3", "platform": "linux/amd64",
		"filename": "demo-1.2.3-py3-none-any.whl", "size": "12", "sha256": string(portableToolTestDigest),
		"resolver": "https-sha256", "tags": []any{"py3-none-any"}, "requires_python": ">=3.10",
	...
	packageSet.Record.Value = canonical.Object{
		"schema": portableToolPackageSetSchemaV1, "id": packageSet.Reference.ID, "manager": "apt",
		"requirements": []any{"ca-certificates"}, "repositories": []any{}, "validation_metadata": []any{},
	}
	refreshPortableToolTestRecordDigest(&packageSet.Reference, packageSet.Record)
	...
	manifest := portableToolLockManifestV1("demo", "1.2.3", "1", inputs, profile.Reference)
	plan.Tools[0].Provenance.ManifestDigest = manifest.Reference.Digest
...
func portableToolLockManifestV1(tool, version, revision string, inputs []PortableToolArtifactAcquisitionInputV1, profile PortableToolRecordReferenceV1) PortableToolSelectedRecordV1 {
	...
	return portableToolTestSelectedRecord(
		...
		canonical.Object{
			...
			"validation_profiles": []any{canonical.Object{"id": profile.ID, "digest": string(profile.Digest)}},
