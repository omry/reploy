...
func TestProjectPortableToolAPTRootsV1MergesOrdinaryRootsWithoutMutatingInput(t *testing.T) {
	...
	input := []providers.ResolvedComponentRequestV1{portableAPTComponentForTest(t, "application/browser/os", "curl", "libasound2")}
	...
	if !containsAPTRoot(roots, "curl") || !containsAPTRoot(roots, "libasound2") || len(roots) != 34 {
