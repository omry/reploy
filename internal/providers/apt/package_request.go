...
func ValidateCanonicalPackageRequestV1(request providers.CanonicalPackageRequest) error {
	_, err := DecodeCanonicalPackageRequestV1(request)
	return err
}

func DecodeCanonicalPackageRequestV1(request providers.CanonicalPackageRequest) (blueprint.APTPackageRequest, error) {
	if request.Schema != PackageRequestSchemaV1 {
		return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request schema must be %q", PackageRequestSchemaV1)
	}
	name, ok := request.Value["name"].(string)
	if !ok {
		return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request name must be a string")
	}
	version := ""
	if value, exists := request.Value["version"]; exists {
		...
		if !valid {
			return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request version must be a string")
		}
	}
	exportValues, ok := request.Value["exports"].([]any)
	if !ok {
		return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request exports must be an array")
	}
	if len(request.Value) != 2 && !(len(request.Value) == 3 && version != "") {
		return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request contains unknown or empty optional fields")
	}
	...
	for index, value := range exportValues {
		...
		if !ok || len(object) != 2 {
			return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request export %d must contain name and executable", index)
		}
		exportName, nameOK := object["name"].(string)
		executable, executableOK := object["executable"].(string)
		if !nameOK || !executableOK {
			return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request export %d must contain string name and executable", index)
		}
		if _, exists := exports[exportName]; exists {
			return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request contains duplicate export %q", exportName)
		}
		exports[exportName] = blueprint.ExecutableExport{Executable: executable}
	}
	result := blueprint.APTPackageRequest{Name: name, Version: version, Exports: exports}
	normalized, err := CanonicalPackageRequestV1(result)
	if err != nil {
	...
	if !bytes.Equal(actual, expected) {
		return blueprint.APTPackageRequest{}, fmt.Errorf("APT package request is not canonically normalized")
	}
	return result, nil
}
