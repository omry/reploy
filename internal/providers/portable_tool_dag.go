...
func validatePortableToolProviderSharedClaimsV1(
	...
	for _, operation := range operations {
		...
		domain, domainFound := domainByScope[operation.Scope]
		if operation.Kind != PortableToolOperationAcquisitionBarrierV1 && !domainFound {
			return fmt.Errorf("portable tool provider operation %q uses an unknown scope", operation.ID)
		}
		...
		if operation.Kind == PortableToolOperationBindingContractV1 && operation.Record != nil {
			...
			if found {
				requirements, _, err := portableToolStringListFieldV1(selected.Record.Value, "requirements")
				if err != nil {
					return fmt.Errorf("binding contract requirements: %w", err)
				}
				for _, requirement := range requirements {
					distribution, err := PythonPackageRootDistributionNameV1(requirement)
					if err != nil {
						return fmt.Errorf("binding contract requirement %q: %w", requirement, err)
					}
		...
		if operation.Record != nil && (operation.Kind == PortableToolOperationBindingArtifactMaterializationV1 || operation.Kind == PortableToolOperationPayloadMaterializationV1) {
			...
			if found {
				if operation.Kind == PortableToolOperationPayloadMaterializationV1 {
					if logicalPath, present := selected.Record.Value["logical_path"]; present {
						logicalPathValue, ok := logicalPath.(string)
						if !ok || logicalPathValue == "" {
							return fmt.Errorf("payload logical_path must be a nonempty string")
						}
						if err := addClaim(operation.Domain+"\x00payload-logical-path:"+logicalPathValue,
