...
func validatePortableToolProviderOperationFieldsV1(operation PortableToolProviderOperationV1) error {
	...
	case PortableToolOperationExportV1:
		if operation.Record != nil || operation.InstallRoot != "" || operation.Environment != nil || operation.Export == nil || operation.Network != "" {
			return fmt.Errorf("export fields are inconsistent")
		}
	default:
...
func validatePortableToolProviderSharedClaimsV1(
	...
	for _, operation := range operations {
		...
		semanticKey, value, ok := portableToolProviderClaimV1(operation, entry)
		if ok {
			if err := addClaim(operation.Domain+"\x00"+semanticKey, value, operation.ID); err != nil {
				return err
			}
		}
		if operation.Kind == PortableToolOperationRuntimeInstallRootV1 {
			if err := addPortableToolFilesystemClaimV1(filesystemClaims, operation.Domain, operation.InstallRoot, "install-root", operation.ID); err != nil {
				return err
			}
		}
		if operation.Record != nil && (operation.Kind == PortableToolOperationBindingArtifactMaterializationV1 || operation.Kind == PortableToolOperationPayloadMaterializationV1) {
			...
			if found {
				...
				if err := addPortableToolFilesystemClaimV1(filesystemClaims, operation.Domain, destination,
					string(selected.Reference.Digest), operation.ID); err != nil {
					return err
				}
			}
		}
...
func portableToolProviderClaimV1(
	operation PortableToolProviderOperationV1,
	entry PortableToolPlanEntryV1,
) (string, string, bool) {
	switch operation.Kind {
	case PortableToolOperationRuntimeEnvironmentV1:
		return "environment:" + operation.Environment.Name, operation.Environment.Value, true
	case PortableToolOperationExportV1:
		return "export:" + operation.Export.Name, operation.Export.Path, true
	case PortableToolOperationBindingArtifactAcquisitionV1, PortableToolOperationBindingArtifactMaterializationV1,
		...
		PortableToolOperationBindingContractV1, PortableToolOperationNativePackageSetV1:
		if operation.Record == nil {
			return "", "", false
		}
		selected, found := portableToolFindSelectedRecordV1(entry, operation.Kind, *operation.Record)
		if !found {
			return "record:" + operation.Kind + ":" + operation.Record.ID, string(operation.Record.Digest), true
		}
		return "record:" + portableToolRecordResponsibilityKindV1(operation.Kind) + ":" + operation.Record.ID,
			string(selected.Reference.Digest) + "\x00" + portableToolFilesystemDestinationV1(selected), true
	default:
		return "", "", false
	}
}
...
func addPortableToolFilesystemClaimV1(
	...
	// aliases. Relative record-local names have identity semantics only.
	if claimedPath == "" {
		return nil
	}
	...
	claims[domain] = append(claims[domain], portableToolFilesystemClaimV1{path: claimedPath, value: value, owner: owner})
	return nil
}
