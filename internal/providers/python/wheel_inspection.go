...
func inspectWheelReader(ctx context.Context, reader io.ReaderAt, size int64, filename, logicalPath string, expected *providerstore.ArtifactDescriptor) (WheelInspectionV1, error) {
	...
	// Ordinary wheels may preserve distribution, version, and tag spelling.
	// Project their normalized identity through the shared canonical parser, while
	...
	if !strings.HasSuffix(tagSuffix, ".whl") {
		return result, fmt.Errorf("wheel filename must end in .whl")
	}
	tagParts := strings.Split(strings.TrimSuffix(tagSuffix, ".whl"), "-")
	if len(tagParts) != 3 && len(tagParts) != 4 {
		return result, fmt.Errorf("wheel filename must contain Python, ABI, and platform tags")
	}
	// Leave the optional build tag untouched; only compatibility tags have
	// case-insensitive identity. Expansion and grammar checks stay shared.
	for i := len(tagParts) - 3; i < len(tagParts); i++ {
		tagParts[i] = normalizeWheelTagCase(tagParts[i])
	}
	tagSuffix = strings.Join(tagParts, "-") + ".whl"
	...
	err = readBoundedWheelMember(ctx, selected["METADATA"], &consumed, func(r io.Reader) error {
		return readMetadataFields(r, "wheel metadata", func(name string) bool {
			...
			case "requires-python":
				var err error
				result.RequiresPython, err = normalizeWheelVersionSpecifiers(value)
				if err != nil {
	...
	err = readBoundedWheelMember(ctx, selected["WHEEL"], &consumed, func(r io.Reader) error {
		return readMetadataFields(r, "WHEEL metadata", func(name string) bool { return name == "wheel-version" || name == "root-is-purelib" || name == "tag" }, func(name, value string) error {
			...
			case "tag":
				value = normalizeWheelTagCase(value)
...
// Fold only ASCII letters so Unicode lookalikes cannot become valid tags.
// The shared validator still checks grammar after this spelling normalization.
func normalizeWheelTagCase(value string) string {
