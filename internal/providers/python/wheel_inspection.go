...
func inspectWheelPath(ctx context.Context, filename, logicalPath string, expected *providerstore.ArtifactDescriptor) (WheelInspectionV1, error) {
	...
	var descriptor providerstore.ArtifactDescriptor
	if expected != nil {
		descriptor = *expected
	} else {
		digest, err := wheelReaderDigest(ctx, file, info.Size())
		if err != nil {
			return WheelInspectionV1{}, err
		}
		descriptor = providerstore.ArtifactDescriptor{LogicalPath: logicalPath, Kind: "wheel", Size: strconv.FormatInt(info.Size(), 10), SHA256: digest}
	}
	return InspectWheelReaderV1(ctx, file, info.Size(), filepath.Base(filename), descriptor)
}
...
func InspectWheelReaderV1(ctx context.Context, reader io.ReaderAt, size int64, filename string, descriptor providerstore.ArtifactDescriptor) (WheelInspectionV1, error) {
	...
	// Ordinary wheels may preserve distribution casing and separators. Project
	...
	projection, err := portabletool.ProjectWheelFilenameV1(canonicalDistribution + "-" + suffix)
	if err != nil {
		return result, err
	}
	...
	err = readBoundedWheelMember(ctx, selected["METADATA"], &consumed, func(r io.Reader) error {
		return readMetadataFields(r, "wheel metadata", func(name string) bool {
			...
			case "requires-dist":
				name, err := RequirementDistributionName(value)
				if err != nil {
