...
func TestProjectPortableToolPythonBindingsV1RejectsConflictingOverridesAndDirectURLs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "same distribution VCS URL with egg", requirement: "git+https://example.invalid/demo.git#egg=demo", wantError: true, wantMessage: "conflicts with its selected exact wheel"},
		...
		{name: "unrelated VCS URL with egg", requirement: "git+https://example.invalid/other.git#egg=other"},
		{name: "unidentifiable credential-bearing VCS URL", requirement: "git+https://user:SENTINEL@example.invalid/private.git", wantError: true, wantMessage: "cannot be proven distinct", wantAbsent: "SENTINEL"},
		...
		{name: "unrelated named source archive", requirement: "other @ https://example.invalid/other-1.0.0.tar.gz"},
...
func TestProjectPortableToolPythonBindingsV1ProjectsSelectedPlaywrightRecords(t *testing.T) {
	...
	wantWheel := PortableToolExactWheelConstraintV1{
		Platform:         "linux/amd64",
		Filename:         "playwright-1.61.0-py3-none-manylinux1_x86_64.whl",
		Distribution:     "playwright",
		EcosystemVersion: "1.61.0",
		Tags:             []string{"py3-none-manylinux1_x86_64"},
		Size:             "47421381",
		SHA256:           canonical.Digest("sha256:54f3b39f6eab832e33458c1dd7da0b5682aedab3b09ae731b5c59fa12fd2024e"),
		RequiresPython:   ">=3.10",
	}
	if binding.Distribution != "playwright" || !reflect.DeepEqual(binding.Wheel, wantWheel) ||
