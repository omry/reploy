...
func TestProjectPortableToolPythonBindingsV1RejectsConflictingOverridesAndDirectURLs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "same distribution direct URL unknown bytes", requirement: "demo @ https://example.invalid/other.whl", wantError: true},
		{name: "same distribution unnamed wheel URL", requirement: "file:///tmp/demo-1.0.0-py3-none-any.whl", wantError: true},
		{name: "same distribution unnamed VCS URL", requirement: "git+https://example.invalid/demo.git#egg=demo", wantError: true},
		{name: "wheel URL mismatched egg", requirement: "https://example.invalid/demo-1.0.0-py3-none-any.whl#egg=other", wantError: true, wantMessage: "does not match wheel distribution"},
		...
		{name: "unrelated direct URL", requirement: "other @ https://example.invalid/other.whl"},
		{name: "unrelated unnamed wheel URL", requirement: "https://example.invalid/other-1.0.0-py3-none-any.whl"},
		{name: "unrelated unnamed VCS URL", requirement: "git+https://example.invalid/other.git#egg=other"},
