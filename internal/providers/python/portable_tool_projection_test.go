...
func TestCanonicalPortableToolPythonProjectionBytesV1RejectsMixedComponentPlatforms(t *testing.T) {
	_, projection, err := ProjectPortableToolPythonBindingsV1(
		portableToolPythonTwoBindingPlanForTest(t),
		[]providers.ResolvedComponentRequestV1{},
	)
	...
	const armTag = "py3-none-manylinux_2_17_aarch64"
	component.Bindings[1].Artifact.ID = "tool:demo/releases/1.0.0/bindings/zpython/artifacts/linux-arm64"
	component.Bindings[1].SupportedTags = []string{armTag}
	component.Bindings[1].Wheel.Platform = "linux/arm64"
	component.Bindings[1].Wheel.Filename = "other-1.0.0-py3-none-manylinux_2_17_aarch64.whl"
	component.Bindings[1].Wheel.Tags = []string{armTag}
	component.TestedTags = []string{"py3-none-manylinux1_x86_64", armTag}

	_, err = CanonicalPortableToolPythonProjectionBytesV1(projection)
	if err == nil || !strings.Contains(err.Error(), "bindings must target one platform") {
		t.Fatalf("error = %v, want mixed component platforms rejected", err)
	}
}
