...
type PortableToolPythonBindingVerificationInputV1 struct {
	// SelectedPlanEntry is the authoritative provider-neutral plan value from
	// which the projection was derived. Keeping it at this boundary prevents a
	// caller from changing all projected copies of scope, closure identity, or
	// selected record references together and having the self-consistent copies
	// accepted as an independently selected binding.
...
func VerifyPortableToolPythonBindingV1(
	...
	// Validate the authoritative selected plan entry independently of the
	// projection. A one-entry plan preserves the provider-neutral plan's full
	// structural validation without adding a second projection or plan parser.
	if err := providerapi.ValidatePortableToolPlanV1(providerapi.PortableToolPlanV1{
		Schema: providerapi.PortableToolPlanSchemaV1,
		Tools:  []providerapi.PortableToolPlanEntryV1{input.SelectedPlanEntry},
	}); err != nil {
		return result, fmt.Errorf("portable Python binding selected plan entry: %w", err)
	}
	if input.Binding.Scope != input.SelectedPlanEntry.Scope {
		return result, fmt.Errorf("portable Python binding scope does not match authoritative selected plan entry")
	}
	if input.Binding.SelectedClosureDigest != input.SelectedPlanEntry.SelectedClosureDigest {
		return result, fmt.Errorf("portable Python binding selected closure digest does not match authoritative selected plan entry")
	}
	...
	contractMember, err := portableToolPythonSelectedRecordMemberV1(
		input.ContractRecord,
		input.SelectedPlanEntry.Responsibilities.BindingContracts,
	)
	if err != nil {
		return result, fmt.Errorf("portable Python binding contract selected record membership: %w", err)
	}
	if !contractMember {
		return result, fmt.Errorf("portable Python binding contract selected record is not an exact member of the authoritative selected plan entry")
	}
	artifactMember, err := portableToolPythonSelectedRecordMemberV1(
		input.ArtifactRecord,
		input.SelectedPlanEntry.Responsibilities.BindingArtifacts,
	)
	if err != nil {
		return result, fmt.Errorf("portable Python binding artifact selected record membership: %w", err)
	}
	if !artifactMember {
		return result, fmt.Errorf("portable Python binding artifact selected record is not an exact member of the authoritative selected plan entry")
	}
	...
	if !portableToolExportSelectedV1(input.SelectedPlanEntry.Exports, contract.CLI) {
		return result, fmt.Errorf("portable Python binding CLI is not an exact export of the authoritative selected plan entry")
	}
...
// portableToolPythonSelectedRecordMemberV1 checks both the exact reference and
// the canonical selected-record envelope. References are content-addressed, but
// comparing the retained envelope as well makes the selected-plan membership
// assertion explicit at this boundary.
func portableToolPythonSelectedRecordMemberV1(
	selected providerapi.PortableToolSelectedRecordV1,
	records []providerapi.PortableToolSelectedRecordV1,
) (bool, error) {
	selectedBytes, err := canonical.Marshal(selected.Record)
	if err != nil {
		return false, err
	}
	for _, candidate := range records {
		if candidate.Reference != selected.Reference {
			continue
		}
		candidateBytes, err := canonical.Marshal(candidate.Record)
		if err != nil {
			return false, err
		}
		return bytes.Equal(candidateBytes, selectedBytes), nil
	}
	return false, nil
}
