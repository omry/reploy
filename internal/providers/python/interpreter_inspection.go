...
architecture_aliases={"x86_64":("x86_64",),"aarch64":("aarch64","arm64"),"armv7l":("armv7l",)}
if not any(item.platform != "any" and any(item.platform.endswith("_"+architecture) for architecture in architecture_aliases[target]) for item in all_tags): raise RuntimeError("interpreter architecture does not match selected target")
