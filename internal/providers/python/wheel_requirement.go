...
func wheelRequirementDistributionName(requirement string) (string, error) {
	if !utf8.ValidString(requirement) {
		return "", fmt.Errorf("requirement contains invalid UTF-8")
	}
	...
	value := strings.Trim(requirement, " \t")
	if value == "" {
	...
	// Only quoted marker literals may contain non-ASCII text. The requirement
	// body and unquoted marker tokens retain their ASCII grammar.
	for i := 0; i < len(body); i++ {
		if body[i] >= 0x7f {
			return "", fmt.Errorf("requirement body contains invalid character")
		}
	}
...
func splitWheelRequirementMarker(value string) (string, string, error) {
	...
	suffix := strings.Trim(value[len(body):], " \t")
