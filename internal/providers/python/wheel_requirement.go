		...
		if strings.HasPrefix(remainder, "@") {
			...
			if strings.HasPrefix(remainder, "(") {
				if !strings.HasSuffix(remainder, ")") {
					return "", fmt.Errorf("unclosed specifier parentheses")
				}
				remainder = strings.TrimSpace(remainder[1 : len(remainder)-1])
			}
			if remainder == "" {
				return "", fmt.Errorf("empty version specifier")
			}
			if err := validateWheelRequirementSpecifiers(remainder); err != nil {
...
func validateWheelRequirementSpecifiers(value string) error {
	...
	for index, part := range parts {
		...
		// Arbitrary equality explicitly permits non-PEP-440 version text.
		if match[1] != "===" {
			if _, err := pep440.NewSpecifiers(part); err != nil {
				return err
			}
		}
	}
