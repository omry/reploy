...
func TestOrdinaryWheelInspectionPreservesMixedCaseFilename(t *testing.T) {
	...
	content := testInspectionWheel(t, filename, "PyYAML", "6.0.2", "Metadata-Version: 2.1\nName: PyYAML\nVersion: 6.0.2\nRequires-Python: >=3.11RC1\nRequires-Dist: requests===foo/bar; python_implementation == 'CPython' and platform_release != 'café'\n", "Wheel-Version: 1.0\nRoot-Is-Purelib: true\nTag: py3-none-any\n", "")
	...
	if observed.Distribution != "pyyaml" || observed.Version != "6.0.2" || observed.RequiresPython != ">=3.11rc1" || observed.Filename != filename || observed.Artifact != descriptor || !reflect.DeepEqual(observed.FilenameTags, []string{"py3-none-any"}) {
...
func TestOrdinaryWheelInspectionNormalizesRequiresPython(t *testing.T) {
	for _, tc := range []struct{ input, want string }{
		{">=3.11RC1", ">=3.11rc1"},
		{">=v03.011, < 4", ">=3.11,<4"},
		{"!=3.11PRE1", "!=3.11rc1"},
		{">=3.11POST, !=3.12DEV", ">=3.11.post0,!=3.12.dev0"},
		{"~=3.12.00", "~=3.12.0"},
		{"==03.011.*", "==3.11.*"},
		{"(>=3.11RC1, <4,)", ">=3.11rc1,<4"},
		{"===v03.11RC1", "===v03.11rc1"},
		...
		t.Run(tc.input, func(t *testing.T) {
			...
			if err != nil || got.RequiresPython != tc.want {
				t.Fatalf("Requires-Python = %q, %v; want %q", got.RequiresPython, err, tc.want)
			}
		})
	}
	for _, value := range []string{"", "*", "=3.11", "3.11", ">=3.11<4", ">=3.11||<4", ">=3.11,,<4", ">=3 .11", ">=3.11.*", "==3.11RC1.*", ">=3.11+LOCAL", "~=3", ">=3.11; os_name == 'posix'", "(>=3.11", "()"} {
		filename := "demo-1-py3-none-any.whl"
		...
		if _, err := InspectWheelReaderV1(context.Background(), bytes.NewReader(content), int64(len(content)), filename, testInspectionDescriptor(content, filename)); err == nil {
			t.Errorf("accepted malformed Requires-Python %q", value)
		}
	}
}

func TestOrdinaryWheelInspectionNormalizesVersionSpelling(t *testing.T) {
	for _, version := range []string{"1.0RC1", "1.0dev", "1.0+LOCAL"} {
		t.Run(version, func(t *testing.T) {
			filename := "demo-" + version + "-py3-none-any.whl"
			content := testInspectionWheel(t, filename, "demo", version, "Name: demo\nVersion: "+version+"\n", "Wheel-Version: 1.0\nRoot-Is-Purelib: true\nTag: py3-none-any\n", "")
			descriptor := testInspectionDescriptor(content, filename)
			observed, err := InspectWheelReaderV1(context.Background(), bytes.NewReader(content), int64(len(content)), filename, descriptor)
			if err != nil {
				t.Fatal(err)
			}
			if observed.Filename != filename || observed.Artifact != descriptor || observed.Version == version {
				t.Fatalf("filename/version observation = %#v", observed)
			}
			if _, err := portabletool.ProjectWheelFilenameV1(filename); err == nil {
				t.Fatal("portable catalog accepted noncanonical version spelling")
			}
			dir := t.TempDir()
			file := filepath.Join(dir, filename)
			if err := os.WriteFile(file, content, 0o600); err != nil {
			...
			if _, _, err := DescribeSourceWheelFileV1(file, descriptor.LogicalPath); err != nil {
				t.Fatal(err)
			}
			if _, err := InspectSourceWheelFileV1(file, descriptor); err != nil {
				t.Fatal(err)
			}
			if _, err := InspectPreparedWheelDistributionsV1(context.Background(), dir); err != nil {
				t.Fatal(err)
			}
			if _, err := InspectWheelDeclaredDependenciesV1(file, nil); err != nil {
				t.Fatal(err)
			}
			if _, err := InspectWheelDeclaredDependenciesReaderV1(bytes.NewReader(content), int64(len(content)), descriptor, nil); err != nil {
				t.Fatal(err)
			}
		})
	}
...
func TestOrdinaryWheelInspectionNormalizesTagCase(t *testing.T) {
	for _, suffix := range []string{"Py2.pY3-NoNe-ANY", "1Build-Py2.pY3-NoNe-ANY"} {
		t.Run(suffix, func(t *testing.T) {
			filename := "demo-1-" + suffix + ".whl"
			...
			if observed.Filename != filename || observed.Artifact != descriptor || !reflect.DeepEqual(observed.FilenameTags, want) || !reflect.DeepEqual(observed.InternalTags, want) {
				t.Fatalf("normalized tags = %#v", observed)
			}
			wrong := descriptor
			wrong.LogicalPath = strings.ToLower(wrong.LogicalPath)
			if _, err := InspectWheelReaderV1(context.Background(), bytes.NewReader(content), int64(len(content)), filename, wrong); err == nil {
				t.Fatal("accepted a descriptor with normalized filename spelling")
			}
			if _, err := portabletool.ProjectWheelFilenameV1(filename); err == nil {
				t.Fatal("portable catalog accepted noncanonical tags")
			}
			dir := t.TempDir()
			file := filepath.Join(dir, filename)
			if err := os.WriteFile(file, content, 0o600); err != nil {
			...
			if got, _, err := DescribeSourceWheelFileV1(file, descriptor.LogicalPath); err != nil || got != descriptor {
				t.Fatalf("source description = %#v, %v", got, err)
			}
			if _, err := InspectSourceWheelFileV1(file, descriptor); err != nil {
				t.Fatal(err)
			}
			...
			if got, err := inspectWheel(context.Background(), file); err != nil || !reflect.DeepEqual(got.Tags, want) {
				t.Fatalf("prepared tags = %#v, %v", got, err)
			}
			if _, err := InspectWheelDeclaredDependenciesV1(file, []string{"requests"}); err != nil {
				t.Fatal(err)
			}
			if _, err := InspectWheelDeclaredDependenciesReaderV1(bytes.NewReader(content), int64(len(content)), descriptor, []string{"requests"}); err != nil {
				t.Fatal(err)
			}
		})
	}
...
func TestOrdinaryWheelTagNormalizationRejectsMalformedTags(t *testing.T) {
	for _, tc := range []struct{ name, filenameTag, internalTags string }{
		{"duplicate filename tags", "Py3.py3-none-any", "Tag: py3-none-any\n"},
		{"duplicate internal tags", "py3-none-any", "Tag: PY3-none-ANY\nTag: py3-none-any\n"},
		{"unicode filename tag", "py3-none-K", "Tag: py3-none-k\n"},
		{"unicode internal tag", "py3-none-k", "Tag: py3-none-K\n"},
		{"compressed internal tag", "Py2.Py3-none-any", "Tag: Py2.Py3-none-any\n"},
		{"uppercase extension", "py3-none-any.WHL", "Tag: py3-none-any\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			filename := "demo-1-" + tc.filenameTag + ".whl"
			if tc.name == "uppercase extension" {
				filename = strings.TrimSuffix(filename, ".whl")
			}
			content := testInspectionWheel(t, filename, "demo", "1", "Name: demo\nVersion: 1\n", "Wheel-Version: 1.0\nRoot-Is-Purelib: true\n"+tc.internalTags, "")
			if _, err := InspectWheelReaderV1(context.Background(), bytes.NewReader(content), int64(len(content)), filename, testInspectionDescriptor(content, filename)); err == nil {
				t.Fatal("accepted malformed or duplicate tags")
			}
		})
	}
}

func TestWheelInspectionDerivesDescriptorWithoutExtraDigestPass(t *testing.T) {
	filename := "demo-1-py3-none-any.whl"
	content := testInspectionWheel(t, filename, "demo", "1", "Name: demo\nVersion: 1\n", "Wheel-Version: 1.0\nRoot-Is-Purelib: true\nTag: py3-none-any\n", "")
	descriptor := testInspectionDescriptor(content, filename)
	derivedReader := &inspectionReadCounter{Reader: bytes.NewReader(content)}
	derived, err := inspectWheelReader(context.Background(), derivedReader, int64(len(content)), filename, descriptor.LogicalPath, nil)
	if err != nil {
		t.Fatal(err)
	}
	verifiedReader := &inspectionReadCounter{Reader: bytes.NewReader(content)}
	verified, err := InspectWheelReaderV1(context.Background(), verifiedReader, int64(len(content)), filename, descriptor)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(derived, verified) || derivedReader.reads != verifiedReader.reads {
		t.Fatalf("derived and verified inspections differ: reads %d / %d", derivedReader.reads, verifiedReader.reads)
	}
	if derived.Artifact != descriptor {
		t.Fatalf("derived descriptor = %#v, want %#v", derived.Artifact, descriptor)
	}
}

func TestInspectWheelReaderV1ValidatesCompleteRequiresDist(t *testing.T) {
	for _, requirement := range []string{"requests !!!", "requests[bad,,extra]", "requests >=no-version", "requests; python_version >", "requests; unknown_variable == 'x'"} {
		t.Run(requirement, func(t *testing.T) {
			filename := "demo-1-py3-none-any.whl"
			content := testInspectionWheel(t, filename, "demo", "1", "Name: demo\nVersion: 1\nRequires-Dist: "+requirement+"\n", "Wheel-Version: 1.0\nRoot-Is-Purelib: true\nTag: py3-none-any\n", "")
			if _, err := InspectWheelReaderV1(context.Background(), bytes.NewReader(content), int64(len(content)), filename, testInspectionDescriptor(content, filename)); err == nil {
				t.Fatal("accepted malformed consumed Requires-Dist")
			}
		})
	}
}
