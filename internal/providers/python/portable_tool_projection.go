...
func portableToolPythonRequirementIdentityV1(requirement string) (string, string, error) {
	...
	if name != "" {
		...
		if strings.HasPrefix(remainder, "@") {
			...
			distribution, err := portableToolPythonSourceLocationDistributionV1(location)
			...
			return distribution, "direct URL", nil
		}
	...
	if err == nil && portableToolPythonDirectURLSchemeV1(parsed.Scheme) {
		distribution, err := portableToolPythonSourceLocationDistributionV1(body)
		return distribution, "direct URL", err
	}
...
// before source classification. Pip treats the first semicolon as the marker
// delimiter for both named direct references and ordinary requirements.
...
func portableToolPythonSourceLocationDistributionV1(location string) (string, error) {
	...
	if strings.HasSuffix(strings.ToLower(filename), ".whl") {
		...
		return RequirementDistributionName(wheelRequirement)
	}
	return "", nil
}
