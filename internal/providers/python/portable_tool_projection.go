...
func mergePortableToolPythonRequirementsV1(request *PythonProviderRequestV1, bindings []PortableToolPythonBindingV1) error {
	...
	for _, requirement := range request.Requirements {
		...
		distribution, sourceKind, err := portableToolPythonRequirementIdentityV1(value)
		if err != nil {
			return err
		}
		if sourceKind != "" && distribution == "" {
			return fmt.Errorf("unnamed %s Python package requirement %q cannot be proven distinct from selected portable wheels", sourceKind, value)
		}
		if _, bound := bindingsByDistribution[distribution]; bound && sourceKind != "" {
...
func portableToolPythonRequirementIdentityV1(requirement string) (string, string, error) {
	...
	if name != "" {
		...
		if strings.HasPrefix(remainder, "@") {
			return NormalizeDistributionName(name), "direct URL", nil
		}
	}
	parsed, err := url.Parse(value)
	if err == nil && portableToolPythonDirectURLSchemeV1(parsed.Scheme) {
		distribution, err := portableToolPythonSourceLocationDistributionV1(value, parsed)
		return distribution, "direct URL", err
	}
	if portableToolPythonLocalPathV1(value) {
		parsed = &url.URL{Path: value}
		distribution, err := portableToolPythonSourceLocationDistributionV1(value, parsed)
		return distribution, "local path", err
	}
	if err != nil {
		return "", "", err
	}
	distribution, nameErr := RequirementDistributionName(value)
	return distribution, "", nameErr
}

func portableToolPythonSourceLocationDistributionV1(value string, parsed *url.URL) (string, error) {
	var eggDistribution string
	fragment, err := url.ParseQuery(parsed.Fragment)
	if err != nil {
		return "", fmt.Errorf("Python package source requirement %q has an invalid fragment", value)
	}
	if eggs := fragment["egg"]; len(eggs) > 0 {
		if len(eggs) != 1 {
			return "", fmt.Errorf("Python package source requirement %q has ambiguous egg names", value)
		}
		eggName := requirementNamePattern.FindString(eggs[0])
		if eggName == "" {
			return "", fmt.Errorf("Python package source requirement %q has an invalid egg name", value)
		}
		remainder := strings.TrimSpace(strings.TrimPrefix(eggs[0], eggName))
		if remainder != "" && !(strings.HasPrefix(remainder, "[") && strings.HasSuffix(remainder, "]")) {
			return "", fmt.Errorf("Python package source requirement %q has an invalid egg name", value)
		}
		eggDistribution = NormalizeDistributionName(eggName)
	}
	filename := path.Base(parsed.Path)
	if strings.HasSuffix(strings.ToLower(filename), ".whl") {
		wheel, err := portabletool.ProjectWheelFilenameV1(filename)
		if err != nil {
			return "", fmt.Errorf("Python package source requirement %q has an invalid wheel filename: %w", value, err)
		}
		if eggDistribution != "" && eggDistribution != wheel.Distribution {
			return "", fmt.Errorf("Python package source requirement %q egg distribution %q does not match wheel distribution %q", value, eggDistribution, wheel.Distribution)
		}
		return wheel.Distribution, nil
	}
	return eggDistribution, nil
}
...
func portableToolPythonLocalPathV1(value string) bool {
	...
	for _, suffix := range []string{".whl", ".zip", ".tar.gz", ".tar.bz2", ".tar.xz", ".tgz"} {
