	...
	for index, component := range projection.Components {
		...
		for bindingIndex, binding := range component.Bindings {
			...
			if componentPlatform == "" {
				componentPlatform = binding.Wheel.Platform
			} else if binding.Wheel.Platform != componentPlatform {
				return nil, fmt.Errorf("portable Python projection component %q bindings must target one platform", component.Component)
			}
