...
func TestVerifyPortableToolPythonBindingV1SelectedPlaywrightWheel(t *testing.T) {
	...
	input := PortableToolPythonBindingVerificationInputV1{
		SelectedPlanEntry: clonePortableToolPlanEntryForVerificationTest(t, plan.Tools[0]),
...
func TestVerifyPortableToolPythonBindingV1RejectsBoundaryMismatches(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "projected closure digest",
			mutate: func(input *PortableToolPythonBindingVerificationInputV1) {
				// Mutating both projected copies keeps the projection internally
				// self-consistent. Verification must still compare them with the
				// independently supplied selected plan entry.
				digest := canonical.Digest("sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
				input.Binding.SelectedClosureDigest = digest
				input.Component.Bindings[0].SelectedClosureDigest = digest
			},
			want: "selected closure digest does not match authoritative",
		},
		{
			name: "projected scope",
			mutate: func(input *PortableToolPythonBindingVerificationInputV1) {
				// Update every projected owner copy so the projection remains
				// canonical; the authoritative selected entry must still win.
				input.Binding.Scope = "application:other"
				input.Binding.Component = "application/other/python"
				input.Component.Component = "application/other/python"
				input.Component.Bindings[0].Scope = input.Binding.Scope
				input.Component.Bindings[0].Component = input.Binding.Component
			},
			want: "scope does not match authoritative",
		},
		{
			name: "contract record membership",
			mutate: func(input *PortableToolPythonBindingVerificationInputV1) {
				input.ContractRecord.Record.Value["cli"] = map[string]any{
					"name": "demo", "path": "/opt/demo/bin/other",
				}
				portableToolProjectionRebindRecordForTest(t, &input.ContractRecord)
				input.Binding.Contract = input.ContractRecord.Reference
				input.Binding.CLI.Path = "/opt/demo/bin/other"
				input.Component.Bindings[0].Contract = input.Binding.Contract
				input.Component.Bindings[0].CLI.Path = input.Binding.CLI.Path
			},
			want: "contract selected record is not an exact member",
		},
		{
			name: "artifact record membership",
			mutate: func(input *PortableToolPythonBindingVerificationInputV1) {
				input.ArtifactRecord.Record.Value["size"] = "8"
			...
			want: "artifact selected record is not an exact member",
		},
		{
			name: "selected export absent",
			mutate: func(input *PortableToolPythonBindingVerificationInputV1) {
				input.SelectedPlanEntry.Exports = []providers.PortableToolExportV1{}
			},
			want: "not an exact export of the authoritative selected plan entry",
		},
		{
			name: "selected export path changed",
			mutate: func(input *PortableToolPythonBindingVerificationInputV1) {
				input.SelectedPlanEntry.Exports[0].Path = "/opt/demo/bin/other"
			},
			want: "not an exact export of the authoritative selected plan entry",
		},
	} {
...
func addBundledComponentForVerificationTest(
	...
	portableToolProjectionRebindRecordForTest(t, &input.ArtifactRecord)
	// Keep the authoritative selected-plan copy aligned with the fixture's
	// intentionally refreshed record envelopes while retaining it as the
	// independent source for the verifier's membership checks.
	input.SelectedPlanEntry.Responsibilities.BindingContracts[0] = input.ContractRecord
	input.SelectedPlanEntry.Responsibilities.BindingArtifacts[0] = input.ArtifactRecord
	input.Binding.Contract = input.ContractRecord.Reference
	input.Binding.Artifact = input.ArtifactRecord.Reference
	input.Component.Bindings[0].Contract = input.Binding.Contract
	input.Component.Bindings[0].Artifact = input.Binding.Artifact
...
func clonePortableToolPlanEntryForVerificationTest(
	t *testing.T,
	entry providers.PortableToolPlanEntryV1,
) providers.PortableToolPlanEntryV1 {
	t.Helper()
	encoded, err := canonical.Marshal(entry)
	if err != nil {
		t.Fatal(err)
	}
	var clone providers.PortableToolPlanEntryV1
