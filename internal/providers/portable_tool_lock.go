...
const (
	...
	PortableToolReleaseManifestRecordSchemaV1 = "portable-tool-release-manifest-v1"
...
func validatePortableToolReleaseManifestLockV1(
	...
	if err := validatePortableToolReleaseManifestRecordV1(value, namespace, entry.Provenance.Revision); err != nil {
		return fmt.Errorf("release manifest: %w", err)
	}
	return nil
}

func validatePortableToolReleaseManifestRecordV1(value canonical.Object, releaseNamespace, revision string) error {
	expectedFields := map[string]struct{}{
		"schema": {}, "id": {}, "tool": {}, "version": {}, "aliases": {}, "revision": {},
		"contract": {}, "targets": {}, "artifact_sources": {}, "provenance": {}, "validation_profiles": {},
	}
	if len(value) != len(expectedFields) {
		return fmt.Errorf("record must contain exactly the canonical release-manifest fields")
	}
	for field := range value {
		if _, exists := expectedFields[field]; !exists {
			return fmt.Errorf("record contains unsupported field %q", field)
		}
	}
	if revision == "" || revision[0] == '0' {
		return fmt.Errorf("revision must be a canonical positive decimal string")
	}
	...
	aliases, err := portableToolObjectStringArrayV1(value, "aliases")
	if err != nil {
		return err
	}
	if err := validatePortableToolSortedUniqueStringsV1("aliases", aliases, true); err != nil {
		return err
	}
	for _, alias := range aliases {
		if !utf8.ValidString(alias) || validatePortableToken("release alias", alias) != nil || alias == portableToolObjectStringV1(value, "version") {
			return fmt.Errorf("release aliases must be canonical versions distinct from the exact version")
		}
	}
	contract, ok := portableToolExactObjectReferenceV1(value["contract"])
	if !ok || contract.ID != releaseNamespace+"/contract" {
		return fmt.Errorf("contract must be an exact reference to the selected release contract")
	}
	targets, err := portableToolObjectReferenceArrayV1(value, "targets", false)
	if err != nil {
		return err
	}
	for _, target := range targets {
		segments := strings.Split(target.ID, "/")
		prefix := strings.Split(releaseNamespace, "/")
		validPrefix := len(prefix) == 3 && len(segments) == 7 && strings.Join(segments[:3], "/") == releaseNamespace
		validArchitecture := len(segments) == 7 && (segments[6] == "amd64" || segments[6] == "arm64")
		if !validPrefix || segments[3] != "targets" || validatePortableRecordIdentifier("target OS release ID", segments[4]) != nil || !validPortableToolTargetVersionSegmentV1(segments[5]) || !validArchitecture {
			return fmt.Errorf("target %q must remain beneath the selected release target namespace", target.ID)
		}
	}
	profiles, err := portableToolObjectReferenceArrayV1(value, "validation_profiles", false)
	if err != nil {
		return err
	}
	for _, profile := range profiles {
		name, found := strings.CutPrefix(profile.ID, releaseNamespace+"/validation/profiles/")
		if !found || name == "" || strings.Contains(name, "/") || !validPortableRecordSegment(name) {
			return fmt.Errorf("validation profile %q must use the selected release profile namespace", profile.ID)
		}
	}
	provenance, err := portableToolObjectStringArrayV1(value, "provenance")
	if err != nil {
		return err
	}
	if err := validatePortableToolCanonicalSourceURLsV1("release provenance", provenance, true); err != nil {
		return err
	}
	mappings, err := portableToolObjectArrayV1(value, "artifact_sources")
	if err != nil || len(mappings) > portableToolLockMaxRecordReferencesV1 {
	...
	for index, raw := range mappings {
		...
		if !ok || len(mapping) != 3 {
			return fmt.Errorf("artifact source mapping %d must contain exactly artifact_sha256, artifact, and source", index)
		}
		...
		artifact, artifactOK := portableToolExactObjectReferenceV1(mapping["artifact"])
		source, sourceOK := portableToolExactObjectReferenceV1(mapping["source"])
		if !artifactOK || !sourceOK {
			return fmt.Errorf("artifact source mapping %d must use exact record references", index)
		}
		if !validPortableToolArtifactSourceTargetV1(artifact.ID, releaseNamespace) {
			return fmt.Errorf("artifact source mapping %d artifact must name a payload or binding artifact in the selected release", index)
		}
		wantSourcePrefix := releaseNamespace + "/revisions/" + revision + "/sources/"
		sourceName, found := strings.CutPrefix(source.ID, wantSourcePrefix)
		if !found || sourceName == "" || strings.Contains(sourceName, "/") || !validPortableRecordSegment(sourceName) {
			return fmt.Errorf("artifact source mapping %d source is outside the selected release revision", index)
		}
		if index > 0 && previousDigest >= digest {
...
func validPortableToolTargetVersionSegmentV1(value string) bool {
...
func validPortableToolArtifactSourceTargetV1(id, releaseNamespace string) bool {
	segments := strings.Split(id, "/")
	if len(segments) < 5 || strings.Join(segments[:3], "/") != releaseNamespace {
		return false
	}
	switch {
	case len(segments) == 5 && segments[3] == "payloads":
		return validPortableToolPayloadLeafV1(segments[4])
	case len(segments) == 6 && segments[3] == "payloads":
		return validatePortableRecordIdentifier("payload selector", segments[4]) == nil && validPortableToolPayloadLeafV1(segments[5])
	case len(segments) == 7 && segments[3] == "bindings" && segments[5] == "artifacts":
		return validatePortableRecordIdentifier("binding", segments[4]) == nil && validPortableToolPlatformLeafV1(segments[6])
	default:
		return false
	}
}

func validPortableToolPayloadLeafV1(value string) bool {
	...
	return validatePortableRecordIdentifier("payload", value[:name]) == nil && validPortableToolPlatformLeafV1(value[name+1:])
}

func validPortableToolPlatformLeafV1(value string) bool {
	return value == "linux-amd64" || value == "linux-arm64"
}

func portableToolObjectStringArrayV1(value canonical.Object, field string) ([]string, error) {
	raw, err := portableToolObjectArrayV1(value, field)
	if err != nil {
		return nil, err
	}
	if len(raw) > portableToolLockMaxRecordReferencesV1 {
		return nil, fmt.Errorf("%s must use at most %d entries", field, portableToolLockMaxRecordReferencesV1)
	}
	result := make([]string, len(raw))
	for index, item := range raw {
		text, ok := item.(string)
		if !ok || text == "" || strings.TrimSpace(text) != text {
			return nil, fmt.Errorf("%s must contain canonical nonempty strings", field)
		}
		result[index] = text
	}
	return result, nil
...
func portableToolObjectReferenceArrayV1(value canonical.Object, field string, allowEmpty bool) ([]PortableToolRecordReferenceV1, error) {
	raw, err := portableToolObjectArrayV1(value, field)
	if err != nil {
		return nil, err
	}
	if len(raw) > portableToolLockMaxRecordReferencesV1 || !allowEmpty && len(raw) == 0 {
		return nil, fmt.Errorf("%s must use a bounded nonempty array", field)
	}
	result := make([]PortableToolRecordReferenceV1, len(raw))
	for index, item := range raw {
		reference, ok := portableToolExactObjectReferenceV1(item)
		if !ok {
			return nil, fmt.Errorf("%s entry %d must be an exact record reference", field, index)
		}
		if index > 0 && result[index-1].ID >= reference.ID {
			return nil, fmt.Errorf("%s must be unique and sorted by ID", field)
		}
...
func portableToolExactObjectReferenceV1(value any) (PortableToolRecordReferenceV1, bool) {
	object, ok := portableToolCanonicalMapV1(value)
	if !ok || len(object) != 2 {
		return PortableToolRecordReferenceV1{}, false
	}
	return portableToolObjectReferenceV1(object)
}

func validatePortableToolSortedUniqueStringsV1(field string, values []string, allowEmpty bool) error {
	if len(values) > portableToolLockMaxRecordReferencesV1 || !allowEmpty && len(values) == 0 {
		return fmt.Errorf("%s must use a bounded nonempty array", field)
	}
...
func validatePortableToolArtifactAcquisitionLockV1(
	...
	if err := validatePortableToolCanonicalSourceURLsV1("source record provenance", historical, false); err != nil {
		return err
	}
	...
	if err != nil || redirects < 0 || strconv.Itoa(redirects) != acquisition.Outcome.RedirectHops {
		return fmt.Errorf("redirect hops must be a canonical nonnegative decimal string")
	}
...
func validatePortableToolCanonicalSourceURLsV1(field string, values []string, allowEmpty bool) error {
	if err := validatePortableToolSortedUniqueStringsV1(field, values, allowEmpty); err != nil {
		return err
	}
	for index, raw := range values {
		canonicalURL, err := portableToolCanonicalSourceURLV1(raw)
		if err != nil {
...
func portableToolCanonicalSourceURLV1(raw string) (string, error) {
	parsed, err := url.Parse(raw)
	if err != nil || parsed.Scheme != "https" || parsed.Opaque != "" || parsed.Host == "" || parsed.Hostname() == "" || parsed.User != nil || parsed.ForceQuery || parsed.RawQuery != "" || parsed.Fragment != "" || strings.Contains(raw, "#") || parsed.Host != strings.ToLower(parsed.Host) || strings.HasSuffix(parsed.Hostname(), ".") || parsed.Port() == "443" || !portableToolASCIIURLHostV1(parsed.Hostname()) || !portableToolCanonicalPercentEscapesV1(parsed.EscapedPath()) || portableToolHasURLDotSegmentV1(parsed.Path) {
		return "", fmt.Errorf("source URL must be a canonical credential-free HTTPS URL without query or fragment")
	}
	port := parsed.Port()
	if strings.HasSuffix(parsed.Host, ":") || port != "" && (!portableToolCanonicalPositivePortV1(port)) {
	...
	} else if strings.Contains(host, ":") || portableToolNumericURLHostV1(host) {
		return "", fmt.Errorf("source URL must use a canonical authority")
	}
	if port != "" {
		host += ":" + port
	...
	return "https://" + host + portableToolCanonicalSourcePathV1(escapedPath), nil
}

func portableToolCanonicalPositivePortV1(value string) bool {
...
func portableToolNumericURLHostV1(host string) bool {
	if host == "" {
		return false
	}
...
func portableToolCanonicalSourcePathV1(escapedPath string) string {
...
func portableToolASCIIURLHostV1(host string) bool {
...
func portableToolCanonicalPercentEscapesV1(value string) bool {
	for index := 0; index < len(value); index++ {
		if value[index] != '%' {
			continue
		}
		if index+2 >= len(value) || !portableToolUppercaseHexV1(value[index+1]) || !portableToolUppercaseHexV1(value[index+2]) {
			return false
		}
		index += 2
	}
...
func portableToolUppercaseHexV1(value byte) bool {
	return value >= '0' && value <= '9' || value >= 'A' && value <= 'F'
}

func portableToolHasURLDotSegmentV1(value string) bool {
