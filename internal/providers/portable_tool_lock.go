...
func ValidatePortableToolLockV1(lock PortableToolLockV1) error {
	...
	for index, acquisition := range lock.Acquisitions {
		...
		if err := validatePortableToolArtifactAcquisitionLockV1(acquisition, selected.entry, selected.artifact, release.Manifest); err != nil {
...
func validatePortableToolReleaseManifestLockV1(
	...
	if _, err := portableToolObjectArrayV1(value, "artifact_sources"); err != nil {
...
func validatePortableToolArtifactAcquisitionLockV1(
	...
	if !strings.HasPrefix(acquisition.Source.Reference.ID, wantPrefix) || len(acquisition.Source.Reference.ID) == len(wantPrefix) {
	...
	if err := validatePortableToolManifestSourceAuthorizationV1(manifest, acquisition); err != nil {
		return err
	}
...
func validatePortableToolManifestSourceAuthorizationV1(
	...
	for index, raw := range values {
		...
		if digest == acquisition.Descriptor.SHA256 && artifact == acquisition.Artifact && source == acquisition.Source.Reference {
			matches++
		}
	}
	if matches != 1 {
		return fmt.Errorf("release manifest must authorize the exact artifact and source record once")
	}
	return nil
}
