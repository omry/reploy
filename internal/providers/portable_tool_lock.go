...
func validatePortableToolArtifactAcquisitionLockV1(
	...
	if err := validatePortableToolArtifactSourceRecordV1(acquisition.Source.Record.Value); err != nil {
		return fmt.Errorf("source record: %w", err)
	}
...
func validatePortableToolArtifactSourceRecordV1(value canonical.Object) error {
	expectedFields := map[string]struct{}{
		"schema": {}, "id": {}, "sha256": {}, "mirrors": {}, "provenance": {}, "diagnostics": {},
	}
	if len(value) != len(expectedFields) {
		return fmt.Errorf("record must contain exactly the canonical artifact-source fields")
	}
	for field := range value {
		if _, exists := expectedFields[field]; !exists {
			return fmt.Errorf("record contains unsupported field %q", field)
	...
	digest := portableToolObjectDigestV1(value, "sha256")
	if err := digest.Validate(); err != nil {
		return fmt.Errorf("sha256: %w", err)
	}
	mirrors, err := portableToolObjectStringArrayV1(value, "mirrors")
	if err != nil {
		return err
	}
	if len(mirrors) == 0 || len(mirrors) > providerstore.CoreMaxArtifactMirrors {
		return fmt.Errorf("mirrors must contain between 1 and %d entries", providerstore.CoreMaxArtifactMirrors)
	}
	seenMirrors := make(map[string]struct{}, len(mirrors))
	for index, mirror := range mirrors {
		canonicalURL, err := providerstore.CanonicalSourceURLV1(mirror)
		if err != nil {
			return fmt.Errorf("mirrors %d: %w", index, err)
		}
		if mirror != canonicalURL {
			return fmt.Errorf("mirrors %d must use canonical spelling %q", index, canonicalURL)
		}
		if _, exists := seenMirrors[mirror]; exists {
			return fmt.Errorf("mirrors must be unique")
		}
	...
	if err := validatePortableToolCanonicalSourceURLsV1("source record provenance", provenance, false); err != nil {
		return err
	}
	diagnostics, err := portableToolObjectStringArrayV1(value, "diagnostics")
	if err != nil {
		return err
	}
	if err := validatePortableToolSortedUniqueStringsV1("source record diagnostics", diagnostics, true); err != nil {
		return err
	}
	return nil
}
...
func validatePortableToolCanonicalSourceURLsV1(field string, values []string, allowEmpty bool) error {
	...
	for index, raw := range values {
		canonicalURL, err := providerstore.CanonicalSourceURLV1(raw)
		if err != nil {
