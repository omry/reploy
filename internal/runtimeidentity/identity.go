...
// POSIX credential-changing calls reserve the all-ones uid_t/gid_t value as
// "leave unchanged". It must never become a planned runtime identity.
const credentialUnchangedSentinelV1 uint64 = math.MaxUint32
...
func ValidateIdentityV1(identity IdentityV1) error {
	...
	if !ok {
		return fmt.Errorf("runtime UID must use a canonical unsigned 32-bit decimal string other than math.MaxUint32 (%d)", credentialUnchangedSentinelV1)
	}
	primaryGID, ok := canonicalUnsignedV1(identity.GID)
	if !ok {
		return fmt.Errorf("runtime GID must use a canonical unsigned 32-bit decimal string other than math.MaxUint32 (%d)", credentialUnchangedSentinelV1)
	}
	...
	for index, value := range identity.SupplementaryGIDs {
		...
		if !ok {
			return fmt.Errorf("supplementary GID %q must use a canonical unsigned 32-bit decimal string other than math.MaxUint32 (%d)", value, credentialUnchangedSentinelV1)
		}
		if gid == primaryGID {
