...
func ValidateIdentityV1(identity IdentityV1) error {
	...
	if !ok {
		return fmt.Errorf("runtime UID must use a canonical unsigned 32-bit decimal string")
	}
	primaryGID, ok := canonicalUnsignedV1(identity.GID)
	if !ok {
		return fmt.Errorf("runtime GID must use a canonical unsigned 32-bit decimal string")
	}
	...
	for index, value := range identity.SupplementaryGIDs {
		...
		if !ok {
			return fmt.Errorf("supplementary GID %q must use a canonical unsigned 32-bit decimal string", value)
		}
		if gid == primaryGID {
