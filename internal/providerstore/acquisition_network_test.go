...
func TestResolvePublicAcquisitionAddressesRejectsMixedAnswersAndPreservesValidatedSet(t *testing.T) {
	...
	if _, err := resolvePublicAcquisitionAddresses(context.Background(), "mixed.example.test", publicAndPrivate); !errors.Is(err, errRejectedDestination) {
	...
	addresses, err := resolvePublicAcquisitionAddresses(context.Background(), "public.example.test", deterministic)
	if err != nil {
		t.Fatal(err)
	}
	want := []netip.Addr{netip.MustParseAddr("1.1.1.1"), netip.MustParseAddr("8.8.8.8")}
	if !slices.Equal(addresses, want) {
		t.Fatalf("validated addresses = %v, want %v", addresses, want)
	}
}

func TestAcquisitionDialFallsBackAcrossOneValidatedAddressSet(t *testing.T) {
	...
	network := &acquisitionNetwork{
		...
		dial: func(_ context.Context, _ string, address string) (net.Conn, error) {
			...
			if address == "93.184.216.34:443" {
				return nil, errors.New("controlled first-address failure")
			}
	...
	if resolutions.Load() != 1 || !slices.Equal(dialTargets, want) {
		t.Fatalf("resolution count/targets = %d/%v, want 1/%v", resolutions.Load(), dialTargets, want)
	}
}

func TestAcquisitionDialPreservesDeadlineForLaterValidatedAddresses(t *testing.T) {
	...
	request, err := http.NewRequestWithContext(parentContext, http.MethodGet, "https://download.example.test/artifact", nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := pinAcquisitionRequest(request, network); err != nil {
	...
	connection.Close()
	if firstDeadline.IsZero() || !firstDeadline.Before(parentDeadline) {
		t.Fatalf("first dial deadline = %v, want before parent deadline %v", firstDeadline, parentDeadline)
	}
	if secondDeadline != parentDeadline {
		t.Fatalf("last dial deadline = %v, want remaining parent deadline %v", secondDeadline, parentDeadline)
	}
}

func TestAcquisitionDialStopsBeforeDialingWithExpiredDeadline(t *testing.T) {
	parentContext, cancel := context.WithDeadline(context.Background(), time.Now().Add(-time.Second))
	defer cancel()
	...
	if _, err := acquisitionDialContext(request.Context(), "tcp", "download.example.test:443", network); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("expired attempt error = %v, want deadline exceeded", err)
	}
	if dials.Load() != 0 {
		t.Fatalf("expired attempt dial count = %d, want 0", dials.Load())
	}
}
...
func TestAcquireAttemptClosesPerAttemptTransportIdleConnections(t *testing.T) {
	content := []byte("artifact")
	descriptor := acquisitionTestDescriptor(content)
	transport := &acquisitionCloseTrackingRoundTripper{content: content}
	...
	_, err = store.AcquireArtifact(context.Background(), AcquisitionRequest{
		...
		Source: ArtifactSource{
			ID: "source", SHA256: descriptor.SHA256, Mirrors: []string{"https://mirror.example.test/artifact"},
		},
		client:  &http.Client{Transport: transport},
	...
	if !transport.closed.Load() {
		t.Fatal("per-attempt transport idle connections were not closed")
	}
}

type acquisitionCloseTrackingRoundTripper struct {
	closed  atomic.Bool
	content []byte
}

func (transport *acquisitionCloseTrackingRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	return acquisitionTestResponse(request, http.StatusOK, string(transport.content), ""), nil
}

func (transport *acquisitionCloseTrackingRoundTripper) CloseIdleConnections() {
