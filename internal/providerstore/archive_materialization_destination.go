...
func (destination *archiveMaterializationDestination) rollbackPublished(workspace *archiveMaterializationWorkspace, name string) error {
	...
	if closeErr != nil {
		rollbackErr = fmt.Errorf("close published archive after destination-root identity change: %w", closeErr)
	}
	if removeErr != nil {
		rollbackErr = errors.Join(rollbackErr, fmt.Errorf("remove published archive after destination-root identity change: %w", removeErr))
	}
	return rollbackErr
}
