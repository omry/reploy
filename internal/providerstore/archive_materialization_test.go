...
func TestMaterializeArchiveRejectsOversizedZipCentralDirectoryMetadata(t *testing.T) {
	tests := []struct {
		name    string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			store, destination := newArchiveTestStore(t)
			request := newArchiveMaterializationTestRequest(t, store, destination, test.archive(t))
			request.Format = ArchiveFormatZip
			_, err := store.MaterializeArchive(context.Background(), request)
			if err == nil || !strings.Contains(err.Error(), "central directory") {
				t.Fatalf("oversized ZIP metadata error = %v", err)
			}
			assertArchiveDestinationAbsent(t, destination, "install")
			assertArchiveTempClean(t, destination)
		})
...
func TestMaterializeArchiveRejectsTarMetadataByteBudget(t *testing.T) {
	store, destination := newArchiveTestStore(t)
	request := newArchiveMaterializationTestRequest(t, store, destination, makeTarGzWithGNUlongNameChain(t, 2))
	_, err := store.MaterializeArchive(context.Background(), request)
	if err == nil || !strings.Contains(err.Error(), "decompressed byte budget") {
		t.Fatalf("oversized tar metadata error = %v", err)
	}
	assertArchiveDestinationAbsent(t, destination, "install")
	assertArchiveTempClean(t, destination)
}
...
func TestMaterializeArchiveRejectsPortableCaseFoldedDestinationAliases(t *testing.T) {
	...
	zipCases := []struct {
		...
		{name: "unicode-normalization-implicit-parent", entries: []zipTestEntry{
			{name: "café/a", content: "x"},
			{name: "café/b", content: "y"},
		}, expected: "2"},
...
func TestMaterializeArchiveRejectsTarUnicodeNormalizationAlias(t *testing.T) {
	store, destination := newArchiveTestStore(t)
	request := newArchiveMaterializationTestRequest(t, store, destination, makeTarGz(t, []archiveTestEntry{
	...
	_, err := store.MaterializeArchive(context.Background(), request)
	if err == nil || !strings.Contains(err.Error(), "case-insensitive") {
		t.Fatalf("case-folded tar Unicode destination error = %v", err)
	}
	assertArchiveDestinationAbsent(t, destination, "install")
	assertArchiveTempClean(t, destination)
}
