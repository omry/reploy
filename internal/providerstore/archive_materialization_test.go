...
func TestMaterializeArchiveAcceptsMaximumLengthInstallDirectory(t *testing.T) {
	store, destination := newArchiveTestStore(t)
	installDirectory := strings.Repeat("a", CoreMaxArchiveComponentBytes)
	descriptor := publishArchiveTestBytes(t, store, makeTarGz(t, []archiveTestEntry{
		{header: tar.Header{Name: "payload", Typeflag: tar.TypeReg}, content: "x"},
	}))
	result, err := store.MaterializeArchive(context.Background(), ArchiveMaterializationRequest{
		Artifact: descriptor, Format: ArchiveFormatTarGz, DestinationRoot: destination,
		InstallDirectory: installDirectory, ArchiveRoot: ".", ExpectedEntryCount: "1", ExpectedUnpackedSize: "1",
	})
	if err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(destination, installDirectory, "payload")
	content, err := os.ReadFile(path)
	if err != nil || string(content) != "x" {
		t.Fatalf("materialized content = %q, err = %v", content, err)
	}
	if result.FinalPath != filepath.Join(destination, installDirectory) {
		t.Fatalf("result final path = %q, want %q", result.FinalPath, filepath.Join(destination, installDirectory))
	}
}
...
func assertArchiveTempClean(t *testing.T, root string) {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(root, archiveMaterializationWorkspacePattern))
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) != 0 {
