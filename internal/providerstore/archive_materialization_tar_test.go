...
func TestArchiveMaterializationTarMaterializesContainedRegularSymbolicLink(t *testing.T) {
	...
	if got := info.Mode().Perm(); got != 0o555 {
		t.Fatalf("declared materialized symbolic-link mode = %o, want 555", got)
	}
...
func TestArchiveMaterializationTarSymbolicLinkDoesNotInheritExecutableMode(t *testing.T) {
	...
	targetInfo, err := os.Lstat(filepath.Join(materializer.stage, "jdk", "bin", "java"))
	if err != nil {
		t.Fatal(err)
	}
	if got := targetInfo.Mode().Perm(); got != 0o555 {
		t.Fatalf("declared target mode = %o, want 555", got)
	}
	aliasInfo, err := os.Lstat(filepath.Join(materializer.stage, "jdk", "bin", "java-alias"))
	if err != nil {
		t.Fatal(err)
	}
	if got := aliasInfo.Mode().Perm(); got != 0o444 {
		t.Fatalf("undeclared alias mode = %o, want 444", got)
	}
}
