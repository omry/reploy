...
func preflightZipCentralDirectory(ctx context.Context, file *os.File, size int64) error {
	...
	// archive/zip also treats the 16-bit 0xffff value as the ZIP64
	// directory-size sentinel, so the preflight must follow that same path.
	if declaredRecords == 0xffff || directorySize == 0xffff || directorySize == 0xffffffff || directoryOffset == 0xffffffff {
...
func zipDirectoryHeaderAt(file *os.File, size int64, offset int64) (bool, error) {
	...
	metadataSize := int64(binary.LittleEndian.Uint16(header[28:30])) +
		int64(binary.LittleEndian.Uint16(header[30:32])) +
		int64(binary.LittleEndian.Uint16(header[32:34]))
	return metadataSize <= size-offset-int64(directoryHeaderLen), nil
}
