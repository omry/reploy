...
func TestArchiveMaterializationPublishErrorRollsBackPublishedDirectory(t *testing.T) {
	...
	if matches, err := filepath.Glob(filepath.Join(destinationPath, archiveMaterializationRollbackPattern)); err != nil {
		t.Fatal(err)
	} else if len(matches) != 0 {
		t.Fatalf("rollback quarantine remains after valid rollback: %v", matches)
	}
...
func TestArchiveMaterializationRollbackPreservesReplacementAfterPrevalidatedName(t *testing.T) {
	destinationPath := t.TempDir()
	destination, err := openArchiveMaterializationDestination(destinationPath)
	if err != nil {
		t.Fatal(err)
	...
	if err := destination.requirePublishedIdentity(workspace, "install"); err != nil {
		t.Fatal(err)
	}
	if err := destination.root.Rename("install", "moved-install"); err != nil {
		t.Fatal(err)
	...
	if err := destination.rollbackPublished(workspace, "install"); err == nil {
		t.Fatal("prevalidated replacement unexpectedly rolled back successfully")
	}
	if _, err := destination.root.Lstat("install/replacement"); err != nil {
		t.Fatalf("replacement destination was removed: %v", err)
	}
...
func TestArchiveMaterializationQuarantinePreservesReplacementAfterPrevalidatedName(t *testing.T) {
	destinationPath := t.TempDir()
	destination, err := openArchiveMaterializationDestination(destinationPath)
	if err != nil {
		t.Fatal(err)
	...
	quarantine, published, err := destination.quarantinePublished("install")
	if err != nil || !published {
		t.Fatalf("quarantine replacement = %q, published = %v, err = %v", quarantine, published, err)
	}
	if err := destination.requirePublishedIdentity(workspace, quarantine); err == nil {
		t.Fatal("replacement in quarantine unexpectedly matched published workspace")
	}
	if _, err := destination.root.Lstat(filepath.Join(quarantine, "replacement")); err != nil {
		t.Fatalf("quarantined replacement was removed: %v", err)
	}
	if _, err := destination.root.Lstat("moved-install/marker"); err != nil {
		t.Fatalf("published directory was removed: %v", err)
	}
	if err := closeArchiveMaterializationWorkspaceRoot(workspace); err != nil {
		t.Fatal(err)
	}
}

func TestArchiveMaterializationRollbackUnavailableWorkspaceIsSafe(t *testing.T) {
	destinationPath := t.TempDir()
	destination, err := openArchiveMaterializationDestination(destinationPath)
	if err != nil {
		t.Fatal(err)
	...
	if err := destination.rollbackPublished(nil, "install"); err == nil {
		t.Fatal("nil workspace rollback unexpectedly succeeded")
	}
	if err := destination.rollbackPublished(&archiveMaterializationWorkspace{}, "install"); err == nil {
		t.Fatal("unavailable workspace rollback unexpectedly succeeded")
	}
	publishErr := errors.New("publication failed")
	if err := destination.handlePublicationResult(nil, "install", true, publishErr); !errors.Is(err, publishErr) {
		t.Fatalf("nil workspace publication error = %v, want %v", err, publishErr)
	}
}
