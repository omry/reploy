...
func (materializer *archiveMaterializer) acceptRegular(destinationPath string, size int64, content io.Reader, executable bool) error {
	...
	if copyErr == nil {
		if err := file.Chmod(mode); err != nil {
			chmodErr = fmt.Errorf("normalize archive member %q mode: %w", destinationPath, err)
		} else if err := file.Sync(); err != nil {
			syncErr = fmt.Errorf("sync archive member %q: %w", destinationPath, err)
		}
	}
	closeErr := file.Close()
...
func normalizeMaterializedEntry(root *os.Root, path string) (bool, error) {
	...
	if info.IsDir() {
		if err := chmodArchiveMaterializationDirectory(root, filepath.FromSlash(path), file, 0o555); err != nil {
