...
func (reader *archiveTarEndMarkerReader) hasCompleteEndMarker() bool {
	if reader.readBytes < uint64(len(reader.lastBytes)) || reader.lastCount < len(reader.lastBytes) {
		return false
	}
...
func (materializer *archiveMaterializer) extractTar(reader io.Reader) error {
	...
	for {
		...
		if errors.Is(err, io.EOF) {
			if !endMarkerReader.hasCompleteEndMarker() {
