...
const (
	...
	CoreMaxArchiveComponentBytes = 255
	...
	archiveMaterializationMaxComponentBytes uint64 = CoreMaxArchiveComponentBytes
)
...
func validateArchiveMaterializationRequest(request ArchiveMaterializationRequest) (validatedArchiveMaterializationRequest, error) {
	...
	if uint64(len(request.ExecutablePaths)) > entryLimit {
		return validatedArchiveMaterializationRequest{}, fmt.Errorf("archive executable path count %d exceeds expected archive entry count %d", len(request.ExecutablePaths), entryLimit)
	}
	portableExecutableKeys := make(map[string]string, len(request.ExecutablePaths))
	for index, executable := range request.ExecutablePaths {
		...
		if !archivePathStrictlyWithin(archiveRoot, normalized) {
		...
		portableKey := portableArchiveDestinationKey(normalized)
		if previous, exists := portableExecutableKeys[portableKey]; exists {
			return validatedArchiveMaterializationRequest{}, fmt.Errorf("archive executable paths %q and %q alias on portable filesystems", previous, executable)
		}
		portableExecutableKeys[portableKey] = executable
	}
	portableExecutableKeyOrder := make([]string, 0, len(portableExecutableKeys))
	for portableKey := range portableExecutableKeys {
		portableExecutableKeyOrder = append(portableExecutableKeyOrder, portableKey)
	}
	sort.Strings(portableExecutableKeyOrder)
	for _, portableKey := range portableExecutableKeyOrder {
		ancestor := portableKey
		for {
			...
			if executable, exists := portableExecutableKeys[ancestor]; exists {
				return validatedArchiveMaterializationRequest{}, fmt.Errorf("archive executable path %q is below declared ancestor %q", portableExecutableKeys[portableKey], executable)
			}
		}
	}
	request.ArchiveRoot = archiveRoot
	request.ExecutablePaths = append([]string(nil), request.ExecutablePaths...)
...
func validateInstallDirectory(installDirectory string) error {
	...
	if err := validateArchivePathComponentBytes(installDirectory); err != nil {
...
func archivePathStrictlyWithin(root string, member string) bool {
	return root == "." || strings.HasPrefix(member, root+"/")
}
...
func normalizeArchivePath(value string, directory bool) (string, error) {
	...
	for _, part := range parts {
		...
		if err := validateArchivePathComponentBytes(part); err != nil {
			return "", err
		}
	}
