...
func resolvePublicAcquisitionAddress(ctx context.Context, host string, network *acquisitionNetwork) (netip.Addr, error) {
	if host == "" {
		return netip.Addr{}, errRejectedDestination
	}
	...
	if err != nil {
		return netip.Addr{}, fmt.Errorf("resolve acquisition destination: %w", err)
	}
	if len(addresses) == 0 {
		return netip.Addr{}, errRejectedDestination
	}
...
func pinAcquisitionRequest(request *http.Request, network *acquisitionNetwork) error {
	...
	address, err := resolvePublicAcquisitionAddress(request.Context(), request.URL.Hostname(), network)
	if err != nil {
		return err
	}
	pin := &acquisitionPinnedAddress{host: strings.ToLower(request.URL.Hostname()), address: address}
...
func acquisitionDialContext(ctx context.Context, networkName, address string, network *acquisitionNetwork) (net.Conn, error) {
	...
	if pinned, found = takeAcquisitionPin(ctx, host); !found {
		pinned, err = resolvePublicAcquisitionAddress(ctx, host, network)
		if err != nil {
			return nil, err
		}
	}
	if networkName == "tcp4" && !pinned.Is4() || networkName == "tcp6" && !pinned.Is6() {
		return nil, errRejectedDestination
	}
	...
	return dialer(ctx, networkName, net.JoinHostPort(pinned.String(), port))
}
