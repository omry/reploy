...
func acquisitionDialContext(ctx context.Context, networkName, address string, network *acquisitionNetwork) (net.Conn, error) {
	...
	dialAddresses := make([]netip.Addr, 0, len(pinned))
	...
	for index, address := range dialAddresses {
		dialContext := ctx
		cancel := func() {}
		if deadline, bounded := ctx.Deadline(); bounded && index+1 < len(dialAddresses) {
			remaining := time.Until(deadline)
			if remaining <= 0 {
				dialErrors = append(dialErrors, context.DeadlineExceeded)
				break
			}
			dialContext, cancel = context.WithTimeout(ctx, remaining/time.Duration(len(dialAddresses)-index))
		}
		connection, dialErr := dialer(dialContext, networkName, net.JoinHostPort(address.String(), port))
		cancel()
		if dialErr == nil {
