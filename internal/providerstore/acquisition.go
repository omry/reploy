...
const (
	...
	AcquisitionOutcomeByteLimit      = "byte-limit"
)
...
func (store Store) AcquireArtifact(ctx context.Context, request AcquisitionRequest) (AcquisitionResult, error) {
	...
	if operationID == "" {
		operationID = fmt.Sprintf("acquisition-%d", time.Now().UTC().UnixNano())
	}
	...
	for _, mirror := range request.Source.Mirrors {
		for attempt := 1; attempt <= policy.AttemptsPerMirror && totalAttempts < policy.MaxAttempts; attempt++ {
			...
			if err != nil {
				...
				if errors.Is(err, errAcquisitionStorage) {
					return AcquisitionResult{}, err
				}
				continue
			}
...
func cloneAcquisitionClient(base *http.Client, maxRedirects int) (*http.Client, *acquisitionRedirectCounter) {
	...
	client.CheckRedirect = func(request *http.Request, _ []*http.Request) error {
		redirects.count++
		if redirects.count > maxRedirects || !credentialFreeAcquisitionURL(request.URL) {
...
func credentialFreeAcquisitionURL(parsed *url.URL) bool {
	return parsed != nil && parsed.Scheme == "https" && parsed.Opaque == "" && parsed.Host != "" && parsed.Hostname() != "" && parsed.User == nil && !parsed.ForceQuery && parsed.RawQuery == "" && parsed.Fragment == "" && !strings.ContainsAny(parsed.String(), "\r\n")
}
