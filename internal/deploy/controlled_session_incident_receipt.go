...
// watchdog write target while the deployment operation lock is held. Existing
// completed receipts are never overwritten or silently evicted.
...
func WriteControlledSessionIncidentReceiptV1(file *os.File, receipt ControlledSessionIncidentReceiptV1) error {
	...
	if count, err := file.Write(content); err != nil || count != len(content) {
		...
		return fmt.Errorf("write controlled-session incident receipt: %w", err)
	}
	if err := file.Sync(); err != nil {
		return fmt.Errorf("sync controlled-session incident receipt: %w", err)
	}
	return nil
}
