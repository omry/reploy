...
func TestApplicationRuntimeLayerDoesNotFollowInheritedVerifierSymlink(t *testing.T) {
	if os.Getenv("REPLOY_DOCKER_INTEGRATION") != "1" {
		t.Skip("set REPLOY_DOCKER_INTEGRATION=1 to run Docker integration evidence")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 4*time.Minute)
...
RUN mkdir -p /mnt/attacker && ln -s /mnt/attacker/reploy-probe /reploy-probe
USER 12345:23456
`
	if err := os.WriteFile(filepath.Join(baseContext, "Dockerfile"), []byte(baseDockerfile), 0o600); err != nil {
		t.Fatal(err)
	}
	base := uniqueDockerIntegrationName("reploy-verifier-symlink-base")
	runDockerIntegration(t, ctx, "build", "--tag", base, baseContext)
	t.Cleanup(func() {
		_ = exec.CommandContext(context.Background(), "docker", "image", "rm", base).Run()
	})

	image, _ := buildApplicationStartupVerifierIntegrationImageFromBase(t, ctx, base)
	attackerDirectory := t.TempDir()
	if err := os.Chmod(attackerDirectory, 0o755); err != nil {
		t.Fatal(err)
	}
	attackerVerifier := []byte("#!/bin/sh\nprintf 'attacker-verifier-ran\\n'\n")
	if err := os.WriteFile(filepath.Join(attackerDirectory, "reploy-probe"), attackerVerifier, 0o755); err != nil {
		t.Fatal(err)
	}
	mount, err := dockerMountArgument(
		"type=bind", "source="+attackerDirectory, "target=/mnt/attacker", "readonly",
	)
	if err != nil {
		t.Fatal(err)
	}
	output := runDockerIntegration(
		t, ctx,
		"run", "--rm", "--pull", "never", "--user", "12345:23456", "--read-only",
		"--cap-drop", "ALL", "--security-opt", "no-new-privileges=true",
		"--mount", mount,
		"--entrypoint", deploy.ApplicationStartupVerifierPathV1, image,
		"verify-exec", "--", "/bin/sh", "-c", "printf 'trusted-verifier-pass\\n'",
	)
	if strings.TrimSpace(output) != "trusted-verifier-pass" {
		t.Fatalf("verifier output = %q, want the trusted verifier to execute the workload", output)
	}
}

func buildApplicationStartupVerifierIntegrationImage(t *testing.T, ctx context.Context) (string, blueprint.Platform) {
	t.Helper()
	return buildApplicationStartupVerifierIntegrationImageFromBase(t, ctx, "debian:bookworm-slim")
}

func buildApplicationStartupVerifierIntegrationImageFromBase(
	t *testing.T,
	ctx context.Context,
	base string,
) (string, blueprint.Platform) {
	t.Helper()
