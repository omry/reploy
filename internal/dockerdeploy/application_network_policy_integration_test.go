...
func TestApplicationNetworkPolicyDockerIntegration(t *testing.T) {
	...
	localNetwork, publicNetwork, localAddresses, publicAddresses, dnsName := createNetworkPolicyIntegrationPeers(t, ctx, image, helper)
	...
	for _, test := range []struct {
		...
		t.Run("transient "+test.name, func(t *testing.T) {
			plan := networkPolicyIntegrationPlan(t, image, helper, test.public, test.local)
			command := ResolvedEnvironmentCommand{Argv: networkPolicyIntegrationWorkloadArgv(
				plan.Sandbox.RuntimeUser.UID, localAddresses, publicAddresses, test.wantLocal, test.wantPublic, false, dnsName,
			)}
	...
	t.Run("transient root deny-deny", func(t *testing.T) {
		plan := networkPolicyIntegrationPlan(t, image, helper, blueprint.NetworkAccessDeny, blueprint.NetworkAccessDeny)
		plan.Sandbox = newApplicationSandboxPlanWithNetworkV1(
			RuntimeUserPlan{UID: 0, GID: 0, DockerUser: "0:0"},
			blueprint.RuntimeNetwork{Public: blueprint.NetworkAccessDeny, Local: blueprint.NetworkAccessDeny},
		)
		command := ResolvedEnvironmentCommand{Argv: networkPolicyIntegrationWorkloadArgv(
			0, localAddresses, publicAddresses, false, false, false, dnsName,
		)}
		execution, err := PlanTransientContainerExecutionV1(plan, command, nil, "run-0000000000000001", false, false)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("persistent endpoint survives default denial", func(t *testing.T) {
		...
		plan.Workload = &WorkloadExecutionPlan{
			Argv: networkPolicyIntegrationWorkloadArgv(plan.Sandbox.RuntimeUser.UID, localAddresses, publicAddresses, false, false, true, dnsName),
...
func networkPolicyIntegrationPlan(t *testing.T, image string, helper string, public blueprint.NetworkAccess, local blueprint.NetworkAccess) DockerExecutionPlan {
	t.Helper()
	return DockerExecutionPlan{
		...
		Sandbox: newApplicationSandboxPlanWithNetworkV1(
			...
			blueprint.RuntimeNetwork{Public: public, Local: local},
		),
...
func networkPolicyIntegrationWorkloadArgv(uid int, local []string, public []string, wantLocal bool, wantPublic bool, serve bool, dnsName string) []string {
	mode := "exit"
	if serve {
		mode = "serve"
	}
	return []string{
		...
		local[0], local[1], public[0], public[1],
		strconv.FormatBool(wantLocal), strconv.FormatBool(wantPublic), mode,
		dnsName, strconv.FormatBool(wantPublic),
	}
}
...
func createNetworkPolicyIntegrationPeers(t *testing.T, ctx context.Context, image string, helper string) (string, string, []string, []string, string) {
	t.Helper()
	...
	return localNetwork, publicNetwork,
		[]string{localV4 + ":9090", "[" + localV6 + "]:9090"},
		[]string{publicV4 + ":9090", "[" + publicV6 + "]:9090"}, publicPeerName
}
