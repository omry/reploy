...
type LockedProviderBuildExecutionInputV1 struct {
	...
	// DesiredPortableToolPlan is the selected application-scoped portable
	// plan for this graph. A nil value means that no runtime portable tools are
	// selected and prevents an older build lock from supplying bindings.
...
func portableToolLockForCompletedGraphV1(
	...
	if reusable != nil && reusable.PortableTools != nil {
		matches, err := portableToolPythonSelectionsMatchCurrentBuildV1(reusable.PortableTools, desired)
		if err != nil {
