...
type cachedDockerEndpointCheckV1 struct {
	mu      sync.Mutex
	checked bool
}

var defaultDockerEndpointCheckV1 cachedDockerEndpointCheckV1

func requireDefaultLocalDockerEndpointV1(ctx context.Context) error {
	defaultDockerEndpointCheckV1.mu.Lock()
	defer defaultDockerEndpointCheckV1.mu.Unlock()
	if defaultDockerEndpointCheckV1.checked {
		return nil
	}
	if err := requireLocalDockerEndpointV1(ctx, CommandSpec{Name: "docker"}, defaultDockerPreflightTimeout); err != nil {
		return err
	}
	defaultDockerEndpointCheckV1.checked = true
	return nil
}
...
func inspectDockerContextEndpointV1(ctx context.Context, spec CommandSpec, timeout time.Duration, contextName string) (string, error) {
	...
	endpoint := strings.TrimSpace(output.String())
	if endpoint == "" {
