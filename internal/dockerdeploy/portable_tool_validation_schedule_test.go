...
func TestValidateProviderBuildCompletionInputAcceptsTheFinalOnlyPortableToolSchedule(t *testing.T) {
	...
	schedule, err := PortableToolFinalImageScheduleFromBuildLockV1(&lock)
	if err != nil {
		t.Fatal(err)
	}
	...
	// prepareProviderGraphValidation produces it, and the lock it was derived
...
func TestPortableToolFinalImageScheduleFromBuildLockV1ProjectsLockedProfiles(t *testing.T) {
	fixture := newPreparedPythonGraphReuseFixture(t)
	lock := portableToolLockForValidationScheduleV1(t, fixture.request.Plan, fixture.request.NodeID)
	schedule, err := PortableToolFinalImageScheduleFromBuildLockV1(&lock)
	if err != nil {
	...
	absent, err := PortableToolFinalImageScheduleFromBuildLockV1(nil)
	if err != nil {
		t.Fatal(err)
	}
...
// Codex review-comment (P2) — the final schedule is exempt from the graph-layer
// comparison, so completion must bind it to the portable-tool lock. Otherwise a
// build can validate one schedule and publish a lock claiming other profiles.
func TestValidateProviderBuildCompletionInputBindsTheFinalScheduleToTheLock(t *testing.T) {
	input, operation, _ := providerBuildCompletionFixture(t)
	defer operation.Unlock()
	policy, err := providerBuildRuntimePolicyV1(input)
	if err != nil {
	...
	// A lock with portable tools but no final schedule records no portable
	// evidence while the assembled lock claims those profiles.
	input.PortableTools = &lock
	err = validateProviderBuildCompletionInput(input, policy)
	if err == nil || !strings.Contains(err.Error(), "does not match the portable-tool lock") {
		t.Fatalf("absent final schedule with a portable-tool lock: error = %v", err)
	}

	// The matching schedule is accepted.
	input.Validation.Final.PortableTools = schedule
	if err := validateProviderBuildCompletionInput(input, policy); err != nil {
		t.Fatalf("matching final schedule rejected: %v", err)
	}

	// A final schedule with no corresponding lock is equally rejected.
	input.PortableTools = nil
	err = validateProviderBuildCompletionInput(input, policy)
	if err == nil || !strings.Contains(err.Error(), "does not match the portable-tool lock") {
		t.Fatalf("final schedule without a portable-tool lock: error = %v", err)
	}
}

// portableToolBuildContextLockV1 locks a tool whose contract declares no
// runtime projection, as the bundled Java build tool does.
func portableToolBuildContextLockV1(
	t *testing.T,
	providerPlan providers.ProviderPlanV1,
	owner providers.NodeID,
) providers.PortableToolLockV1 {
	t.Helper()
	lock := portableToolLockForValidationScheduleV1(t, providerPlan, owner)
	plan := lock.Plan.PortableToolPlan
	plan.Tools[0].Runtime = nil
	dag, err := providers.BuildPortableToolProviderDAGV1(
		lock.Plan.ProviderPlan, plan, lock.Plan.Domains,
	)
	if err != nil {
		t.Fatal(err)
	}
	rebuilt, err := providers.BuildPortableToolLockV1(
		dag,
		[]providers.PortableToolReleaseManifestInputV1{{
			Scope: "application", Tool: "demo", Manifest: lock.Releases[0].Manifest,
		}},
		[]providers.PortableToolArtifactAcquisitionInputV1{},
	)
	if err != nil {
...
// Codex review-comment (P1) — a build-context tool is materialized into the
// source-builder image, so probing its profile against the shipped runtime
// image would execute a path that image does not contain.
func TestPortableToolFinalImageScheduleExcludesBuildContextProfiles(t *testing.T) {
	fixture := newPreparedPythonGraphReuseFixture(t)
	runtimeLock := portableToolLockForValidationScheduleV1(t, fixture.request.Plan, fixture.request.NodeID)
	buildLock := portableToolBuildContextLockV1(t, fixture.request.Plan, fixture.request.NodeID)

	// A runtime-context tool is scheduled against the final image.
	runtimeSchedule, err := PortableToolFinalImageScheduleFromBuildLockV1(&runtimeLock)
	if err != nil {
		t.Fatal(err)
	}
	if len(runtimeSchedule.Entries) == 0 {
		t.Fatal("runtime-context profile was not scheduled for the final image")
	}

	// The same profile under a build-context closure is not.
	complete, err := providers.PortableToolValidationScheduleFromLockV1(buildLock)
	if err != nil {
		t.Fatal(err)
	}
	if len(complete.Entries) == 0 {
		t.Fatal("build-context fixture locks no validation profile")
	}
	buildSchedule, err := PortableToolFinalImageScheduleFromBuildLockV1(&buildLock)
	if err != nil {
		t.Fatal(err)
	}
	if !portableToolScheduleAbsentV1(buildSchedule) {
		t.Fatalf("build-context profile scheduled against the final image: %#v", buildSchedule)
	}
}
