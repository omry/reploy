...
// source-builder image. Probing it against the shipped image would fail on a
// missing path, and silently dropping it would let a build succeed having run
// none of its locked probes, so it is refused until a builder path exists.
func TestPortableToolFinalImageScheduleRefusesBuildContextProfiles(t *testing.T) {
	fixture := newPreparedPythonGraphReuseFixture(t)
	...
	_, err = PortableToolFinalImageScheduleFromBuildLockV1(&buildLock)
	if err == nil || !strings.Contains(err.Error(), "requires source-builder validation") {
		t.Fatalf("build-context profile was not refused: %v", err)
	}
}
