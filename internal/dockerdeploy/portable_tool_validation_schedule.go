...
// PortableToolValidationScheduleFromBuildLockV1 projects the locked schedule
// for a build lock's optional portable-tool section. A build that locked no
// portable tools yields the zero value, which schedules nothing.
//
// Every final-image validation entry point uses this so a locked build is
// revalidated against the exact profiles it locked, rather than silently
// dropping portable evidence.
func PortableToolValidationScheduleFromBuildLockV1(
