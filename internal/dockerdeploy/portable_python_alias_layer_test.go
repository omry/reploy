...
func TestPortablePythonAliasArchiveUsesPrimitiveStagingPayload(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(stagingRoot, "usr", "local", "bin"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("/opt/reploy/providers/python/app/bin/tool", filepath.Join(stagingRoot, "usr", "local", "bin", "tool")); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Remove(filepath.Join(stagingRoot, "usr", "local", "bin", "tool")); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("/wrong", filepath.Join(stagingRoot, "usr", "local", "bin", "tool")); err != nil {
		t.Fatal(err)
	}
...
func TestPortablePythonAliasArchiveSupportsLongRuntimeAndDestinationPaths(t *testing.T) {
	...
	linkPath := filepath.Join(stagingRoot, filepath.FromSlash(strings.TrimPrefix(destination, "/")))
