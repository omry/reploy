...
func TestPortablePythonAliasArchiveUsesPrimitiveStagingPayload(t *testing.T) {
	...
	if err := os.Symlink("/opt/reploy/providers/python/app/bin/tool", linkPath); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Symlink("/wrong", linkPath); err != nil {
		t.Fatal(err)
	}
...
func TestPortablePythonAliasArchiveRejectsUnexpectedStagedEntry(t *testing.T) {
	...
	if err := os.Symlink(alias.Target, linkPath); err != nil {
		t.Fatal(err)
	}
	extra := filepath.Join(filepath.Dir(linkPath), "unowned-entry")
	if err := os.Symlink(alias.Target, extra); err != nil {
		t.Fatal(err)
	}
