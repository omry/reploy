...
func TestStageDesiredStateV1ProbesNativeOnlyForAmbiguousDefault(t *testing.T) {
	...
	result, err := stageDesiredStateV1(t.Context(), DesiredStateStageInputV1{
		...
		setState: func(_ context.Context, _ string, _ blueprint.Document, platform blueprint.Platform, _, _ string, create bool) (deploy.DesiredStateUpdateResult, error) {
			if create {
