...
func ApplicationRuntimeLayerDockerfile(request ApplicationRuntimeLayerBuildRequest) ([]byte, error) {
	...
	destination, err := quoteDockerfileWord(request.Verifier.Path)
	if err != nil {
		return nil, fmt.Errorf("render application startup verifier path: %w", err)
	}
	...
	fmt.Fprintf(&output, "COPY --chown=0:0 --chmod=0555 %s %s\n", probearchive.ExtractedFileName, destination)
