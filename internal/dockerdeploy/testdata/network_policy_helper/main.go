...
func workload(args []string) {
	if len(args) != 10 {
		fail("workload requires UID, four addresses, two expectations, serve mode, DNS name, and DNS expectation")
	}
	checkStatus(args[0])
	wantLocal, err := strconv.ParseBool(args[5])
	if err != nil {
		fail("parse local expectation: %v", err)
	}
	wantPublic, err := strconv.ParseBool(args[6])
	if err != nil {
	...
	for _, item := range []struct {
		address string
		want    bool
	}{{args[1], wantLocal}, {args[2], wantLocal}, {args[3], wantPublic}, {args[4], wantPublic}} {
	...
	wantDNS, err := strconv.ParseBool(args[9])
	if err != nil {
	...
	_, lookupErr := resolver.LookupHost(lookupContext, args[8])
	cancel()
	if got := lookupErr == nil; got != wantDNS {
		fail("DNS lookup %s succeeded=%t want=%t err=%v", args[8], got, wantDNS, lookupErr)
	}
