...
func TestPreparedPythonNodeOperationsReusesWheelThroughRetainedSdist(t *testing.T) {
	store, err := providerstore.NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	sourceArtifact, err := store.Publish(
		context.Background(), "sdists/demo-pkg-1.0.tar.gz", "sdist", strings.NewReader("same sdist"),
	)
	if err != nil {
		t.Fatal(err)
	}
	wheel, err := store.Publish(
		context.Background(), "wheels/demo_pkg-1.0-py3-none-any.whl", "wheel", strings.NewReader("same wheel"),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer cleanup()
	distributions := stageSourceWheelTestDistribution(t, prepared, "demo-pkg", "1.0")
	distributions[0].SourceInputDigest = canonical.Digest("sha256:" + strings.Repeat("b", 64))
	distributions[0].Artifact = sourceArtifact
	prior := testPythonResolvedSourceWithSourceArtifact(
		"application", "demo-pkg", "1.0",
		canonical.Digest("sha256:"+strings.Repeat("a", 64)), sourceArtifact, wheel.SHA256,
	)
	prior.BuildEnvironmentDigest = distributions[0].BuildEnvironmentDigest
	operations := PreparedPythonNodeOperations{
		Store: store, Artifacts: prepared,
		PriorSources:      []providers.ResolvedSourceInput{prior},
		PriorSourceWheels: []providerstore.ArtifactDescriptor{wheel},
	}
	reused, pending, effective, err := operations.reusePythonSourceWheels(
		prepared, "application", distributions, []providerstore.ArtifactDescriptor{},
	)
	if err != nil {
		t.Fatal(err)
	}
	if len(reused) != 1 || len(pending) != 0 || len(effective) != 1 {
		t.Fatalf("reuse = %#v, pending = %#v, effective = %#v", reused, pending, effective)
	}
	if reused[0].SourceInputDigest != distributions[0].SourceInputDigest ||
		reused[0].SourceArtifactDigest != sourceArtifact.SHA256 ||
		reused[0].OutputArtifactDigest != wheel.SHA256 {
		t.Fatalf("reused source identity = %#v", reused[0])
	}
	if err := providerstore.VerifyArtifactFile(
		filepath.Join(prepared.InputHostDir, filepath.Base(wheel.LogicalPath)), wheel,
	); err != nil {
		t.Fatal(err)
	}
}

func TestPreparedPythonNodeOperationsDoesNotReuseWheelAcrossBuildEnvironments(t *testing.T) {
	store, err := providerstore.NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	wheel, err := store.Publish(
		context.Background(), "wheels/demo_pkg-1.0-py3-none-any.whl", "wheel", strings.NewReader("old wheel"),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	prior := testPythonResolvedSourceWithSourceArtifact(
		"application", "demo-pkg", "1.0",
		distributions[0].SourceInputDigest, sourceArtifact, wheel.SHA256,
	)
	prior.BuildEnvironmentDigest = canonical.Digest("sha256:" + strings.Repeat("f", 64))
	operations := PreparedPythonNodeOperations{
		Store: store, Artifacts: prepared,
		PriorSources:      []providers.ResolvedSourceInput{prior},
		PriorSourceWheels: []providerstore.ArtifactDescriptor{wheel},
	...
	if len(reused) != 0 || len(pending) != 1 || len(effective) != 0 {
		t.Fatalf("reuse = %#v, pending = %#v, effective = %#v", reused, pending, effective)
	}
}
