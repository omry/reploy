...
func TestPublishPortablePythonAliasV1TemporaryCleanupFailureReportsDestinationRemovalFailure(t *testing.T) {
	fixture := newPortablePythonAliasFixtureV1(t)
	oldRemove := portablePythonAliasRemoveV1
	temporaryRemoveErr := errors.New("injected temporary alias removal failure")
	destinationRemoveErr := errors.New("injected published alias removal failure")
	removeCalls := 0
	portablePythonAliasRemoveV1 = func(parent *os.Root, name string) error {
		if name == filepath.Base(fixture.Destination) {
			removeCalls++
			return destinationRemoveErr
		}
		return temporaryRemoveErr
	}
	t.Cleanup(func() { portablePythonAliasRemoveV1 = oldRemove })

	_, err := publishPortablePythonAliasV1(fixture.Root, fixture.Destination, fixture.Target, "binding/demo", fixture.Claims)
	if err == nil || !errors.Is(err, temporaryRemoveErr) || !errors.Is(err, destinationRemoveErr) || !strings.Contains(err.Error(), "remove published Python alias destination") {
		t.Fatalf("temporary and destination removal errors = %v", err)
	}
	if removeCalls != 1 {
		t.Fatalf("destination removal calls = %d, want one best-effort removal", removeCalls)
	}
