...
func TestPrepareDockerControllerV1RollsBackOwnedAmbiguousCreateFailure(t *testing.T) {
	...
	_, err = prepareDockerControllerV1(t.Context(), plan, dockerControllerBackendV1{
		...
		run: func(spec CommandSpec, options RunOptions) error {
			...
			case len(spec.Args) >= 2 && spec.Args[0] == "container" && spec.Args[1] == "inspect":
				_, _ = fmt.Fprintf(options.Stdout, "%q %s %q", dockerControllerTestContainerIDV1, encoded, "created")
			}
			return nil
		},
		observe: func(context.Context, CommandSpec, string) (int, error) { return 0, nil },
	...
	if len(runs) != 3 || !reflect.DeepEqual(runs[0].Args, plan.Create.Args) ||
		!reflect.DeepEqual(runs[1].Args, []string{
			"container", "inspect", "--format", "{{json .Id}} {{json .Config.Labels}} {{json .State.Status}}", plan.Container,
		}) ||
		!reflect.DeepEqual(runs[2].Args, []string{"container", "rm", dockerControllerTestContainerIDV1}) {
		t.Fatalf("ambiguous create reconciliation = %#v", runs)
	}
}

func TestPrepareDockerControllerV1RefusesAmbiguousCreateRollbackWithoutFullID(t *testing.T) {
	plan := controlledSessionControllerPlanFixtureV1(t)
	labels := map[string]string{}
	for _, label := range plan.Labels {
		labels[label.Name] = label.Value
	...
	_, err = prepareDockerControllerV1(t.Context(), plan, dockerControllerBackendV1{
		...
		run: func(spec CommandSpec, options RunOptions) error {
			...
			if len(spec.Args) >= 2 && spec.Args[0] == "container" && spec.Args[1] == "inspect" {
				_, _ = fmt.Fprintf(options.Stdout, "%q %s %q", "short-id", encoded, "created")
			}
			return nil
		},
		observe: func(context.Context, CommandSpec, string) (int, error) { return 0, nil },
	})
	if err == nil || !strings.Contains(err.Error(), "invalid full container ID") {
		t.Fatalf("invalid inspected ID error = %v", err)
	}
	if len(runs) != 2 {
		t.Fatalf("invalid inspected ID invoked removal: %#v", runs)
	}
}
...
func TestPrepareDockerControllerV1DoesNotRemoveRunningOwnedCreateCollision(t *testing.T) {
	...
	_, err = prepareDockerControllerV1(t.Context(), plan, dockerControllerBackendV1{
		...
		run: func(spec CommandSpec, options RunOptions) error {
			...
			if len(spec.Args) >= 2 && spec.Args[0] == "container" && spec.Args[1] == "inspect" {
				_, _ = fmt.Fprintf(options.Stdout, "%q %s %q", dockerControllerTestContainerIDV1, encoded, "running")
			}
			return nil
		},
		observe: func(context.Context, CommandSpec, string) (int, error) { return 0, nil },
...
func TestPrepareDockerControllerV1DoesNotRemoveForeignCreateCollision(t *testing.T) {
	...
	_, err := prepareDockerControllerV1(t.Context(), plan, dockerControllerBackendV1{
		...
		run: func(spec CommandSpec, options RunOptions) error {
			...
			if len(spec.Args) >= 2 && spec.Args[0] == "container" && spec.Args[1] == "inspect" {
				_, _ = fmt.Fprintf(options.Stdout, "%q %s %q", dockerControllerTestContainerIDV1, `{"foreign":"container"}`, "created")
			}
			return nil
		},
		observe: func(context.Context, CommandSpec, string) (int, error) { return 0, nil },
