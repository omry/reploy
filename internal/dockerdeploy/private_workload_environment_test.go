...
func TestRenderDockerInputsUsesSecretFreePrivateLauncher(t *testing.T) {
	...
	for _, want := range []string{
		...
		"entrypoint: [/opt/reploy/runtime/reploy-probe]", "command: [sandbox-exec, --uid", "--public, deny", "--local, deny", "/bin/sh, -c",
...
func TestStartAndInjectPrivateWorkloadEnvironmentV1CleansFailedContainer(t *testing.T) {
	...
	if len(order) != 3 || order[0] != "compose up" || !strings.HasPrefix(order[1], "exec -i demo /opt/reploy/runtime/reploy-probe restricted-exec ") || order[2] != "compose down" {
