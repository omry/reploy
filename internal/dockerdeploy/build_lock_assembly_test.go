...
func TestAssembleBuildLockPublishesCompleteGraphLock(t *testing.T) {
	...
	originalPortableProviderPlan := providers.NormalizeProviderPlanV1(portableTools.Plan.ProviderPlan)
	for index := range graph.Plan.Nodes {
		if graph.Plan.Nodes[index].ID != fixture.request.NodeID {
			continue
		}
		requestValue := canonical.Object{}
		for key, value := range graph.Plan.Nodes[index].Request.Value {
			requestValue[key] = value
		}
		overrides, ok := requestValue["overrides"].([]any)
		if !ok || len(overrides) == 0 {
			t.Fatal("Python fixture must contain an override to narrow")
		}
		requestValue["overrides"] = append([]any{}, overrides[:len(overrides)-1]...)
		narrowedRequest := graph.Plan.Nodes[index].Request
		narrowedRequest.Value = requestValue
		for componentIndex := range request.Components {
			if request.Components[componentIndex].Provider == blueprint.ComponentTypePython {
				request.Components[componentIndex].Request = narrowedRequest
			}
		}
	}
	graph.Plan, err = registry.Plan(providers.PlanInput{
	...
	if lock.PortableTools == nil || !reflect.DeepEqual(lock.PortableTools.Plan.ProviderPlan, graph.Plan) {
		t.Fatal("assembled portable tool lock does not use the effective provider plan")
	}
	if !reflect.DeepEqual(portableTools.Plan.ProviderPlan, originalPortableProviderPlan) {
		t.Fatal("assembled build lock mutated its caller-owned portable tool plan")
	}
