...
func TestOpenPortableToolValidationSessionCleansActiveCreateErrorWithOwnership(t *testing.T) {
	descriptor := testProbeImageDescriptor(t, "linux/amd64")
	workspace := testPreparedProbeWorkspace(t, descriptor.Platform, t.TempDir())
	owner := strings.Repeat("a", 64)
	createErr := errors.New("Docker create response lost after dispatch")
	created := false
	inspectCalled := false
	removeCalled := false
	cleanupHasDeadline := false
		...
		return func(spec CommandSpec, options RunOptions) error {
			...
			case "container":
				...
				if options.Context.Err() != nil {
					t.Fatalf("ownership inspection received a canceled context: %v", options.Context.Err())
				}
				if options.Stdout != nil {
					_, _ = options.Stdout.Write([]byte(owner + "\n"))
				}
				return nil
			case "rm":
				removeCalled = true
	...
	_, err := openImageValidationSessionWithCreateOwnedBounded(
		context.Background(), descriptor, workspace, nil,
		portableToolValidationCreateCommandSpecWithOwnershipV1(owner),
		newPortableToolProbeCleanupContext, portableToolProbeCleanupOwnershipVerifierV1(owner),
	)
	if err == nil || !errors.Is(err, createErr) {
		t.Fatalf("active create error = %v, want wrapped create error", err)
	}
	if !created {
		t.Fatal("active create error did not reach the container-creation boundary")
	}
	if !inspectCalled || !removeCalled {
		t.Fatalf("active create error cleanup calls: inspect=%t remove=%t, want both", inspectCalled, removeCalled)
	}
	if !cleanupHasDeadline {
		t.Fatal("active create error cleanup had no explicit deadline")
	}
	if remaining := time.Until(cleanupDeadline); remaining <= 0 || remaining > portableToolProbeCleanupTimeout {
		t.Fatalf("active create error cleanup deadline remaining = %v, want (0, %v]", remaining, portableToolProbeCleanupTimeout)
	}
}

func TestOpenPortableToolValidationSessionTreatsAbsentContainerAfterActiveCreateErrorAsClean(t *testing.T) {
	descriptor := testProbeImageDescriptor(t, "linux/amd64")
	workspace := testPreparedProbeWorkspace(t, descriptor.Platform, t.TempDir())
	owner := strings.Repeat("a", 64)
	createErr := errors.New("Docker create failed before returning a container")
		...
		return func(spec CommandSpec, options RunOptions) error {
			...
			case "container":
				...
				if options.Stderr != nil {
					_, _ = options.Stderr.Write([]byte("Error: No such container: " + imageProbeContainerName(workspace.HostDir) + "\n"))
				}
				return inspectErr
			case "rm":
				t.Fatal("absent container reached docker rm")
				return nil
			default:
				return fmt.Errorf("unexpected Docker command: %#v", spec.Args)
			}
	...
	if err == nil || !errors.Is(err, createErr) {
		t.Fatalf("absent-container create error = %v, want wrapped create error", err)
	}
	if errors.Is(err, inspectErr) || strings.Contains(err.Error(), "refuse image validation container cleanup") {
		t.Fatalf("absent-container create error included spurious cleanup failure: %v", err)
	}
	if !inspectCalled {
		t.Fatal("absent-container create error did not verify cleanup ownership")
	}
}
...
func TestRunPortableToolValidationProfileClassifiesOutputLimitWithTermination(t *testing.T) {
	descriptor := testProbeImageDescriptor(t, "linux/amd64")
	workspace := testPreparedProbeWorkspace(t, descriptor.Platform, t.TempDir())
	profile := portableToolValidationProfile(toolcatalog.RecordProbeV1{Path: "/opt/demo/bin/demo", Args: []string{"--output"}})
	var runErr error
	if runtime.GOOS != "windows" {
		...
		runErr = fmt.Errorf("probe terminated by signal: %w", signalErr)
	}
	stubPortableToolProbeCommands(t, []portableToolProbeStubResponse{{
	...
	result := portableToolProbeResults(t, evidence)[0]
	if got := portableToolStringField(t, result, "Outcome"); got != PortableToolProbeOutcomeOutputLimitV1 {
		t.Fatalf("terminated output-limit outcome = %q, want %q", got, PortableToolProbeOutcomeOutputLimitV1)
	}
	if got := portableToolNullableStringField(t, result, "ExitCode"); got != "" {
		t.Fatalf("terminated output-limit exit code = %q, want null/empty", got)
	}
