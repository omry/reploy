...
func TestRunPortableToolValidationProfileCleanupCannotHangAfterLifecycleDeadline(t *testing.T) {
	descriptor := testProbeImageDescriptor(t, "linux/amd64")
	workspace := testPreparedProbeWorkspace(t, descriptor.Platform, t.TempDir())
	...
	if !cleanupHasDeadline {
		t.Fatal("cleanup context has no explicit deadline")
	}
	select {
	case <-cleanupFinished:
	case <-time.After(time.Second):
		t.Fatal("bounded cleanup runner did not finish after release")
	}
}
