...
const (
	portableRuntimeImageInventoryMaxBytesV1   = int64(providerstore.CoreMaxArchiveUnpackedBytes)
	portableRuntimeImageInventoryMaxEntriesV1 = providerstore.CoreMaxArchiveEntries
)
...
func collectPortableRuntimeImageInventoryV1(
	...
	for _, root := range roots {
		entries, err := copyPortableRuntimeInventoryRootV1(ctx, session, root)
		if err != nil {
			return nil, err
		}
...
func copyPortableRuntimeInventoryRootV1(
	ctx context.Context, session *ImageValidationSession, root string,
) (result map[string]portableRuntimeInventoryEntryV1, resultErr error) {
	...
	counting := &portableRuntimeInventoryCountingReader{reader: reader}
	tarReader := tar.NewReader(io.LimitReader(counting, portableRuntimeImageInventoryMaxBytesV1))
	observed := map[string]portableRuntimeInventoryEntryV1{}
	entryCount := 0
	for {
		...
		entryCount++
		if entryCount > portableRuntimeImageInventoryMaxEntriesV1 {
			resultErr = fmt.Errorf("final runtime payload inventory %s exceeds %d-entry limit", root, portableRuntimeImageInventoryMaxEntriesV1)
			break
		}
		...
		case tar.TypeReg, tar.TypeRegA:
			if header.Size < 0 || header.Size > portableRuntimeImageInventoryMaxBytesV1 {
	...
	if counting.bytes >= portableRuntimeImageInventoryMaxBytesV1 {
		return nil, fmt.Errorf("final runtime payload inventory %s exceeds %d-byte limit", root, portableRuntimeImageInventoryMaxBytesV1)
	}
	return observed, nil
}
...
func (reader *portableRuntimeInventoryCountingReader) Read(p []byte) (int, error) {
	if reader.bytes >= portableRuntimeImageInventoryMaxBytesV1 {
		return 0, io.ErrUnexpectedEOF
	}
	if remaining := portableRuntimeImageInventoryMaxBytesV1 - reader.bytes; int64(len(p)) > remaining {
		p = p[:remaining]
	}
