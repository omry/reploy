...
func requirePortableRuntimeDestinationsAbsent(
	...
	if err := validatePortableRuntimeDestinationCheckV1(ctx, session, destinations, portableRuntimeDestinationAbsenceCheckV1, "destination absence"); err != nil {
		return err
	}
...
func validatePortableRuntimeDestinationAncestorsV1(
	...
	return validatePortableRuntimeDestinationCheckV1(ctx, session, destinations, portableRuntimeDestinationAncestorSymlinkCheckV1, "destination ancestor symlink")
}

// These destinations are positional docker exec arguments, not comma-delimited
// mount options. Keep their domain aligned with the validated catalog paths.
func validatePortableRuntimeDestinationPathV1(destination string) error {
	if destination == "" || destination == "/" || !utf8.ValidString(destination) ||
		!path.IsAbs(destination) || path.Clean(destination) != destination || strings.Contains(destination, `\`) {
		return fmt.Errorf("runtime payload destination %q is not a canonical absolute path", destination)
	}
	for _, char := range destination {
		if unicode.IsControl(char) {
			return fmt.Errorf("runtime payload destination %q contains a control character", destination)
		}
	}
	return nil
}

func validatePortableRuntimeDestinationCheckV1(
	ctx context.Context, session *ImageValidationSession, destinations []string, script, label string,
) error {
	if session == nil || session.closed {
		return fmt.Errorf("image validation session is not open")
	}
	if ctx == nil {
		return fmt.Errorf("runtime payload %s context is required", label)
	}
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("validate runtime payload %s: %w", label, err)
	}
	if len(destinations) == 0 {
		return fmt.Errorf("runtime payload %s requires destinations", label)
	}
	for _, destination := range destinations {
		if err := validatePortableRuntimeDestinationPathV1(destination); err != nil {
			return fmt.Errorf("runtime payload %s check: %w", label, err)
		}
	}
	args := []string{
		"exec", "--user", "0:0", "--workdir", "/", session.containerName,
		"/bin/sh", "-c", script, "reploy-validation",
	}
	...
	if err := session.runDockerCommand(CommandSpec{Name: "docker", Args: args}, RunOptions{
		...
		return fmt.Errorf("runtime payload %s check failed: %w", label, imageValidationCommandError(label+" check", session.descriptor.Platform.Canonical, stderr.String(), err))
	}
	return nil
}
