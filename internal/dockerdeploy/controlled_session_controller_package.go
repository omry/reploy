...
const (
	...
	controlledSessionControllerExecutableV1    = "/usr/local/bin/reploy"
...
func buildControlledSessionControllerPackageV1(
	...
	if platform.OS != "linux" || !probearchive.SupportsController(platform.Canonical) {
	...
	extracted, err := probearchive.ExtractController(ctx, hostExecutable, platform.Canonical, contextDir)
	if err != nil {
		return nil, fmt.Errorf("extract matching controller Reploy: %w", err)
	}
...
func controlledSessionControllerPackageDockerfileV1(source InspectedImageCandidate, extracted probearchive.ExtractedController) ([]byte, deploy.BaseConfig, map[string]string, error) {
	...
	if !probearchive.SupportsController(extracted.Platform) || extracted.Path == "" || extracted.Size == "" {
		return nil, deploy.BaseConfig{}, nil, fmt.Errorf("controlled-session controller package requires an extracted supported controller executable")
	}
	if err := extracted.SHA256.Validate(); err != nil {
		return nil, deploy.BaseConfig{}, nil, fmt.Errorf("controlled-session controller executable: %w", err)
	}
	...
	output.WriteString("COPY --chmod=0555 reploy /usr/local/bin/reploy\n")
...
func validateControlledSessionControllerPackageImageV1(source InspectedImageCandidate, inspected InspectedImageCandidate, expectedConfig deploy.BaseConfig, expectedLabels map[string]string) error {
	...
	if !reflect.DeepEqual(inspected.Labels, expectedLabels) {
		return fmt.Errorf("controlled-session controller package labels do not match the exact embedded executable")
	}
...
func validateControllerReleaseV1(release probearchive.ReleaseV1, want probearchive.ReleaseV1) error {
	if release != want {
		return fmt.Errorf("embedded controller Reploy release does not match host Reploy: controller %#v, host %#v", release, want)
	}
	return nil
}
...
func validateControlledSessionControllerPackageForReleaseV1(packagePlan ControlledSessionControllerPackageV1, hostRelease probearchive.ReleaseV1) error {
	...
	if packagePlan.Platform.OS != "linux" || !probearchive.SupportsController(packagePlan.Platform.Canonical) {
