...
func writePortablePythonAliasArchiveV1(contextDir, stagingRoot string, aliases []portablePythonAliasSpecV1, missingParents []string) error {
	...
	if err := filepath.WalkDir(stagingRoot, func(current string, entry os.DirEntry, walkErr error) error {
		...
		if entry.Type()&os.ModeSymlink == 0 {
			return fmt.Errorf("Python alias staging entry %q must be a symlink", filepath.ToSlash(relative))
		}
		return nil
	}); err != nil {
	...
	for _, alias := range aliases {
		...
		if err != nil {
			return fmt.Errorf("inspect staged Python alias %q: %w", alias.Name, err)
		}
		if info.Mode()&os.ModeSymlink == 0 {
			return fmt.Errorf("Python alias staging root contains an unexpected non-symlink for %q", alias.Name)
		}
		target, err := os.Readlink(stagedPath)
		if err != nil {
...
func validatePortablePythonAliasFinalImageEvidenceV1(
	...
	for index, alias := range aliases {
		...
		if strings.Contains(observation.Terminal.Path, "/.reploy") {
