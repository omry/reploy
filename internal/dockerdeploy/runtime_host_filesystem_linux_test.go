...
func TestRuntimeHostMountIdentityForPathV1FallsBackWithoutMountIDs(t *testing.T) {
	...
	if !runtimeHostMountIdentitiesExposeSameRootV1(rootAlias, root, "/safe/root") {
	...
	protected, err := runtimeHostMountIdentitySharesProtectedTreeV1(data, devAlias, "/dev")
	if err != nil {
		t.Fatal(err)
	}
	if !protected {
...
func TestRuntimeHostMountSharesProtectedTreeV1(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "dev shm alias", candidate: 44, tree: "/dev", want: true},
		{name: "dev shm subdirectory alias", candidate: 46, tree: "/dev", want: true},
		{name: "sys resctrl alias", candidate: 48, tree: "/sys", want: true},
		{name: "unrelated tmpfs", candidate: 45, tree: "/dev", want: false},
	} {
		t.Run(test.name, func(t *testing.T) {
			got, err := runtimeHostMountSharesProtectedTreeV1(data, test.candidate, test.tree)
			if err != nil {
				t.Fatal(err)
			}
			if got != test.want {
