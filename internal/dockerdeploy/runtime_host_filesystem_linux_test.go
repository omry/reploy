...
func TestProtectedRuntimeHostPathWithResolverV1FallsBackWithoutOpenat2(t *testing.T) {
	...
	for _, path := range []string{alias + string(filepath.Separator) + "..", target + string(filepath.Separator) + "."} {
		if _, err := protectedRuntimeHostPathWithResolverV1(path, unsupported); err == nil {
			t.Fatalf("non-normalized path %q accepted without openat2", path)
		}
	}
}
...
func TestProtectedRuntimeHostFilesystemNameV1RecognizesKernelInterfaces(t *testing.T) {
	for _, filesystem := range []string{
		"proc", "sysfs", "devpts", "devtmpfs", "cgroup2", "configfs",
		"debugfs", "securityfs", "bpf", "binfmt_misc", "efivarfs",
		"fusectl", "hugetlbfs", "mqueue", "resctrl", "rpc_pipefs",
	} {
		if !protectedRuntimeHostFilesystemNameV1(filesystem) {
			t.Fatalf("kernel interface filesystem %q was not protected", filesystem)
		}
	}
	for _, filesystem := range []string{"ext4", "tmpfs", "fuse", "overlay", "rootfs", "9p"} {
		if protectedRuntimeHostFilesystemNameV1(filesystem) {
			t.Fatalf("ordinary data filesystem %q was protected", filesystem)
		}
	}
}
...
func TestRuntimeHostMountContainsProtectedSubmountV1(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "standalone nested kernel filesystem",
			data: "41 41 0:1 / / rw - ext4 /dev/root rw\n" +
