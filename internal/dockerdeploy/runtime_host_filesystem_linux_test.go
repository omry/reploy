...
func TestRuntimeHostMountSharesProtectedTreeV1(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "dev shm alias", candidate: 44, want: true},
		{name: "dev shm subdirectory alias", candidate: 46, want: true},
		{name: "unrelated tmpfs", candidate: 45, want: false},
	} {
		t.Run(test.name, func(t *testing.T) {
			got, err := runtimeHostMountSharesProtectedTreeV1(data, test.candidate, "/dev")
			if err != nil {
				t.Fatal(err)
			}
			if got != test.want {
