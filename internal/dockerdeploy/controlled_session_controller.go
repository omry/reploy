...
func parseDockerContainerIDV1(output string) (string, error) {
	...
	if err := validateDockerContainerIDV1(containerID); err != nil {
		return "", fmt.Errorf("Docker create returned %w", err)
	}
	return containerID, nil
}

func validateDockerContainerIDV1(containerID string) error {
	if len(containerID) != 64 {
		return fmt.Errorf("invalid full container ID %q", containerID)
	}
...
func rollbackAmbiguousControlledSessionControllerCreateV1(
	...
	if err := decoder.Decode(&containerID); err != nil {
		return fmt.Errorf("decode possible controller container ID: %w", err)
	}
	if err := validateDockerContainerIDV1(containerID); err != nil {
		return fmt.Errorf("validate possible controller container ID: %w", err)
	}
	...
	defer removeCancel()
	remove := CommandSpec{Name: plan.Cleanup.Name, Args: []string{"container", "rm", containerID}}
