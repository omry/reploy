...
type FullImageValidationInput struct {
	...
	// PortableTools carries the selected validation profiles compiled into the
	// provider plan. Its zero value schedules no portable-tool validation,
	// which is the correct outcome for a build that selects no portable tool.
