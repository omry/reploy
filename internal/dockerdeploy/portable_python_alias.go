...
func publishPortablePythonAliasV1(
	...
	if err := portablePythonAliasRemoveV1(parent, temporary); err != nil && !errors.Is(err, fs.ErrNotExist) {
		removeErr := removePortablePythonAliasDestinationAfterFailureV1(parent, destinationName)
...
// removePortablePythonAliasDestinationAfterFailureV1 removes the destination
// after publication has become unverifiable. A failed removal is part of the
// returned error because the destination may still be a usable alias and the
// caller must be able to retry or surface that state to its owner.
func removePortablePythonAliasDestinationAfterFailureV1(parent *os.Root, destination string) error {
	if err := portablePythonAliasRemoveV1(parent, destination); err != nil && !errors.Is(err, fs.ErrNotExist) {
