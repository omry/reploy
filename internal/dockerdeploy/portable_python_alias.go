...
// validation. The default publish operation is an OS-specific no-replace
// rename of the temporary symlink. Both names are resolved relative to the
// held parent handle.
var (
	portablePythonAliasCreateSymlinkV1 = func(parent *os.Root, target, name string) error {
...
// and an already matching symlink return false, nil. Errors leave no usable
...
func publishPortablePythonAliasV1(
	...
	existing, statErr := root.Lstat(relativeDestination)
	switch {
	case statErr == nil:
		if existing.Mode()&os.ModeSymlink == 0 {
			return false, fmt.Errorf("Python alias destination %q is pre-existing and unowned", destination)
		}
		linkTarget, readErr := root.Readlink(relativeDestination)
		if readErr != nil {
	...
	if linkTarget, readErr := parent.Readlink(temporary); readErr != nil || linkTarget != target {
		if readErr != nil {
	...
	// The destination now owns the symlink inode. If cleanup of the temporary
	...
	// Re-read the link from the held parent handle. The payload is deliberately
	...
	linkTarget, err := parent.Readlink(destinationName)
	if err != nil {
...
func preparePortablePythonAliasParentV1(root *os.Root, relative string) (*os.Root, []string, error) {
	...
	for index := range parts {
		...
		if createdHere && createdInfo.Mode().Perm() != 0o755 {
			rollbackParents()
...
func createPortablePythonAliasTemporaryV1(parent *os.Root, target string) (string, error) {
	for attempt := 0; attempt < 32; attempt++ {
		...
		if err := portablePythonAliasCreateSymlinkV1(parent, target, name); err != nil {
