...
func TestPrepareProviderGraphValidationBuildsCumulativeLayerAndFinalInputs(t *testing.T) {
	...
	finalOnly, err := prepareProviderGraphValidation(
		context.Background(), fixture.lock.Base, fixture.request.EarlierCatalog, graph, fixture.lock.RuntimePolicy,
		false,
		func(_ context.Context, candidate BuiltImageCandidate, _ blueprint.Platform) (InspectedImageCandidate, error) {
			if candidate.ImageID != layerImage.ConfigDigest {
				t.Fatalf("final-only inspection candidate = %#v", candidate)
			}
			return inspectedValidationCandidate(t, layerDescriptor), nil
		},
	)
	...
	if len(finalOnly.Layers) != 0 || !reflect.DeepEqual(finalOnly.Final, plan.Final) {
		t.Fatalf("final-only validation plan = %#v", finalOnly)
	}
}
...
func TestPrepareProviderGraphValidationRejectsChangedLayerAndCatalogDrift(t *testing.T) {
	...
	_, err = prepareProviderGraphValidation(context.Background(), fixture.lock.Base, fixture.request.EarlierCatalog, graph, fixture.lock.RuntimePolicy, true, func(context.Context, BuiltImageCandidate, blueprint.Platform) (InspectedImageCandidate, error) {
