...
func (network *DockerSessionNetworkV1) verifyInertContainerV1(
	...
	if !reflect.DeepEqual(inspection.Config.Labels, controlledSessionContainerLabelMapV1(plan)) {
		return fmt.Errorf("controlled-session %s container ownership labels do not match the exact plan", plan.Role)
	}
