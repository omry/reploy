...
func TestDockerControllerIntegration(t *testing.T) {
	...
	t.Run("removes an owned inert create collision", func(t *testing.T) {
		...
		if err := exec.CommandContext(ctx, "docker", "container", "inspect", plan.Controller.Container).Run(); err == nil {
			t.Fatal("owned inert controller survived ambiguous create reconciliation")
		}
	})
	...
	t.Run("preserves a foreign create collision", func(t *testing.T) {
		...
		if _, err := PrepareDockerControllerV1(ctx, plan.Controller); err == nil || !strings.Contains(err.Error(), "ownership labels do not match") {
	...
	t.Run("preserves a running owned create collision", func(t *testing.T) {
		...
		if _, err := PrepareDockerControllerV1(ctx, plan.Controller); err == nil || !strings.Contains(err.Error(), `Docker state is "running"`) {
