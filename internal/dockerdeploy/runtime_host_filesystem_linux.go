...
func runtimeHostSharesProtectedMountV1(path string) (bool, error) {
	...
	for _, protectedTree := range []string{"/proc", "/dev", "/sys"} {
		matched, err := runtimeHostMountIdentitySharesProtectedTreeV1(data, candidate, protectedTree)
		if err != nil || matched {
			return matched, err
		}
	}
...
func runtimeHostSharesRootFilesystemV1(path string) (bool, error) {
	...
	return runtimeHostMountIdentitiesExposeSameRootV1(candidate, root, path), nil
}
...
func runtimeHostMountIdentityByPathV1(data []byte, path string) (runtimeHostMountIdentityV1, error) {
	...
	var best runtimeHostMountIdentityV1
	bestLength := -1
	for _, line := range strings.Split(string(data), "\n") {
		...
		length := len(filepath.Clean(identity.mountPoint))
...
func runtimeHostMountIdentitiesExposeSameRootV1(candidate runtimeHostMountIdentityV1, root runtimeHostMountIdentityV1, path string) bool {
	return candidate.mountPoint != root.mountPoint &&
		filepath.Clean(path) == filepath.Clean(candidate.mountPoint) &&
		candidate.device == root.device && candidate.root == root.root
}
...
func runtimeHostMountSharesProtectedTreeV1(data []byte, candidateID uint64, protectedTree string) (bool, error) {
	...
	return runtimeHostMountIdentitySharesProtectedTreeV1(data, candidate, protectedTree)
}

func runtimeHostMountIdentitySharesProtectedTreeV1(data []byte, candidate runtimeHostMountIdentityV1, protectedTree string) (bool, error) {
	for _, line := range strings.Split(string(data), "\n") {
		fields := strings.Fields(line)
		if len(fields) == 0 {
			continue
