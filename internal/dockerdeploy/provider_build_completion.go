...
func completeProviderBuild(
	...
	if backend.validateAndFinalize == nil || backend.assemble == nil || backend.publish == nil {
	...
	state, err := backend.publish(ctx, operation, store, BuildPublicationInput{
		Environment: input.Environment, DeploymentDir: input.DeploymentDir, Document: input.Document, Lock: lock,
	})
	if err != nil {
		return ProviderBuildCompletionResult{}, err
	}
