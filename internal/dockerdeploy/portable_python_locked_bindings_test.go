...
func TestExecutePreparedPythonGraphDerivesLockedBindingsAndRejectsCachedSubstitution(t *testing.T) {
	...
	_, err = ExecutePreparedPythonGraph(context.Background(), PreparedPythonGraphExecutionInput{
		...
		DesiredPortableToolPlan: &lockedFixture.lock.Plan.PortableToolPlan,
...
func TestExecutePreparedPythonGraphDropsRemovedLockedBinding(t *testing.T) {
	reuseFixture := newPreparedPythonGraphReuseFixture(t)
	lockedFixture := newPortableToolPythonLockedTestFixture(t)
	var err error
	for _, node := range reuseFixture.request.Plan.Nodes {
	...
	config := configs[reuseFixture.request.NodeID]
	if config.PortableToolBindings != nil || config.PortableToolLockedPlan != nil {
		t.Fatalf("removed locked binding was selected: %#v", config)
	}
	if _, found := execution.CachedResolutions[reuseFixture.request.NodeID]; found {
		t.Fatalf("cached resolution retained removed locked binding: %#v", execution.CachedResolutions)
	}
}

func TestExecutePreparedPythonGraphRequiresFreshPlanForChangedDesiredBinding(t *testing.T) {
	reuseFixture := newPreparedPythonGraphReuseFixture(t)
	lockedFixture := newPortableToolPythonLockedTestFixture(t)
	for _, node := range reuseFixture.request.Plan.Nodes {
		if node.ID != "base" {
	...
	reuseFixture.lock.PortableTools = &lockedFixture.lock
	desired := lockedFixture.lock.Plan.PortableToolPlan
	desired.Tools = append([]providers.PortableToolPlanEntryV1{}, desired.Tools...)
	desired.Tools[0].SelectedClosureDigest = rendererDigest("d")

	_, err := ExecutePreparedPythonGraph(context.Background(), PreparedPythonGraphExecutionInput{
		Store: reuseFixture.store, Plan: reuseFixture.request.Plan,
		BaseDescriptor: reuseFixture.lock.Base, Sources: []providers.ResolvedSourceInput{},
		SourceWheels: []providerstore.ArtifactDescriptor{}, CurrentLock: &reuseFixture.lock,
		DesiredPortableToolPlan: &desired,
		FinalImageConfig:        pythonConsumerTestImageConfig(),
	})
	if err == nil || !strings.Contains(err.Error(), "requires a fresh plan") {
		t.Fatalf("changed desired binding error = %v", err)
	}
}

func TestExecutePreparedPythonGraphRejectsMismatchedFreshAndDesiredBindings(t *testing.T) {
	...
	_, err := ExecutePreparedPythonGraph(context.Background(), PreparedPythonGraphExecutionInput{
		...
		PortablePython: &fresh, DesiredPortableToolPlan: &desired,
		FinalImageConfig: pythonConsumerTestImageConfig(),
	})
	if err == nil || !strings.Contains(err.Error(), "fresh portable Python selection does not match desired selection") {
		t.Fatalf("mismatched fresh selection error = %v", err)
	}
}

func TestExecutePreparedPythonGraphIgnoresLockedBindingForRemovedComponent(t *testing.T) {
	reuseFixture := newPreparedPythonGraphReuseFixture(t)
	lockedFixture := newPortableToolPythonLockedTestFixture(t)
	var err error
	for _, node := range reuseFixture.request.Plan.Nodes {
	...
	reuseFixture.lock.PortableTools = &lockedFixture.lock
	plan := reuseFixture.request.Plan
	plan.Nodes = append([]providers.NodeSpec{}, plan.Nodes...)
	plan.Edges = append([]providers.ProviderEdgeV1{}, plan.Edges...)
	removedID := providers.NodeID("python/application/removed")
	for index := range plan.Nodes {
		if plan.Nodes[index].Provider != blueprint.ComponentTypePython {
			continue
		}
		oldID := plan.Nodes[index].ID
		plan.Nodes[index].ID = removedID
		plan.Nodes[index].Components = []string{
			blueprint.ApplicationContributionID("removed", blueprint.ContributionProviderPython),
		}
		for edgeIndex := range plan.Edges {
			if plan.Edges[edgeIndex].Consumer == oldID {
				plan.Edges[edgeIndex].Consumer = removedID
			}
		}
	}
	...
	_, err = ExecutePreparedPythonGraph(context.Background(), PreparedPythonGraphExecutionInput{
		Store: reuseFixture.store, Plan: plan,
		BaseDescriptor: reuseFixture.lock.Base, Sources: []providers.ResolvedSourceInput{},
		SourceWheels: []providerstore.ArtifactDescriptor{}, CurrentLock: &reuseFixture.lock,
		FinalImageConfig: pythonConsumerTestImageConfig(),
	})
	...
	config := configs[removedID]
	if config.PortableToolBindings != nil || config.PortableToolLockedPlan != nil {
		t.Fatalf("removed component received locked binding: %#v", config)
	}
	if _, found := execution.CachedResolutions[removedID]; found {
		t.Fatalf("removed component retained cached resolution: %#v", execution.CachedResolutions)
	}
}
