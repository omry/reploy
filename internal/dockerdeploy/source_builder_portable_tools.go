...
// portable-tool plan for the immutable local-source recipes selected so far
// in one build. Exact reuse obtains the same selected identities from the
// already validated lock only after selected source identity matches.
...
// SourceBuilderSelectedRecipeV1 is path-free recipe data read from one
// immutable local-source snapshot after dependency resolution selected it.
type SourceBuilderSelectedRecipeV1 struct {
	Distribution string
	Recipe       PythonLocalSourceRecipeV1
}

func cloneSourceBuilderSelectedRecipeV1(selected SourceBuilderSelectedRecipeV1) SourceBuilderSelectedRecipeV1 {
	clone := selected
	if selected.Recipe.Requirements == nil {
		return clone
	}
	clone.Recipe.Requirements = make([]toolrequest.CanonicalRequirementGroupV1, len(selected.Recipe.Requirements))
	for index, group := range selected.Recipe.Requirements {
		if group.VersionConstraints != nil {
			group.VersionConstraints = append([]string{}, group.VersionConstraints...)
		}
		if group.Binding.Explicit != nil {
			group.Binding.Explicit = append([]string{}, group.Binding.Explicit...)
		}
		if group.Selections != nil {
			group.Selections = make(map[string][]string, len(group.Selections))
			for dimension, values := range selected.Recipe.Requirements[index].Selections {
				group.Selections[dimension] = append([]string{}, values...)
			}
		}
		clone.Recipe.Requirements[index] = group
	}
	return clone
}

// PlanSourceBuilderPortableToolsInputV1 carries the exact inputs one build
// supplies to source-builder portable-tool planning: the selected base image,
// the provider plan that owns the builder domains, selected immutable recipe
// data, and the blueprint and client identities sealed into the resolution
// snapshot.
...
// PlanSourceBuilderPortableToolsV1 resolves canonical build-scope tool
// requirements read from selected immutable source snapshots against the
// embedded portable catalog for the observed base target, then compiles the
// PTD-21 plan and provider DAG. A selection whose recipes declare no tool
// requirements plans nothing and returns nil. No acquisition or
...
func PlanSourceBuilderPortableToolsV1(
	...
	if input.SelectedRecipes == nil {
		return nil, fmt.Errorf("plan source-builder portable tools selected recipes must use an array")
	}
	recipes := map[string]SourceBuilderRecipeIdentityV1{}
	groups := []toolrequest.CanonicalRequirementGroupV1{}
	for index, selected := range input.SelectedRecipes {
		if selected.Distribution == "" || pythonprovider.NormalizeDistributionName(selected.Distribution) != selected.Distribution {
			return nil, fmt.Errorf("selected local Python recipe %d distribution is not normalized: %q", index, selected.Distribution)
		}
		if index > 0 && input.SelectedRecipes[index-1].Distribution >= selected.Distribution {
			return nil, fmt.Errorf("selected local Python recipes must be unique and sorted")
		}
		recipe := cloneSourceBuilderSelectedRecipeV1(selected).Recipe
		if !recipe.Found {
			if recipe.Project != "" || recipe.Build != "" || recipe.Digest != "" || len(recipe.Requirements) != 0 {
				return nil, fmt.Errorf("selected local source recipe for %q has inconsistent absent-recipe data", selected.Distribution)
			}
			continue
		}
		if recipe.Project != selected.Distribution {
			return nil, fmt.Errorf("selected local source recipe project %q does not match distribution %q", recipe.Project, selected.Distribution)
		}
		if err := recipe.Digest.Validate(); err != nil {
			return nil, fmt.Errorf("selected local source recipe for %q digest: %w", selected.Distribution, err)
		}
		if recipe.Requirements == nil {
			return nil, fmt.Errorf("selected local source recipe for %q requirements must use an array", selected.Distribution)
		}
		if len(recipe.Requirements) == 0 {
			continue
		}
		scope := sourceBuilderRecipeScopePrefixV1 + selected.Distribution
		for _, group := range recipe.Requirements {
			if group.Scope != scope {
				return nil, fmt.Errorf("local source recipe for %q carries requirement scope %q, want %q", selected.Distribution, group.Scope, scope)
			}
		}
		recipes[selected.Distribution] = SourceBuilderRecipeIdentityV1{
