...
func RunControlledSessionV1(
	...
	if filepath.Dir(filepath.Dir(operation.Path())) != absoluteDir {
		return ControlledSessionRunResultV1{}, releaseControlledSessionOperationV1(operation, fmt.Errorf("controlled-session operation lock does not belong to workload deployment %q", absoluteDir))
	}
