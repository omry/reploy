...
func TestValidatePythonRequiresPythonV1(t *testing.T) {
	...
	for _, value := range []string{
		...
		if err := portabletool.ValidatePythonRequiresPythonV1(value); err != nil {
			t.Errorf("canonical requires_python %q: %v", value, err)
		}
	}
	for _, value := range []string{
		"",
...
func TestValidateRecordEnvelopeV1RejectsNonPEPRequiresPythonExtensions(t *testing.T) {
	t.Parallel()
	for _, requiresPython := range []string{"3.12", "=3.12", ">=3.12||<2"} {
		value := readDefinitionObjectV1(t, "playwright/releases/1.61.0/bindings/python/linux-amd64.json")
		value["requires_python"] = requiresPython
		if err := portabletool.ValidateRecordEnvelopeV1(canonical.Envelope{
			Schema: portabletool.BindingArtifactSchemaV1,
			Value:  value,
		}); err == nil {
			t.Errorf("binding artifact requires_python %q was accepted", requiresPython)
		}
	}
}
