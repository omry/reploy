...
// container-native reploy-probe variants and controlled-session Reploy
...
var supportedControllerPlatforms = [...]string{"linux/amd64", "linux/arm64"}
...
type ManifestV1 struct {
	...
	Controllers []EntryV1 `json:"controllers"`
}
...
func Append(executable string, release ReleaseV1, inputs []HelperInput, controllers []ControllerInput) error {
	return appendWithVerifier(executable, release, inputs, controllers, func(path string) error {
		_, err := Verify(path)
		return err
	})
}

func appendWithVerifier(executable string, release ReleaseV1, inputs []HelperInput, controllers []ControllerInput, verify func(string) error) (resultErr error) {
	...
	helpers, manifest, err := prepare(release, inputs, controllers)
	if err != nil {
		return err
	}
...
func Verify(executable string) (ManifestV1, error) {
	...
	for _, entry := range archive.manifest.Controllers {
		file := archive.entries[entry.ArchivePath]
		reader, err := file.Open()
		if err != nil {
			return ManifestV1{}, fmt.Errorf("open embedded controller Reploy %s: %w", entry.Platform, err)
		}
		hash := sha256.New()
		size, copyErr := io.Copy(hash, reader)
		closeErr := reader.Close()
		if copyErr != nil {
			return ManifestV1{}, fmt.Errorf("read embedded controller Reploy %s: %w", entry.Platform, copyErr)
		}
		if closeErr != nil {
			return ManifestV1{}, fmt.Errorf("close embedded controller Reploy %s: %w", entry.Platform, closeErr)
		}
		if strconv.FormatInt(size, 10) != entry.Size {
			return ManifestV1{}, fmt.Errorf("embedded controller Reploy %s size does not match its manifest", entry.Platform)
		}
		actual := canonical.Digest(fmt.Sprintf("sha256:%x", hash.Sum(nil)))
		if actual != entry.SHA256 {
			return ManifestV1{}, fmt.Errorf("embedded controller Reploy %s digest does not match its manifest", entry.Platform)
		}
	}
	return archive.manifest, nil
}

func prepare(release ReleaseV1, inputs []HelperInput, controllers []ControllerInput) ([]preparedHelper, ManifestV1, error) {
	...
	helpers := make([]preparedHelper, 0, len(supportedPlatforms)+len(supportedControllerPlatforms))
	manifest := ManifestV1{
		Schema: ManifestSchemaV1, Release: release,
		Entries:     make([]EntryV1, 0, len(supportedPlatforms)),
		Controllers: make([]EntryV1, 0, len(supportedControllerPlatforms)),
	}
	...
	if len(controllers) != len(supportedControllerPlatforms) {
		return nil, ManifestV1{}, fmt.Errorf("runtime archive requires exactly %d controller executables", len(supportedControllerPlatforms))
	}
	controllersByPlatform := make(map[string]ControllerInput, len(controllers))
	for _, input := range controllers {
		if input.Platform == "" || input.Path == "" {
			return nil, ManifestV1{}, fmt.Errorf("controller executable platform and path are required")
		}
		if _, exists := controllersByPlatform[input.Platform]; exists {
			return nil, ManifestV1{}, fmt.Errorf("runtime archive repeats controller platform %q", input.Platform)
		}
		controllersByPlatform[input.Platform] = input
	}
	for _, platform := range supportedControllerPlatforms {
		input, exists := controllersByPlatform[platform]
		if !exists {
			return nil, ManifestV1{}, fmt.Errorf("runtime archive is missing controller platform %q", platform)
		}
		info, err := os.Lstat(input.Path)
		if err != nil {
			return nil, ManifestV1{}, fmt.Errorf("inspect controller Reploy %s: %w", platform, err)
		}
		if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 {
			return nil, ManifestV1{}, fmt.Errorf("controller Reploy %s must be a regular file", platform)
		}
		content, err := os.ReadFile(input.Path)
		if err != nil {
			return nil, ManifestV1{}, fmt.Errorf("read controller Reploy %s: %w", platform, err)
		}
		if len(content) == 0 {
			return nil, ManifestV1{}, fmt.Errorf("controller Reploy %s must not be empty", platform)
		}
		digest := sha256.Sum256(content)
		entry := EntryV1{
			Platform: platform, ArchivePath: controllerArchivePath(platform),
			Size: strconv.Itoa(len(content)), SHA256: canonical.Digest(fmt.Sprintf("sha256:%x", digest)),
		}
		helpers = append(helpers, preparedHelper{entry: entry, content: content})
		manifest.Controllers = append(manifest.Controllers, entry)
	}
	if len(controllersByPlatform) != len(supportedControllerPlatforms) {
		return nil, ManifestV1{}, fmt.Errorf("runtime archive contains an unsupported controller platform")
	}
...
func validateLayout(manifest ManifestV1, entries map[string]*zip.File) error {
	...
	if len(manifest.Entries) != len(supportedPlatforms) || len(manifest.Controllers) != len(supportedControllerPlatforms) || len(entries) != len(supportedPlatforms)+len(supportedControllerPlatforms)+1 {
		return fmt.Errorf("embedded runtime archive must contain exactly its manifest, three probes, and two controller executables")
	}
	...
	for index, platform := range supportedControllerPlatforms {
		entry := manifest.Controllers[index]
		if entry.Platform != platform || entry.ArchivePath != controllerArchivePath(platform) {
			return fmt.Errorf("embedded controller entries must use the complete sorted platform matrix")
		}
		if !canonicalPositive(entry.Size) {
			return fmt.Errorf("embedded controller Reploy %s has a noncanonical size", platform)
		}
		if err := entry.SHA256.Validate(); err != nil {
			return fmt.Errorf("embedded controller Reploy %s digest: %w", platform, err)
		}
		file, exists := entries[entry.ArchivePath]
		if !exists {
			return fmt.Errorf("embedded runtime archive omits controller Reploy %s", platform)
		}
		if err := validateZipEntry(file, 0o555); err != nil {
			return fmt.Errorf("embedded controller Reploy %s: %w", platform, err)
		}
		if strconv.FormatUint(file.UncompressedSize64, 10) != entry.Size {
			return fmt.Errorf("embedded controller Reploy %s ZIP size does not match its manifest", platform)
		}
	}
	return nil
}
...
func controllerArchivePath(platform string) string {
	switch platform {
	case "linux/amd64":
		return "reploy-controller/linux-amd64"
	case "linux/arm64":
		return "reploy-controller/linux-arm64"
	default:
		return ""
	}
}
...
func validateRelease(release ReleaseV1) error {
	...
	if release.BuildDirty != "" && release.BuildDirty != "true" && release.BuildDirty != "false" {
		return fmt.Errorf("runtime archive release build dirty must be empty, true, or false")
	}
	return nil
}
