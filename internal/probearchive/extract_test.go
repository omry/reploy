...
func TestExtractControllerWritesMatchingReleaseExecutable(t *testing.T) {
	dir := t.TempDir()
	executable := writeTestFile(t, dir, "reploy", []byte("prefix"), 0o755)
	controllers := testControllers(t, dir)
	if err := Append(executable, testRelease(), testHelpers(t, dir), controllers); err != nil {
		t.Fatal(err)
	}
	...
	result, err := ExtractController(context.Background(), executable, "linux/arm64", workspace)
	if err != nil {
		t.Fatal(err)
	}
	want, err := os.ReadFile(controllers[1].Path)
	if err != nil {
		t.Fatal(err)
	}
	...
	if string(got) != string(want) || result.Platform != "linux/arm64" || result.Path != filepath.Join(workspace, ExtractedControllerFileName) || result.Release != testRelease() {
		t.Fatalf("controller extraction = %#v; content = %q", result, got)
	}
}
