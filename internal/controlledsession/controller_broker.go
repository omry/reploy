...
const (
	DefaultControllerAttachTimeoutV1 = 10 * time.Second
	controllerRequestWriteTimeoutV1  = time.Second
)
...
type controllerBrokerPendingTerminalV1 struct {
	input  *RequestV1
	resize *RequestV1
}
...
func RunControllerBrokerV1(ctx context.Context, options ControllerBrokerOptionsV1) (resultErr error) {
	...
	defer cancelSetup()
	terminalReadCtx, cancelTerminalRead := context.WithCancel(ctx)
	defer cancelTerminalRead()
	requests := make(chan controllerBrokerRequestV1)
	inputErrors := make(chan controllerBrokerInputErrorV1, 2)
	var pendingTerminal controllerBrokerPendingTerminalV1
	...
	for session == nil || terminal == nil {
		...
		case terminalAccepted := <-terminalResult:
			...
			cancelAttach()
			go readControllerBrokerTerminalRequestsV1(terminalReadCtx, terminal, requests, inputErrors)
		...
		case inputErr := <-inputErrors:
			return failControllerBrokerV1(streamWriter, "terminal_attachment_error", inputErr.err)
		case request := <-requests:
			if err := pendingTerminal.add(request); err != nil {
				return failControllerBrokerV1(streamWriter, "terminal_attachment_error", err)
			}
		}
	}
	cancelSetup()
	...
	return runClaimedControllerBrokerV1(ctx, session, terminal, streamReader, streamWriter, requests, inputErrors, pendingTerminal, opened.Authorization.Operations, controllerRequestWriteTimeoutV1, terminalWriteTimeout)
}
...
func runClaimedControllerBrokerV1(
	...
	requests chan controllerBrokerRequestV1,
	inputErrors chan controllerBrokerInputErrorV1,
	pendingTerminal controllerBrokerPendingTerminalV1,
	...
	state := controllerBrokerStateV1{
		...
		requestWriteTimeout:  requestWriteTimeout,
		terminalWriteTimeout: terminalWriteTimeout,
	}
	for _, operation := range operations {
		state.operations[operation] = true
	}
	for _, request := range pendingTerminal.requests() {
...
func (pending *controllerBrokerPendingTerminalV1) add(input controllerBrokerRequestV1) error {
	if input.source != controllerBrokerRequestTerminalV1 {
		return fmt.Errorf("setup request did not originate from the terminal attachment")
	}
	switch input.request.Kind {
	case RequestInputV1:
		pendingLength := len(input.request.Bytes)
		if pending.input != nil {
			pendingLength += len(pending.input.Bytes)
		}
		if pendingLength > MaxFramePayloadV1 {
			return fmt.Errorf("terminal input buffered before host open exceeds %d bytes", MaxFramePayloadV1)
		}
		if pending.input == nil {
			pending.input = &RequestV1{Kind: RequestInputV1, Bytes: make([]byte, 0, pendingLength)}
		}
		pending.input.Bytes = append(pending.input.Bytes, input.request.Bytes...)
	case RequestResizeV1:
		request := input.request
		pending.resize = &request
	default:
		return fmt.Errorf("terminal attachment cannot carry %q", input.request.Kind)
	}
	return nil
}

func (pending controllerBrokerPendingTerminalV1) requests() []controllerBrokerRequestV1 {
	requests := make([]controllerBrokerRequestV1, 0, 2)
	if pending.resize != nil {
		requests = append(requests, controllerBrokerRequestV1{source: controllerBrokerRequestTerminalV1, request: *pending.resize})
	}
	if pending.input != nil {
		requests = append(requests, controllerBrokerRequestV1{
			source:  controllerBrokerRequestTerminalV1,
			request: RequestV1{Kind: RequestInputV1, Bytes: append([]byte{}, pending.input.Bytes...)},
		})
	}
	return requests
}
...
func (state *controllerBrokerStateV1) applyRequest(ctx context.Context, session controllerBrokerSessionV1, input controllerBrokerRequestV1) error {
	...
	if request.Kind == RequestAcknowledgeTerminatedV1 {
		...
		if err := state.writeHostRequest(ctx, session, request); err != nil {
			return err
		}
...
func (state *controllerBrokerStateV1) applyHostEvent(ctx context.Context, session controllerBrokerSessionV1, terminal *ControllerTerminalConnectionV1, writer *ControllerStreamWriterV1, event EventV1) error {
	...
	case EventReadyV1:
		...
		if state.pendingTerminalResize != nil {
			if err := state.writeHostRequest(ctx, session, *state.pendingTerminalResize); err != nil {
				return err
			}
		...
		if state.pendingTerminalInput != nil {
			if err := state.writeHostRequest(ctx, session, *state.pendingTerminalInput); err != nil {
				return err
			}
...
func (state *controllerBrokerStateV1) writeHostRequest(ctx context.Context, session controllerBrokerSessionV1, request RequestV1) error {
	timeout := state.requestWriteTimeout
	if timeout == 0 {
		timeout = controllerRequestWriteTimeoutV1
	}
	writeCtx, cancelWrite := context.WithTimeout(ctx, timeout)
	defer cancelWrite()
	return session.WriteRequest(writeCtx, request)
}
