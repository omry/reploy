...
func runClaimedControllerBrokerV1(
	...
	for {
		...
		case event := <-hostEvents:
			if err := state.applyHostEvent(ctx, terminal, streamWriter, event); err != nil {
...
func (state *controllerBrokerStateV1) applyHostEvent(ctx context.Context, terminal *ControllerTerminalConnectionV1, writer *ControllerStreamWriterV1, event EventV1) error {
	switch event.Kind {
	case EventReadyV1:
	...
	case EventOutputV1:
		if !state.ready || state.outputsFinalized || state.terminated {
			return fmt.Errorf("output event is out of order")
		}
		return terminal.WriteOutput(ctx, event.Bytes)
	case EventWorkloadExitV1:
		if !state.ready || state.workloadExited || state.outputsFinalized || state.terminated || event.WorkloadExit == nil {
	...
	case EventTerminatedV1:
		if state.terminated || event.Terminated == nil || !state.terminating {
		...
		if !state.terminalEnded {
			...
			if err := terminal.WriteEnd(ctx, WorkloadOutputFinalizationStatusV1{Kind: WorkloadOutputFinalizationDrainedV1}); err != nil {
				return err
			}
			state.terminalEnded = true
			state.outputsFinalized = true
