...
func RunControllerBrokerV1(ctx context.Context, options ControllerBrokerOptionsV1) (resultErr error) {
	...
	cancelSetup()
	return runClaimedControllerBrokerV1(ctx, session, terminal, streamReader, streamWriter, session.Opened().Authorization.Operations)
}
...
func runClaimedControllerBrokerV1(
	...
	go readControllerBrokerTerminalRequestsV1(runCtx, terminalReady, terminal, requests, inputErrors)
	go readControllerBrokerHostEventsV1(runCtx, session, hostEvents, hostErrors)
	state := controllerBrokerStateV1{operations: make(map[OperationV1]bool, len(operations))}
...
func readControllerBrokerTerminalRequestsV1(ctx context.Context, ready <-chan struct{}, terminal *ControllerTerminalConnectionV1, requests chan<- controllerBrokerRequestV1, failures chan<- controllerBrokerInputErrorV1) {
	for {
...
func (state *controllerBrokerStateV1) applyRequest(ctx context.Context, session controllerBrokerSessionV1, input controllerBrokerRequestV1) error {
	...
	if !state.ready {
		if input.source == controllerBrokerRequestTerminalV1 && request.Kind == RequestResizeV1 && !state.terminating && !state.terminated {
			if !state.operations[OperationResizeV1] {
				return fmt.Errorf("operation %q was not granted", OperationResizeV1)
			}
			pending := request
...
func (state *controllerBrokerStateV1) applyHostEvent(ctx context.Context, session controllerBrokerSessionV1, terminal *ControllerTerminalConnectionV1, writer *ControllerStreamWriterV1, event EventV1) error {
	...
	case EventReadyV1:
		...
		state.ready = true
		return writer.WriteEvent(ControllerStreamEventV1{Kind: ControllerStreamEventReadyV1})
	case EventOutputV1:
		if state.outputsFinalized || state.terminated {
	...
	case EventWorkloadOutputsFinalizedV1:
		...
		if err := terminal.WriteEnd(ctx, status); err != nil {
			return err
		}
	...
	case EventTerminatedV1:
		...
		if !state.terminalEnded {
			...
			if err := terminal.WriteEnd(ctx, event.Terminated.WorkloadOutputFinalizationStatus); err != nil {
				return err
			}
			state.terminalEnded = true
			state.outputsFinalized = true
