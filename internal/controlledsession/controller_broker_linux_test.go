...
func TestRunClaimedControllerBrokerV1ProcessesTerminateWhileTerminalOutputBlocks(t *testing.T) {
	...
	go func() {
		done <- runClaimedControllerBrokerV1(ctx, session, terminal, streamReader, streamWriter, []OperationV1{OperationTerminateV1}, 100*time.Millisecond, 100*time.Millisecond)
	}()
	public := bufio.NewReader(publicOutputReader)
