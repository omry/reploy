...
func TestRunControllerBrokerV1CompletesAcknowledgedSession(t *testing.T) {
	...
	case <-time.After(15 * time.Second):
		t.Fatal("broker did not exit after terminated acknowledgement")
	}
...
func TestRunControllerBrokerV1FailsWhenAttachmentIsLostBeforeHostOpened(t *testing.T) {
	hostListener, hostSocket := newControllerBrokerHostListenerV1(t)
	defer hostListener.Close()
	hostDone := make(chan struct{})
	go func() {
	...
	event := readControllerBrokerJSONLineV1(t, public)
	if event["type"] != string(ControllerStreamEventClientErrorV1) || event["code"] != "terminal_attachment_error" {
		t.Fatalf("pre-open attachment-loss event = %#v", event)
	}
	select {
	case err := <-brokerDone:
		if err == nil {
			t.Fatal("pre-open attachment loss returned nil")
		}
	case <-ctx.Done():
...
func TestRunControllerBrokerV1FailsWhenAttachmentIsLostAfterPreReadyInput(t *testing.T) {
	hostListener, hostSocket := newControllerBrokerHostListenerV1(t)
	defer hostListener.Close()
	hostDone := make(chan struct{})
	go func() {
	...
	if err := WriteTerminalRequestV1(terminal, RequestV1{Kind: RequestInputV1, Bytes: []byte("typed during startup")}); err != nil {
		t.Fatal(err)
	}
	if err := terminal.Close(); err != nil {
		t.Fatal(err)
	...
	case err := <-brokerDone:
		if err == nil {
			t.Fatal("pre-ready attachment loss returned nil")
		}
	case <-ctx.Done():
		t.Fatal("broker did not detect pre-ready attachment loss")
	}
	<-hostDone
}

func TestRunClaimedControllerBrokerV1ProcessesTerminateWhileTerminalOutputBlocks(t *testing.T) {
	brokerConnection, attachmentConnection := net.Pipe()
	defer brokerConnection.Close()
	defer attachmentConnection.Close()
	writeStarted := make(chan struct{})
	terminal := &ControllerTerminalConnectionV1{connection: &controllerBrokerSignalingWriteConnectionV1{Conn: brokerConnection, writeStarted: writeStarted}}
	publicInputReader, publicInputWriter := io.Pipe()
	defer publicInputWriter.Close()
	publicOutputReader, publicOutputWriter := io.Pipe()
	defer publicOutputReader.Close()
	streamReader, err := NewControllerStreamReaderV1(publicInputReader)
	if err != nil {
		t.Fatal(err)
	}
	streamWriter, err := NewControllerStreamWriterV1(publicOutputWriter)
	if err != nil {
		t.Fatal(err)
	}
	session := newControllerBrokerChannelSessionV1()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	done := make(chan error, 1)
	requests := make(chan controllerBrokerRequestV1)
	inputErrors := make(chan controllerBrokerInputErrorV1, 2)
	go readControllerBrokerTerminalRequestsV1(ctx, terminal, requests, inputErrors)
	go func() {
		done <- runClaimedControllerBrokerV1(ctx, session, terminal, streamReader, streamWriter, requests, inputErrors, controllerBrokerPendingTerminalV1{}, []OperationV1{OperationTerminateV1}, 100*time.Millisecond, 100*time.Millisecond)
	}()
	public := bufio.NewReader(publicOutputReader)
	session.events <- EventV1{Kind: EventReadyV1}
	if event := readControllerBrokerJSONLineV1(t, public); event["type"] != string(ControllerStreamEventReadyV1) {
		t.Fatalf("ready public event = %#v", event)
	}
	go func() { _, _ = io.Copy(io.Discard, public) }()
	session.events <- EventV1{Kind: EventOutputV1, Bytes: []byte{}}
	select {
	case <-writeStarted:
	case <-ctx.Done():
		t.Fatal(ctx.Err())
	}
	writeControllerBrokerPublicRequestV1(t, publicInputWriter, ControllerStreamRequestTerminateV1, 0, 0)
	select {
	case request := <-session.requests:
		if request.Kind != RequestTerminateV1 {
			t.Fatalf("request while output blocked = %#v", request)
		}
	case <-ctx.Done():
		t.Fatal("terminate was not forwarded while terminal output was blocked")
	}
	select {
	case err := <-done:
		if err == nil {
			t.Fatalf("blocked terminal output error = %v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("broker did not time out blocked terminal output")
	}
}
...
func TestControllerBrokerStateV1HoldsTerminalInputUntilReady(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	session := &controllerBrokerFakeSessionV1{}
	state := controllerBrokerStateV1{operations: map[OperationV1]bool{OperationInputV1: true}}
	input := controllerBrokerRequestV1{
		source:  controllerBrokerRequestTerminalV1,
		request: RequestV1{Kind: RequestInputV1, Bytes: []byte("typed during startup")},
	}
	if err := state.applyRequest(ctx, session, input); err != nil {
		t.Fatal(err)
	}
	if requests := session.requestKinds(); len(requests) != 0 {
		t.Fatalf("input forwarded before ready: %#v", requests)
	}
	var publicOutput bytes.Buffer
	writer, err := NewControllerStreamWriterV1(&publicOutput)
	if err != nil {
	...
	requests := append([]RequestV1(nil), session.requests...)
	session.mu.Unlock()
	if len(requests) != 1 || requests[0].Kind != RequestInputV1 || string(requests[0].Bytes) != "typed during startup" {
		t.Fatalf("forwarded input = %#v", requests)
	}
	if state.pendingTerminalInput != nil || !state.ready || !strings.Contains(publicOutput.String(), `"type":"ready"`) {
		t.Fatalf("ready state = %#v, public output = %q", state, publicOutput.String())
	}
}

func TestControllerBrokerPendingTerminalV1PreservesEmptyInput(t *testing.T) {
	var pending controllerBrokerPendingTerminalV1
	if err := pending.add(controllerBrokerRequestV1{
		source:  controllerBrokerRequestTerminalV1,
	...
	if len(requests) != 1 || requests[0].request.Kind != RequestInputV1 || requests[0].request.Bytes == nil || len(requests[0].request.Bytes) != 0 {
		t.Fatalf("pending empty input = %#v", requests)
	}
}

func TestControllerBrokerStateV1BoundsHostRequestWrites(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	state := controllerBrokerStateV1{
		operations:          map[OperationV1]bool{OperationInputV1: true},
		requestWriteTimeout: 20 * time.Millisecond,
		ready:               true,
	}
	input := controllerBrokerRequestV1{
		source:  controllerBrokerRequestTerminalV1,
		request: RequestV1{Kind: RequestInputV1, Bytes: []byte("blocked")},
	}
	started := time.Now()
	err := state.applyRequest(ctx, &controllerBrokerBlockingSessionV1{}, input)
	if !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("blocked host request error = %v", err)
	}
	if elapsed := time.Since(started); elapsed >= 200*time.Millisecond {
		t.Fatalf("blocked host request exceeded bound: %s", elapsed)
	}
}

func TestControllerBrokerStateV1IgnoresPreReadyResizeAfterTerminationBegins(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	input := controllerBrokerRequestV1{
		source:  controllerBrokerRequestTerminalV1,
		request: RequestV1{Kind: RequestResizeV1, Columns: 120, Rows: 40},
	}
	for _, state := range []*controllerBrokerStateV1{
		{operations: map[OperationV1]bool{OperationResizeV1: true}, terminating: true},
		{operations: map[OperationV1]bool{OperationResizeV1: true}, terminated: true},
	} {
		session := &controllerBrokerFakeSessionV1{}
		if err := state.applyRequest(ctx, session, input); err != nil {
			t.Fatal(err)
		}
		if state.pendingTerminalResize != nil || len(session.requestKinds()) != 0 {
			t.Fatalf("late resize changed broker state: %#v", state)
		}
	}
}

func TestControllerBrokerStateV1IgnoresTerminalRequestsAfterTerminationBegins(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	requests := []RequestV1{
	...
	for _, terminated := range []bool{false, true} {
		state := controllerBrokerStateV1{
			operations:  map[OperationV1]bool{OperationInputV1: true, OperationResizeV1: true},
			ready:       true,
			terminating: true,
		...
		for _, request := range requests {
			input := controllerBrokerRequestV1{source: controllerBrokerRequestTerminalV1, request: request}
			if err := state.applyRequest(ctx, session, input); err != nil {
				t.Fatalf("terminated=%t request=%s: %v", terminated, request.Kind, err)
			}
		}
		if got := session.requestKinds(); len(got) != 0 {
			t.Fatalf("terminated=%t forwarded requests = %#v", terminated, got)
		}
	}
}

func TestControllerBrokerStateV1RejectsPublicResizeWhileTerminating(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	state := controllerBrokerStateV1{
		operations:  map[OperationV1]bool{OperationResizeV1: true},
		ready:       true,
		terminating: true,
	}
	input := controllerBrokerRequestV1{
	...
	if err := state.applyRequest(ctx, &controllerBrokerFakeSessionV1{}, input); err == nil || !strings.Contains(err.Error(), "while terminating") {
		t.Fatalf("public resize error = %v", err)
	}
}
...
func TestControllerBrokerStateV1PreservesPreActivationOutputFinalizationFailure(t *testing.T) {
	...
	state := controllerBrokerStateV1{terminalWriteTimeout: time.Second}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	done := make(chan error, 1)
	go func() {
...
func TestControllerBrokerStateV1BoundsTerminalEndWrites(t *testing.T) {
	result := ResultV1{
		Cause:                            CauseStartupFailureV1,
		WorkloadStatus:                   ProcessStatusV1{Kind: ProcessStatusUnknownV1},
		WorkloadOutputFinalizationStatus: WorkloadOutputFinalizationStatusV1{Kind: WorkloadOutputFinalizationFailedV1, Reason: "startup output unavailable"},
	...
	tests := []struct {
		...
		{
			name: "outputs finalized",
			state: controllerBrokerStateV1{
				ready:                true,
				workloadExited:       true,
				terminating:          true,
				terminalWriteTimeout: 20 * time.Millisecond,
			},
			event: EventV1{
				Kind: EventWorkloadOutputsFinalizedV1,
				WorkloadOutputsFinalized: &WorkloadOutputsFinalizedV1{
					Status: WorkloadOutputFinalizationDrainedV1,
				},
			},
		},
		{
			name: "pre-activation terminated",
			state: controllerBrokerStateV1{
				terminalWriteTimeout: 20 * time.Millisecond,
			},
			event: EventV1{Kind: EventTerminatedV1, Terminated: &result},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			started := time.Now()
			err = test.state.applyHostEvent(context.Background(), nil, terminal, writer, test.event)
			if !errors.Is(err, context.DeadlineExceeded) {
				t.Fatalf("blocked terminal-end write error = %v", err)
			}
			if elapsed := time.Since(started); elapsed > time.Second {
				t.Fatalf("blocked terminal-end write took %s", elapsed)
			}
		})
	}
}
...
type controllerBrokerBlockingSessionV1 struct{}

func (*controllerBrokerBlockingSessionV1) Opened() OpenedV1 { return testOpenedV1() }
func (*controllerBrokerBlockingSessionV1) ReadEvent(context.Context) (EventV1, error) {
	return EventV1{}, io.EOF
}
func (*controllerBrokerBlockingSessionV1) WriteRequest(ctx context.Context, _ RequestV1) error {
	<-ctx.Done()
	return ctx.Err()
}
func (*controllerBrokerBlockingSessionV1) Close() error { return nil }

type controllerBrokerChannelSessionV1 struct {
	events   chan EventV1
	requests chan RequestV1
}

func newControllerBrokerChannelSessionV1() *controllerBrokerChannelSessionV1 {
	return &controllerBrokerChannelSessionV1{events: make(chan EventV1), requests: make(chan RequestV1, 1)}
}

func (*controllerBrokerChannelSessionV1) Opened() OpenedV1 { return testOpenedV1() }
func (session *controllerBrokerChannelSessionV1) ReadEvent(ctx context.Context) (EventV1, error) {
	select {
	case event := <-session.events:
		return event, nil
	case <-ctx.Done():
		return EventV1{}, ctx.Err()
	}
}
func (session *controllerBrokerChannelSessionV1) WriteRequest(ctx context.Context, request RequestV1) error {
	select {
...
func (*controllerBrokerChannelSessionV1) Close() error { return nil }

type controllerBrokerSignalingWriteConnectionV1 struct {
	net.Conn
	writeStarted chan struct{}
	once         sync.Once
}

func (connection *controllerBrokerSignalingWriteConnectionV1) Write(content []byte) (int, error) {
	connection.once.Do(func() { close(connection.writeStarted) })
	return connection.Conn.Write(content)
}
...
func runControllerBrokerTestHostV1(listener *net.UnixListener, done chan<- error) {
	done <- runControllerBrokerTestHostConnectionV1(listener)
}

func runControllerBrokerTestHostConnectionV1(listener *net.UnixListener) (resultErr error) {
	...
	if err != nil || request.Kind != RequestCompleteV1 {
		return errors.Join(err, errors.New("host expected complete request"))
	}
	...
	if err != nil || request.Kind != RequestAcknowledgeTerminatedV1 {
		return errors.Join(err, errors.New("host expected acknowledge-terminated request"))
	}
	return nil
}
