...
func preparePrivateChannelTransportV1(directory string, uid uint32, gid uint32) (result privateChannelTransportV1, resultErr error) {
	...
	maxSocketPathBytes := len(unix.RawSockaddrUnix{}.Path) - 1
	if len(socket) > maxSocketPathBytes {
		return nil, fmt.Errorf(
			"controlled-session Unix socket path is %d bytes, exceeding the Linux AF_UNIX maximum of %d bytes; use a shorter deployment path",
			len(socket), maxSocketPathBytes,
		)
	}
