	...
	for {
		...
		if errors.Is(readErr, io.EOF) {
			if _, closeErr := pump.closeSource(); closeErr != nil {
				closeFailure := fmt.Errorf("close drained controlled-session PTY output: %w", closeErr)
				pump.latchResult(PTYOutputFinalizationV1{
					Status: failedPTYOutputStatusV1(ptyOutputClosureFailureReasonV1),
					Err:    closeFailure,
				})
				pump.appendResultErrorIfMissing(closeErr, closeFailure)
			} else {
				pump.latchResult(PTYOutputFinalizationV1{
...
func (pump *PTYOutputPumpV1) finalize(deadline time.Time) PTYOutputFinalizationV1 {
	...
	if timedOut {
		pump.cancel()
		if _, closeErr := pump.closeSource(); closeErr != nil {
			pump.appendResultErrorIfMissing(closeErr, fmt.Errorf("close timed-out controlled-session PTY output: %w", closeErr))
		}
	}
	<-pump.done
	return pump.resultForDeadline(deadline)
...
func (pump *PTYOutputPumpV1) failAndClose(reason string, failure error) {
	...
	pump.cancel()
	if _, closeErr := pump.closeSource(); closeErr != nil {
		pump.appendResultErrorIfMissing(closeErr, fmt.Errorf("close failed controlled-session PTY output: %w", closeErr))
	}
}
...
func (pump *PTYOutputPumpV1) appendResultErrorIfMissing(cause error, detail error) {
	if cause == nil || detail == nil {
		return
	}
	pump.resultMu.Lock()
	defer pump.resultMu.Unlock()
	...
	if !errors.Is(pump.result.Err, cause) {
		pump.result.Err = errors.Join(pump.result.Err, detail)
	}
}
