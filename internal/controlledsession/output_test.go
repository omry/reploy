...
func TestPTYOutputPumpV1PreservesConcurrentSourceClosureFailure(t *testing.T) {
	closeErr := errors.New("close failed")
	closeEntered := make(chan struct{})
	allowClose := make(chan struct{})
	source := &blockingCloseReaderV1{
		ReadCloser: &closeErrorReaderV1{
			ReadCloser: &singleReadCloserV1{err: io.EOF},
			err:        closeErr,
		},
		entered: closeEntered,
		allow:   allowClose,
	...
	resultReady := make(chan finalizationResult, 1)
	go func() {
		result, finalizeErr := pump.Finalize(time.Now().Add(-time.Second))
		resultReady <- finalizationResult{result: result, err: finalizeErr}
	}()
	awaitV1(t, time.Second, func() bool {
		pump.resultMu.Lock()
		defer pump.resultMu.Unlock()
		return pump.result != nil && pump.result.Status == failedPTYOutputStatusV1(ptyOutputTimeoutReasonV1)
	})
	close(allowClose)
	finalized := <-resultReady
	if finalized.err != nil {
		t.Fatalf("Finalize() error = %v", finalized.err)
	}
	if finalized.result.Status != failedPTYOutputStatusV1(ptyOutputTimeoutReasonV1) ||
		!errors.Is(finalized.result.Err, context.DeadlineExceeded) ||
		!errors.Is(finalized.result.Err, closeErr) {
		t.Fatalf("Finalize() = %#v", finalized.result)
	}
}
