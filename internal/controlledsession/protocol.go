...
const (
	...
	RequestOpenEndpointV1          RequestKindV1 = "open-endpoint"
...
const (
	...
	EventEndpointOpenedV1     EventKindV1 = "endpoint-opened"
	EventEndpointOpenFailedV1 EventKindV1 = "endpoint-open-failed"
	EventEndpointClosedV1     EventKindV1 = "endpoint-closed"
...
type EndpointOpenedV1 struct {
	RequestID  uint64 `json:"request_id"`
	EndpointID string `json:"endpoint_id"`
	StreamID   uint64 `json:"stream_id"`
}

type EndpointOpenFailedV1 struct {
	RequestID  uint64 `json:"request_id"`
	EndpointID string `json:"endpoint_id"`
	Code       string `json:"code"`
	Message    string `json:"message"`
...
type EventV1 struct {
	...
	EndpointOpened     *EndpointOpenedV1
	EndpointOpenFailed *EndpointOpenFailedV1
	EndpointClosed     *EndpointClosedV1
	Terminated         *ResultV1
}
...
const (
	wireRequestInputV1 wireKindV1 = iota + 1
	wireRequestResizeV1
	wireRequestTerminateV1
	wireRequestCompleteV1
	wireRequestOpenEndpointV1
	wireRequestAcknowledgeTerminatedV1
)

const (
	wireEventOpenedV1 wireKindV1 = 0x81 + iota
	wireEventOutputV1
	wireEventWorkloadExitV1
	wireEventTerminatingV1
	wireEventDiagnosticV1
	wireEventEndpointClosedV1
	wireEventTerminatedV1
	wireEventEndpointOpenedV1
	wireEventEndpointOpenFailedV1
)

func ValidateRequestV1(request RequestV1) error {
	switch request.Kind {
	case RequestInputV1:
		if request.Bytes == nil || request.Columns != 0 || request.Rows != 0 || request.RequestID != 0 || request.EndpointID != "" {
			return fmt.Errorf("controlled-session input request must contain only a byte sequence")
		}
	case RequestResizeV1:
		if request.Bytes != nil || request.RequestID != 0 || request.EndpointID != "" || !validDimensionsV1(request.Columns, request.Rows) {
	...
	case RequestOpenEndpointV1:
		if request.Bytes != nil || request.Columns != 0 || request.Rows != 0 || request.RequestID == 0 || !endpointIDPatternV1.MatchString(request.EndpointID) {
			return fmt.Errorf("controlled-session open-endpoint request requires a nonzero request ID and valid endpoint ID")
		}
	default:
...
func ValidateEventV1(event EventV1) error {
	...
	case EventDiagnosticV1:
		...
		if !endpointIDPatternV1.MatchString(event.Diagnostic.Code) {
			return fmt.Errorf("controlled-session diagnostic code %q is invalid", event.Diagnostic.Code)
		}
		if err := validateRequiredSafeTextV1("diagnostic message", event.Diagnostic.Message); err != nil {
			return err
		}
	case EventEndpointOpenedV1:
		if event.EndpointOpened == nil || eventPayloadCountV1(event) != 1 {
			return fmt.Errorf("controlled-session endpoint-opened event requires exactly one endpoint payload")
		}
		if event.EndpointOpened.RequestID == 0 || event.EndpointOpened.StreamID == 0 || !endpointIDPatternV1.MatchString(event.EndpointOpened.EndpointID) {
			return fmt.Errorf("controlled-session endpoint-opened event has invalid request, endpoint, or stream identity")
		}
	case EventEndpointOpenFailedV1:
		if event.EndpointOpenFailed == nil || eventPayloadCountV1(event) != 1 {
			return fmt.Errorf("controlled-session endpoint-open-failed event requires exactly one endpoint payload")
		}
		if event.EndpointOpenFailed.RequestID == 0 || !endpointIDPatternV1.MatchString(event.EndpointOpenFailed.EndpointID) || !endpointIDPatternV1.MatchString(event.EndpointOpenFailed.Code) {
			return fmt.Errorf("controlled-session endpoint-open-failed event has invalid request, endpoint, or error identity")
		}
		if err := validateRequiredSafeTextV1("endpoint-open-failed message", event.EndpointOpenFailed.Message); err != nil {
			return err
		}
	case EventEndpointClosedV1:
		if event.EndpointClosed == nil || eventPayloadCountV1(event) != 1 {
			return fmt.Errorf("controlled-session endpoint-closed event requires exactly one endpoint payload")
		}
		if event.EndpointClosed.StreamID == 0 || !endpointIDPatternV1.MatchString(event.EndpointClosed.EndpointID) {
			return fmt.Errorf("controlled-session endpoint-closed event has an invalid endpoint or stream ID")
		}
		if err := validateRequiredSafeTextV1("endpoint-closed reason", event.EndpointClosed.Reason); err != nil {
			return err
		}
...
func encodeRequestPayloadV1(request RequestV1) (wireKindV1, []byte) {
	...
	case RequestOpenEndpointV1:
		payload := make([]byte, 8+len(request.EndpointID))
		binary.BigEndian.PutUint64(payload[:8], request.RequestID)
		copy(payload[8:], request.EndpointID)
		return wireRequestOpenEndpointV1, payload
...
func decodeRequestPayloadV1(kind wireKindV1, payload []byte) (RequestV1, error) {
	...
	case wireRequestOpenEndpointV1:
		if len(payload) <= 8 {
			return RequestV1{}, fmt.Errorf("controlled-session open-endpoint frame must contain a request ID and endpoint ID")
		}
		return RequestV1{Kind: RequestOpenEndpointV1, RequestID: binary.BigEndian.Uint64(payload[:8]), EndpointID: string(payload[8:])}, nil
...
func encodeEventPayloadV1(event EventV1) (wireKindV1, []byte, error) {
	...
	case EventEndpointOpenedV1:
		return marshalEventPayloadV1(wireEventEndpointOpenedV1, event.EndpointOpened)
	case EventEndpointOpenFailedV1:
		return marshalEventPayloadV1(wireEventEndpointOpenFailedV1, event.EndpointOpenFailed)
	case EventEndpointClosedV1:
		return marshalEventPayloadV1(wireEventEndpointClosedV1, event.EndpointClosed)
	case EventTerminatedV1:
...
func decodeEventPayloadV1(kind wireKindV1, payload []byte) (EventV1, error) {
	...
	case wireEventEndpointOpenedV1:
		value := new(EndpointOpenedV1)
		return EventV1{Kind: EventEndpointOpenedV1, EndpointOpened: value}, decodeStrictJSONV1("endpoint-opened event", payload, value)
	case wireEventEndpointOpenFailedV1:
		value := new(EndpointOpenFailedV1)
		return EventV1{Kind: EventEndpointOpenFailedV1, EndpointOpenFailed: value}, decodeStrictJSONV1("endpoint-open-failed event", payload, value)
	case wireEventEndpointClosedV1:
		value := new(EndpointClosedV1)
		return EventV1{Kind: EventEndpointClosedV1, EndpointClosed: value}, decodeStrictJSONV1("endpoint-closed event", payload, value)
...
func eventPayloadCountV1(event EventV1) int {
	...
	for _, present := range []bool{
		...
		event.EndpointOpened != nil,
		event.EndpointOpenFailed != nil,
		event.EndpointClosed != nil,
...
func operationForRequestV1(kind RequestKindV1) OperationV1 {
	...
	case RequestOpenEndpointV1:
		return OperationOpenEndpointV1
	default:
		return ""
	}
}
