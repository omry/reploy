...
const (
	...
	EventApplicationExitV1 EventKindV1 = "application-exit"
...
type EventV1 struct {
	...
	Bytes           []byte
	Opened          *OpenedV1
	ApplicationExit *ApplicationExitV1
...
func ValidateRequestV1(request RequestV1) error {
	switch request.Kind {
	case RequestInputV1:
		if request.Bytes == nil || request.Columns != 0 || request.Rows != 0 || request.EndpointID != "" {
			return fmt.Errorf("controlled-session input request must contain only a byte sequence")
		}
	case RequestResizeV1:
		if request.Bytes != nil || request.EndpointID != "" || !validDimensionsV1(request.Columns, request.Rows) {
			return fmt.Errorf("controlled-session resize request requires dimensions between 1 and 65535")
		}
	case RequestTerminateV1, RequestCompleteV1:
		if request.Bytes != nil || request.Columns != 0 || request.Rows != 0 || request.EndpointID != "" {
			return fmt.Errorf("controlled-session %s request must not contain a payload", request.Kind)
		}
	case RequestOpenEndpointV1:
		if request.Bytes != nil || request.Columns != 0 || request.Rows != 0 || !endpointIDPatternV1.MatchString(request.EndpointID) {
			return fmt.Errorf("controlled-session open-endpoint request has an invalid endpoint ID")
		}
	default:
...
func ValidateEventV1(event EventV1) error {
	switch event.Kind {
	case EventOpenedV1:
		if event.Bytes != nil || event.Opened == nil || event.ApplicationExit != nil || event.Terminating != nil || event.Diagnostic != nil || event.EndpointClosed != nil || event.Terminated != nil {
	...
	case EventApplicationExitV1:
		if event.Bytes != nil || event.Opened != nil || event.ApplicationExit == nil || event.Terminating != nil || event.Diagnostic != nil || event.EndpointClosed != nil || event.Terminated != nil {
			return fmt.Errorf("controlled-session application-exit event requires exactly one status payload")
		}
		if err := validateProcessStatusV1(event.ApplicationExit.Status, false); err != nil {
			return fmt.Errorf("controlled-session application-exit event: %w", err)
		}
	...
	case EventEndpointClosedV1:
		...
		if !endpointIDPatternV1.MatchString(event.EndpointClosed.EndpointID) {
			return fmt.Errorf("controlled-session endpoint-closed event has an invalid endpoint ID")
		}
...
func encodeRequestPayloadV1(request RequestV1) (wireKindV1, []byte) {
	...
	case RequestOpenEndpointV1:
		return wireRequestOpenEndpointV1, []byte(request.EndpointID)
	default:
...
func encodeEventPayloadV1(event EventV1) (wireKindV1, []byte, error) {
	...
	case EventApplicationExitV1:
		return marshalEventPayloadV1(wireEventApplicationExitV1, event.ApplicationExit)
	case EventTerminatingV1:
...
func decodeEventPayloadV1(kind wireKindV1, payload []byte) (EventV1, error) {
	...
	case wireEventApplicationExitV1:
		value := new(ApplicationExitV1)
		return EventV1{Kind: EventApplicationExitV1, ApplicationExit: value}, decodeStrictJSONV1("application-exit event", payload, value)
