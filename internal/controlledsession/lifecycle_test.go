...
func TestLifecycleConnectionClosureCannotBecomeSuccess(t *testing.T) {
	...
	if transition.Result.ControllerFinalizationStatus.Kind != ControllerFinalizationLostV1 {
...
func TestLifecycleEnforcesAuthorizationAndIdempotentTermination(t *testing.T) {
	...
	if _, err := machine.ApplyRequest(RequestV1{Kind: RequestOpenEndpointV1, RequestID: 1, EndpointID: "terminal"}); !errors.Is(err, ErrRequestRejected) {
...
func TestLifecycleFinalizationExpiryCannotBeRewrittenByLateComplete(t *testing.T) {
	...
	if snapshot.Cause != CauseWorkloadExitV1 || snapshot.ControllerFinalizationStatus.Kind != ControllerFinalizationTimeoutV1 {
