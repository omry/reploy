...
func TestLifecycleApplicationExitAllowsOnlyBoundedControllerFinalization(t *testing.T) {
	machine := activatedMachineV1(t)
	code := 0
	transition, err := machine.Observe(ObservationV1{
		Kind:              ObservationApplicationExitV1,
		ApplicationStatus: &ProcessStatusV1{Kind: ProcessStatusExitedV1, Code: &code},
	})
	if err != nil {
		t.Fatalf("Observe(application exit) error = %v", err)
	}
	if !transition.CauseLatched || !transition.BeginTermination || !transition.AwaitingControllerFinalization || transition.Cause != CauseApplicationExitV1 {
		t.Fatalf("application-exit transition = %#v", transition)
	}
	...
	if transition.Cause != CauseApplicationExitV1 || transition.AwaitingControllerFinalization {
	...
	if transition.After != StateTerminatedV1 || transition.Result == nil || transition.Result.Cause != CauseApplicationExitV1 {
...
func TestLifecycleConnectionClosureCannotBecomeSuccess(t *testing.T) {
	...
	_, err = machine.Observe(ObservationV1{Kind: ObservationFinishedV1, Finish: &FinishV1{
		ApplicationStatus: ProcessStatusV1{Kind: ProcessStatusTerminatedV1, Reason: "lease teardown"},
		ControllerStatus:  ControllerStatusV1{Kind: ControllerStatusCompletedV1},
		CleanupStatus:     CleanupStatusV1{Kind: CleanupStatusSucceededV1}, RecoveryAction: RecoveryNoneV1,
	}})
...
func TestLifecycleFirstAcceptedCauseWinsConcurrentRace(t *testing.T) {
	...
	observations := []ObservationV1{
		...
		{Kind: ObservationApplicationExitV1, ApplicationStatus: &ProcessStatusV1{Kind: ProcessStatusExitedV1, Code: &code}},
	}
...
func TestLifecycleFinalizationExpiryCannotBeRewrittenByLateComplete(t *testing.T) {
	machine := activatedMachineV1(t)
	code := 1
	if _, err := machine.Observe(ObservationV1{Kind: ObservationApplicationExitV1, ApplicationStatus: &ProcessStatusV1{Kind: ProcessStatusExitedV1, Code: &code}}); err != nil {
		t.Fatal(err)
	}
	...
	if snapshot.Cause != CauseApplicationExitV1 || snapshot.ControllerStatus.Kind != ControllerStatusFinalizationTimeoutV1 {
