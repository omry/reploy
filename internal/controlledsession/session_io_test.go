...
func TestSessionIOBridgeV1CancelsActiveRequestAndContinuesDispatch(t *testing.T) {
	...
	bridge, err := StartSessionIOBridgeV1(transport, io.NopCloser(bytes.NewReader(nil)), func(ctx context.Context, request RequestV1) error {
		if request.Kind == RequestInputV1 {
			close(inputStarted)
			<-ctx.Done()
			return errors.New("Docker resize connection closed after cancellation")
		}
		continued <- request
		return nil
	})
...
func TestSessionIOBridgeV1DoesNotReuseTransportAfterWriteFailure(t *testing.T) {
	transport := newPriorityBridgeTestTransportV1()
	transport.firstWriteErr = context.DeadlineExceeded
	reader, writer := io.Pipe()
	bridge, err := StartSessionIOBridgeV1(transport, reader, func(context.Context, RequestV1) error { return nil })
	if err != nil {
		t.Fatal(err)
	...
	writeErr := make(chan error, 1)
	go func() {
		_, err := writer.Write([]byte("partial output frame"))
		writeErr <- errors.Join(err, writer.Close())
	}()
	select {
	case <-transport.firstWriteEntered:
	...
	lifecycleErr := bridge.SendLifecycleEvent(t.Context(), EventV1{
		Kind: EventDiagnosticV1, Diagnostic: &DiagnosticV1{Code: "test", Message: "must not be written"},
	})
	if lifecycleErr == nil || !strings.Contains(lifecycleErr.Error(), "transport is unusable") {
		t.Fatalf("SendLifecycleEvent() = %v", lifecycleErr)
	}
	transport.mu.Lock()
	writes := transport.writeCalls
	transport.mu.Unlock()
	if writes != 1 {
		t.Fatalf("transport event writes after terminal failure = %d, want 1", writes)
	}
	bridge.StopRequests()
}
...
func (transport *priorityBridgeTestTransportV1) WriteEvent(ctx context.Context, event EventV1) error {
	...
	if first {
		...
		if transport.firstWriteErr != nil {
			return transport.firstWriteErr
		}
	}
	clone := event
