...
// one event write at a time and make both operations cancelable.
...
func (gate *sessionEventWriteGateV1) write(ctx context.Context, event EventV1, lifecycle bool) error {
	...
	defer gate.release()
	return gate.transport.WriteEvent(ctx, event)
}
...
func (gate *sessionEventWriteGateV1) acquire(ctx context.Context, lifecycle bool) error {
	...
	for gate.writing || !lifecycle && gate.lifecycleWaiting > 0 {
...
func (gate *sessionEventWriteGateV1) release() {
	gate.mu.Lock()
	gate.writing = false
	gate.notifyLocked()
	gate.mu.Unlock()
