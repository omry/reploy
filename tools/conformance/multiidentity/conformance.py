...
def query_subids(runner: Runner, username: str, *, gids: bool) -> list[tuple[int, int]]:
    ...
    for line in runner.run(command).stdout.splitlines():
        fields = line.split()
        if len(fields) != 4 or not fields[0].endswith(":") or fields[1] != username:
...
def assert_host_authority(
    ...
    authoritative_uids = query_subids(runner, username, gids=False)
    authoritative_gids = query_subids(runner, username, gids=True)
    ...
    return {**positive, "negative_collision_cases": negative_cases}
