...
def require_enumerable_subid_backend() -> list[str]:
    nsswitch = pathlib.Path("/etc/nsswitch.conf")
    if not nsswitch.is_file():
        raise ConformanceError("cannot establish the subordinate-ID backend without /etc/nsswitch.conf")
    for raw_line in nsswitch.read_text(encoding="utf-8").splitlines():
        line = raw_line.split("#", 1)[0].strip()
        if not line:
            continue
        database, separator, value = line.partition(":")
        if separator and database.strip() == "subid":
            sources = value.split()
            if sources != ["files"]:
                raise ConformanceError(
                    "cannot enumerate every subordinate-ID delegation from "
                    f"the configured subid sources: {sources}"
                )
            return sources
    # libsubid defaults to the files backend when nsswitch.conf has no subid entry.
    return ["files"]
...
def host_context(runner: Runner) -> dict[str, Any]:
    ...
    subid_sources = require_enumerable_subid_backend()
...
def build_image(runner: Runner, output: pathlib.Path, tag: str) -> dict[str, Any]:
    ...
    staging.mkdir()
    probe = staging / "probe"
    posix_ipc_probe = staging / "posix-ipc-probe"
    ...
    runner.run(
        [
            "cc", "-O2", "-static", "-pthread",
            "-o", posix_ipc_probe, HERE / "posix_ipc.c",
        ]
    )
...
def common_run_args(
    ...
    *,
    ...
    seccomp_override: str | None = None,
) -> list[str]:
    args = [
        ...
        *mapping_args(profile, slot),
        ...
        "--security-opt", f"seccomp={seccomp_override or seccomp_path}",
        "--user", "0:0",
    ]
    if capabilities:
...
def owned_subid_ranges(
    entries: list[tuple[str, int, int]],
    caller_names: set[str],
) -> list[tuple[int, int]]:
    return [
        (start, count)
        for name, start, count in entries
...
def query_subids(
    ...
    *,
    ...
    executable = shutil.which("getsubids")
    if executable is None:
        raise ConformanceError("getsubids is required to verify the active libsubid delegation")
    command = [executable]
    if gids:
        command.append("-g")
    command.append(username)
    ranges: list[tuple[int, int]] = []
    for line in runner.run(command).stdout.splitlines():
        fields = line.split()
        if (
            len(fields) != 4
            or not fields[0].endswith(":")
            or fields[1] not in caller_names
        ):
            raise ConformanceError(f"unexpected getsubids output: {line!r}")
        ranges.append((int(fields[2]), int(fields[3])))
    if not ranges:
        kind = "GID" if gids else "UID"
        raise ConformanceError(f"getsubids reported no subordinate {kind} ranges for {username}")
    return ranges


def resolved_principals(values: set[int], *, gids: bool) -> dict[int, str]:
    collisions: dict[int, str] = {}
    lookup = grp.getgrgid if gids else pwd.getpwuid
    for value in values:
        try:
            entry = lookup(value)
        except KeyError:
            continue
        collisions[value] = entry.gr_name if gids else entry.pw_name
    return collisions
...
def validate_host_authority(
    ...
    *,
    caller_names: set[str],
    uid_ranges: list[tuple[str, int, int]],
    gid_ranges: list[tuple[str, int, int]],
    own_uids: list[tuple[int, int]],
    own_gids: list[tuple[int, int]],
    resolve_uids: Callable[[set[int]], dict[int, str]],
    resolve_gids: Callable[[set[int]], dict[int, str]],
) -> dict[str, Any]:
    ...
    for values in uid_sets:
        ...
        collisions = resolve_uids(values)
        if collisions:
    ...
    for values in gid_sets:
        ...
        collisions = resolve_gids(values)
        if collisions:
    ...
    for name, start, count in uid_ranges:
        if name not in caller_names and any(start <= value < start + count for value in uid_sets[0] | uid_sets[1]):
            raise ConformanceError(f"UID mapping overlaps delegation for {name}")
    for name, start, count in gid_ranges:
        if name not in caller_names and any(start <= value < start + count for value in gid_sets[0] | gid_sets[1]):
...
def require_authority_rejection(
    ...
    try:
        ...
        if expected not in message:
            raise ConformanceError(
                f"authority negative case {name} failed for the wrong reason: {message}"
            ) from exc
        return {"pass": True, "expected": expected, "actual": message}
    raise ConformanceError(f"authority negative case {name} did not fail closed")
...
def assert_host_authority(
    ...
    require_enumerable_subid_backend()
    username = getpass.getuser()
    caller_names = {username, str(os.getuid())}
    uid_ranges = parse_subids(pathlib.Path("/etc/subuid"))
    gid_ranges = parse_subids(pathlib.Path("/etc/subgid"))
    own_uids = owned_subid_ranges(uid_ranges, caller_names)
    own_gids = owned_subid_ranges(gid_ranges, caller_names)
    if not own_uids or not own_gids:
        raise ConformanceError(
            f"no subordinate ranges for caller aliases {sorted(caller_names)}"
        )
    authoritative_uids = query_subids(
        runner, username, caller_names, gids=False
    )
    authoritative_gids = query_subids(
        runner, username, caller_names, gids=True
    )
    if sorted(own_uids) != sorted(authoritative_uids):
        raise ConformanceError(
            f"/etc/subuid disagrees with libsubid for {username}: "
            f"{own_uids} != {authoritative_uids}"
        )
    if sorted(own_gids) != sorted(authoritative_gids):
        raise ConformanceError(
            f"/etc/subgid disagrees with libsubid for {username}: "
            f"{own_gids} != {authoritative_gids}"
        )

    def resolve_uids(values: set[int]) -> dict[int, str]:
        return resolved_principals(values, gids=False)

    def resolve_gids(values: set[int]) -> dict[int, str]:
        return resolved_principals(values, gids=True)
    ...
    positive = validate_host_authority(
        uid_maps,
        gid_maps,
        **authority_args,
        resolve_uids=resolve_uids,
        resolve_gids=resolve_gids,
    )

    collision_uid = min(mapped_host_ids(uid_maps[0]))
    collision_gid = min(mapped_host_ids(gid_maps[0]))
    negative_cases = {
        "uid_nss_collision": require_authority_rejection(
            "uid_nss_collision",
            "UID mapping aliases NSS principals",
            lambda: validate_host_authority(
                uid_maps,
                gid_maps,
                **authority_args,
                resolve_uids=lambda values: (
                    {collision_uid: "synthetic-nss-principal"}
                    if collision_uid in values
                    else {}
                ),
                resolve_gids=resolve_gids,
            ),
        ),
        "gid_nss_collision": require_authority_rejection(
            "gid_nss_collision",
            "GID mapping aliases NSS principals",
            lambda: validate_host_authority(
                uid_maps,
                gid_maps,
                **authority_args,
                resolve_uids=resolve_uids,
                resolve_gids=lambda values: (
                    {collision_gid: "synthetic-nss-principal"}
                    if collision_gid in values
                    else {}
                ),
            ),
        ),
        "uid_delegation_collision": require_authority_rejection(
            "uid_delegation_collision",
            "UID mapping overlaps delegation for synthetic-other-principal",
            lambda: validate_host_authority(
                uid_maps,
                gid_maps,
                **{
                    **authority_args,
                    "uid_ranges": [
                        *uid_ranges,
                        ("synthetic-other-principal", collision_uid, 1),
                    ],
                },
                resolve_uids=resolve_uids,
                resolve_gids=resolve_gids,
        ...
        "gid_delegation_collision": require_authority_rejection(
            "gid_delegation_collision",
            "GID mapping overlaps delegation for synthetic-other-principal",
            lambda: validate_host_authority(
                uid_maps,
                gid_maps,
                **{
                    **authority_args,
                    "gid_ranges": [
                        *gid_ranges,
                        ("synthetic-other-principal", collision_gid, 1),
                    ],
                },
                resolve_uids=resolve_uids,
                resolve_gids=resolve_gids,
    ...
    numeric_owner_probe = owned_subid_ranges(
        [(str(os.getuid()), own_uids[0][0], own_uids[0][1])],
        caller_names,
    )
    if numeric_owner_probe != [own_uids[0]]:
        raise ConformanceError("numeric subordinate-ID owner alias is not recognized")
    return {
        **positive,
        "caller_owner_aliases": sorted(caller_names),
        "numeric_owner_alias_supported": True,
        "negative_collision_cases": negative_cases,
    }
...
def run_trial(
    ...
    try:
        ...
        for action, value in (
            ...
            ("reject-group", 3999),
            ("reject-group", RUNTIME_GID),
            ("reject-group", SUPERVISOR_GID),
        ):
            key = f"{action}-{value}"
        ...
        seccomp_control_name = f"reploy-mi-{token}-{profile}-{iteration}-seccomp-control"
        seccomp_control_args = common_run_args(
            seccomp_control_name,
            seccomp_path,
            profile,
            3,
            capabilities=False,
            seccomp_override="unconfined",
        )
        created_containers.append(seccomp_control_name)
        seccomp_control = runner.json(
            [*seccomp_control_args, "--rm", image, "seccomp-control"]
        )
        ...
        evidence = {
            ...
            "seccomp_unfiltered_control": seccomp_control,
            "owners": owners,
