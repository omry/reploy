...
func runAppReport(args []string) error {
	...
	_, _, bpfErr := unix.Syscall(unix.SYS_BPF, 0, 0, 0)
	results = append(results, result{
		Test: "seccomp_bpf_without_sys_admin", Pass: bpfErr == unix.EPERM,
		Expected: "operation not permitted", Actual: errorString(bpfErr),
	})
...
func runPrivilegeTarget() error {
	...
	pass := os.Getuid() == declaredUID && os.Geteuid() == declaredUID &&
		os.Getgid() == declaredGID && os.Getegid() == declaredGID && allCapabilitySetsZero(status)
...
func runServiceServer(args []string) error {
	...
	if err := os.WriteFile("/dev/shm/reploy-"+token, []byte("shm"), 0o600); err != nil {
		return err
	}
	if err := os.WriteFile("/dev/shm/sem.reploy-"+token, []byte("sem"), 0o600); err != nil {
		return err
	}
	...
	ready, err := json.Marshal(map[string]any{
		"pass": true, "sysv_shm_id": shm, "posix_mq_round_trip": true,
	})
	if err != nil {
		return err
	}
...
func runCrossProbe(args []string) error {
	...
	for _, path := range []string{"/dev/shm/reploy-" + token, "/dev/shm/sem.reploy-" + token, "/probe-state/service-secret"} {
		f, openErr := os.Open(path)
		if openErr == nil {
			_ = f.Close()
		}
		results = append(results, denied("cross_open_"+filepath.Base(path), openErr))
	}
	mqName := "reploy-" + token
	mqPtr, _ := unix.BytePtrFromString(mqName)
