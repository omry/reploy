...
or upstream `version`. The accepted local-source-exclusion extension also
permits a local mapping to contain an `exclude` array of exact canonical
source-relative subtrees. This extension is implemented in dependent
...
but is not yet available from PR 81's current base, whose strict decoder rejects
`exclude`. Once PR 82 is rebased onto this authority and lands, Reploy applies
the literal paths before observing entry metadata and binds the normalized list
into local-source identity. During `reploy build`,
