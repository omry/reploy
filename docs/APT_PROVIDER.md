...
The conceptual-design review is complete. This document is authoritative for
APT-provider product semantics and refines the provider-neutral public contract
in `BLUEPRINT_ENVIRONMENT_MODEL.md`; APT-specific decisions are not promoted
back into that global authority. This document is not the concrete
...
option and output names are owner-local. Thus `arbiter.arbiter_server` is a
valid reference to output `arbiter_server` owned by application `arbiter`.
...
`arbiter.arbiter_server`. Internal provider outputs retain their canonical
