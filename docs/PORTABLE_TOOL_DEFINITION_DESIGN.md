...
named contribution, export, environment, and probe arrays are sorted by their
canonical name or identity; and nested reference arrays use the same reference
ordering. Ordered values such as probe arguments retain declared order. The
input uses the schema-normalized scalar rules above and is encoded with
