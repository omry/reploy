...
1. Normalize the tool name, version constraint, context, binding request,
   selections, and typed parameter values without choosing a release.
...
3. Enumerate authorized release revisions satisfying the version constraint.
   For each candidate, require exactly one target leaf matching the observed
   base; apply the public binding-inference rules plus the requested context,
   selections, and parameter constraints against that target's advertised
   availability; traverse the candidate's selected references; and construct
   its canonical contribution union against the active provider graph. A
   provider conflict makes that candidate ineligible.
4. From the compatible candidates, select the highest eligible upstream version
   and newest eligible definition revision. No compatible candidate is an
   error; multiple matching target leaves within a candidate are invalid
   definition data, not fallback choices.
...
- **Selected-closure identity** hashes the tool name and exact upstream version,
  the canonical resolved contract projection (chosen context, binding,
  selections, normalized parameter values, runtime, exports, and probes), the
  canonical resolved target projection, and only the exact binding, payload,
  package-set, export, probe, and selection records used by the request. The
  target projection retains the exact target identity, unconditional
  references, selected binding and selection references, target probes, and
  other materialization behavior. Both projections exclude unselected
  availability and validation-fixture and validation-profile references. The
  identity also excludes the release manifest, definition revision, artifact
  source records, and retrieval URLs; the full contract and target records
  remain covered transitively by release provenance.
...
normalization, bounds, target compatibility, and filtering a newer
...
Contracts, targets, bindings, payloads, and package sets have semantic IDs
beneath the release namespace without a revision segment. Their lookup and
