...
4. From the compatible candidates, select the highest eligible scheme-native
   tool version and, when revision is unpinned, its newest eligible definition
   revision. A pinned request can select only that exact revision. No compatible
   candidate is an error; multiple matching target leaves within a candidate
   are invalid definition data, not fallback choices.
...
version-scheme ordering, and filtering a newer provider-conflicting release in
favor of the highest compatible release. Selection tests cover complete target
mappings, coupled contributions, and exclusion of unselected payloads and
native roots. Validation-record tests cover strict fixture/profile parsing,
namespace and target agreement, and missing or wrong-kind references.
