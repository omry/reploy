...
the selected context, resolved binding and selections, and every contract field
that governs their resulting behavior. The target projection contains the
exact target identity and all selected materialization contributions, but
...
- **Selected-closure identity** hashes the tool name and exact upstream version,
  ...
  probes, and other materialization behavior. It excludes validation-fixture
  and validation-profile references. Both projections exclude unselected
  availability. The identity also excludes the release manifest, definition
  revision, artifact source records, and retrieval URLs; the full contract and
  target records remain covered transitively by release provenance.
