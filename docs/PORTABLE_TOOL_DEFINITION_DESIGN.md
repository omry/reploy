...
- the bindings, selections, and typed-parameter constraints available on that
  target;
- unconditional architecture-specific binding and payload references;
...
2. A target explicitly references every binding artifact, payload, and native
   package set that can participate in that target, either unconditionally or
   through its selection contribution mapping.
...
1. Load the tool record's immutable version scheme and normalize the tool name,
   version constraint, optional exact definition revision, context, binding
   request, selections, and typed parameter values without choosing a release.
   When an opaque requirement omits its version, normalize it to exact equality
   with the tool record's `default_version`.
2. Observe the base image's OCI platform, `/etc/os-release`, package manager,
   and manager-native architecture.
3. Enumerate authorized release revisions satisfying the version constraint
   and, when supplied, the exact definition-revision pin. For each candidate,
   require exactly one target leaf matching the observed base; apply the public
   binding-inference rules plus the requested context, selections, and parameter
   constraints against that target's advertised availability; traverse the
   candidate's selected references; and construct its canonical contribution
   union against the active provider graph. A provider conflict makes that
   candidate ineligible.
4. From the compatible candidates for an ordered scheme, select the highest
   eligible scheme-native tool version and, when revision is unpinned, its
   newest eligible definition revision. An opaque request already names one
   exact version, either explicitly or through `default_version`, so it performs
   no ordering. A pinned request can select only that exact revision. No
   compatible candidate is an error; multiple matching target leaves within a
   candidate are invalid definition data, not fallback choices.
5. Finalize the chosen target, inferred binding, selections, normalized
   parameters, selected closure, and already-validated contribution union.
   Recheck that the active provider inputs used for candidate filtering have not
...
- advertised bindings or selections with incomplete referenced closures, or a
  target selection without exactly one valid contribution mapping;
...
version-scheme ordering, opaque defaults, and filtering a newer
provider-conflicting release in favor of the highest compatible release.
Selection tests cover complete target mappings, coupled contributions, and
exclusion of unselected payloads and native roots. Validation-record tests
cover strict fixture/profile parsing, namespace and target agreement, and
...
`tool:<name>/releases/<tool-version>`; its manifest is
`tool:<name>/releases/<tool-version>/revisions/<definition-revision>/manifest`.
...
ASCII letters, digits, `.`, `+`, `-`, `:`, and `/`, with no empty, `.` or `..`
path segments.
...
`defaults`. Typed parameter schemas are sorted by canonical parameter name and
...
permits neither a binding nor a selection. The initial Java and Playwright
contracts declare no parameters. Binding requirements remain owned by
