...
Separately from the external-publication aggregate budgets, one definition-wide
ceiling applies to every definition, including embedded ones: a definition
whose required integration coverage exceeds the core cap is rejected, as
described under Reploy Integration Validation. That cap bounds the shared CI
work a definition can demand.
