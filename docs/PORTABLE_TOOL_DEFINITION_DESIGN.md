...
The initial asciinema implementation is upstream `3.2.1` for build use on
Linux AMD64 and ARM64. Its exact GitHub-hosted `x86_64-unknown-linux-gnu` and
`aarch64-unknown-linux-gnu` release assets are pinned by size and SHA-256 and
validated through the shared profile. Asciinema v2 remains a non-published
coexistence analysis; it is neither implemented nor advertised.
...
Additional Java or asciinema versions, runtime Java, Playwright bindings or
browsers, Java or Playwright ARM64, and repository publication remain explicit
later extensions; none is inferred from the initial embedded records.
