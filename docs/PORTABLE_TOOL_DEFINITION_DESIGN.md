...
- **Selected-closure identity** hashes the tool name and exact upstream version,
  the canonical resolved release-contract projection, and only the exact
  target, binding, payload, package-set, export, probe, and selection records
  used by the request. It excludes unselected release-contract options, the
  release manifest, definition revision, artifact source records, and retrieval
  URLs. The full contract record remains covered by release provenance.
