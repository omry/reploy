...
The canonical resolved projection of the release contract, exact target,
selected binding, selected payloads, native package sets, exports, and probes
that affect one resolved tool request. The projection contains the selected
context, resolved binding and selections, and every contract field that governs
their resulting behavior. It excludes unselected option catalogs and defaults
that did not change the resolved request. Records and contract options for
unrelated targets, bindings, or selections are outside the closure.
...
1. Normalize the tool name, version constraint, context, binding request, and
   selections without choosing a release.
...
3. Enumerate authorized release revisions satisfying the version constraint.
   ...
   base and apply the public binding-inference rules plus the requested context
   and selections against that target's advertised availability.
...
5. Finalize the chosen target, inferred binding, and selections, then traverse
   their explicit references to form the selected closure.
...
- **Selected-closure identity** hashes the tool name and exact upstream version,
  ...
  selections, runtime, exports, and probes), the canonical resolved target
  projection, and only the exact binding, payload, and package-set records used
  by the request. The target projection retains the exact target identity,
  unconditional references, selected binding and selection references, target
  probes, and validation references. The projections exclude unselected
  contract options and target availability. The identity also excludes the
  release manifest, definition revision, artifact source records, and retrieval
  URLs; the full contract and target records remain covered transitively by
  release provenance.
...
described above. Identity tests prove that a source-locator-only change
preserves selected-closure identity while a resolved contract behavior,
