...
- the bindings and selection dimensions available on that target;
...
entry; an unadvertised symbol cannot have one. Each entry references the
...
declared in the release contract and advertised by the target. Every advertised
value has exactly one entry; an unadvertised dimension or value cannot have
one. Each entry contains exact
...
3. For each normalized requirement, enumerate authorized release revisions
   ...
   validate the context and selection map against that
   target; then traverse the selected references and construct the candidate
   contribution union. A client, target, binding, selection, or
...
- ARM64 is advertised only if every binding artifact, browser payload, native
  package, extraction rule, and built-in integration validator succeeds for the
  exact target.
...
- duplicate selection dimensions, malformed or duplicate combination tuples,
...
mappings. Build tests prove that catalog data cannot introduce commands and
that cross-platform materialization never requires executing a target binary.
...
binding, or advertised normalized selection combination lacks a runnable case.
Every combination is explicit definition data and is exercised directly under
the core publication cap; Reploy never infers an outer product. Hand-authored
evidence metadata cannot satisfy this coverage gate.
...
failure fails that integration-validation job and produces no successful
support record; it does not make cross-platform build materialization depend on
executing the target binary. Negative fixtures verify that unsupported
OS versions, architectures, bindings, and selections fail
...
and an explicit `combinations` array. Each combination is an equal-length tuple;
tuple positions correspond to `dimensions`, and each cell is a scalar or list
that normalizes to a sorted set. Dimensions and normalized tuples are unique,
values must be advertised for their dimension, and the non-raiseable core cap
bounds the number of combinations. A request is valid only when its normalized
dimension-to-set map exactly matches an advertised combination. Reploy never
infers a Cartesian product. A future authoring helper may expand product
shorthand into this same explicit canonical matrix before validation. Standard
request field names are reserved; publication rejects a dimension-name
collision.
...
native dependency, materialization rule, and built-in integration validator for
the exact target succeeds.
...
digest, exact target tuple, immutable base-image digest, binding set and
selection-dimension map,
fixture ID, validator version, result, and validator-output digest. Only a
