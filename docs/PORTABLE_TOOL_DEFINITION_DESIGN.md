...
- **Selected-closure identity** hashes the tool name and exact upstream version,
  the canonical resolved release-contract and target projections, and only the
  normalized parameter values and exact binding, payload, package-set, export,
  probe, and selection records used by the request. It excludes unselected
  availability, validation-fixture and validation-profile references, the
