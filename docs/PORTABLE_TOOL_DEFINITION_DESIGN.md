...
3. Enumerate authorized release revisions satisfying the version constraint
   ...
   require the running Reploy version to satisfy the release contract and every
   named primitive to be supported; require exactly one target leaf matching
   the observed base; apply the public binding-inference rules plus the
   requested context, selections, and parameter constraints against that
   target's advertised availability; traverse the candidate's selected
   references; and construct its canonical contribution union against the
   active provider graph. A client-compatibility or provider conflict makes that
