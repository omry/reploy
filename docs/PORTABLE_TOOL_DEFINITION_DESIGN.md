...
binding, individual selection, or normalized parameter value lacks a runnable
case. When a release permits multiple simultaneous selections, CI also
exercises every declared compatibility group and at least one union containing
all simultaneously supported selections.
...
selection-compatibility combination. Publication rejects a definition whose
