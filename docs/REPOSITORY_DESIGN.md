...
Stage, install, build, and ordinary trusted search use the accepted local index
without silently contacting repositories. When no usable authenticated index
exists for a new resolution, Reploy fails with the exact `repository update`
operation required.
