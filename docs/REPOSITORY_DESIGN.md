...
A qualified asset identity contains a publisher namespace and asset name, such
as `acme/editor` or `tool:acme/playwright`. Qualification distinguishes that
publisher's asset from another publisher's similarly named asset; it does not
select a repository or establish trust.
...
Every qualified asset identity chooses one immutable Reploy-supported version
scheme:
...
identity is not an ordinary update; the initial protocol requires a new
qualified identity, with any future cross-scheme migration requiring a focused
design.
...
- Different publishers may offer the same visible name. Normal repository
  selection chooses the source, while qualification identifies the publisher.
...
Locks retain the selected repository, version scheme, exact upstream version,
Reploy revision, and content identity, so later policy changes cannot
reinterpret an accepted result.
...
The public requirement `version` is an upstream-facing constraint, not a
definition revision:
...
A definition may expose strict parameters selecting declared sub-payloads:
...
Parameters declare names, types, required/default behavior, allowed values, and
target availability. Missing required or unsupported values fail before
acquisition and list target-supported values. A parameter cannot introduce a
URL, command, package expression, or undeclared repository object.
