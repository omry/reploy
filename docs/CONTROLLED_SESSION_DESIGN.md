...
through another path. Original paths below `/proc`, `/dev`, or `/sys` are
rejected before symlink resolution so process-relative proc magic links cannot
resolve differently for Reploy and Docker. Where Linux reports mount identity,
Reploy distinguishes devtmpfs from ordinary tmpfs and rejects it as well. On
macOS, native devfs and procfs sources are likewise rejected.
...
default-deny public/local application-network
