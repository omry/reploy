...
- Implementation state: Initial global sandbox prerequisites, trusted
  ...
  one validated receipt explicitly; empty targets are not incidents, receipts
...
list and explicit acknowledgement APIs that do not contact Docker. Empty
targets are ignored, malformed or oversized receipts fail closed, completed
receipts are not silently evicted, and the fixed retention bound blocks new
session startup until receipts are handled. A parent-liveness EOF proves parent
loss but cannot by itself distinguish `SIGKILL`, an OOM kill, or another abrupt
process death.
