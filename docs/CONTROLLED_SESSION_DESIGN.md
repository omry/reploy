...
- Implementation state: Global sandbox prerequisites plus authorization,
  protocol, and lifecycle foundations implemented in the current stack; Docker
  session orchestration not started
...
denial includes host-loopback redirection, private and link-local address
ranges, IPv6 local ranges, infrastructure metadata endpoints, and conservative
translation ranges that could embed private destinations. The initial coarse
classifier follows the destination address; topology-resistant peer and gateway
confinement belongs to the deferred L3 gateway design.
...
private, link-local, multicast, reserved, infrastructure metadata, and
conservative address-translation ranges.
Container-local loopback remains available and cannot address host loopback
through the container network namespace. Docker's embedded DNS may be used
only when public access is enabled because it may forward queries externally;
the resulting connection still passes the destination policy. When both
classes are enabled, no packet filter is needed; the helper still applies the
same identity and authority-drop invariant. A backend that cannot install and
verify a requested denied class fails closed.
...
failure. Later events remain diagnostic observations. Workload status,
controller finalization status, and pre-delivery cleanup success are reported
separately in the session result, so a cleanup failure can fail the operation
without hiding its original cause. Controller exit and delivery-tail cleanup
are reported separately by the invoking host operation after teardown.
...
Host Reploy is waiting for controller finalization after a workload exit.

Normal completion is:

1. The controller finalizes its client-owned output and sends `complete`, or
   Host Reploy observes the workload shell exit.
2. Host Reploy atomically latches the cause and enters `terminating`.
3. If the workload exited first, Host Reploy reports that event and gives
   the live controller a bounded finalization period in which to write its
   client-owned output and send `complete`.
4. Host Reploy performs bounded graceful termination followed by forced
   termination when necessary.
5. Host Reploy independently observes the exact workload container stopped.
6. Host Reploy removes the workload container, endpoint publication, temporary
...
8. Host Reploy waits for a bounded `acknowledge_terminated` response. Channel
   closure is not an acknowledgement. Timeout or disconnect does not block
   teardown.
9. Host Reploy closes the channel, stops and removes the controller, removes the
...
reports seccomp filtering, `no-new-privileges`, and empty inheritable,
effective, permitted, bounding, and ambient capability sets before it executes
the exact application argv.
The same helper now installs the default-deny public/local application-network
policy for persistent and transient containers while preserving exact declared
inbound endpoints. Live Docker coverage exercises all four policy combinations
over IPv4 and IPv6, a root default-denial case, non-root authority removal,
