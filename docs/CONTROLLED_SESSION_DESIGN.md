...
- Implementation state: Global sandbox prerequisites in progress; session lifecycle not started
...
The loopback-published port is never disclosed to the toolchain, but any local
...
constrains the toolchain, not unrelated host processes. It must not claim
...
include controller completion, controller-requested termination, application
exit, host cancellation, controller loss, Docker-observation loss, and startup
failure. Later events remain diagnostic observations. Application and
