...
- Implementation state: Initial global sandbox prerequisites, trusted
  ...
  starts. It acknowledges readiness, watches a private parent-liveness pipe,
  and treats EOF or any malformed control byte as parent loss. On loss it
  checks the current boot identity and every exact container ownership label,
  ...
  internal manifest has no independent schema-version marker. Retry during
  Docker unavailability and restart reconciliation remain the next ownership
  phases, and controlled-session networking remains a later phase.
