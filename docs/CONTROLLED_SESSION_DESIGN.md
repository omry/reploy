...
filesystem identity so procfs, sysfs, devpts, and legacy devfs mounts remain
prohibited when exposed through another path. Where Linux reports mount
identity, Reploy distinguishes devtmpfs from ordinary tmpfs and rejects it as
well. On macOS, native devfs and procfs sources are likewise rejected.
