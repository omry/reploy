...
- Implementation state: Initial global sandbox prerequisites, trusted
  ...
  definitive cleanup failure produces the bounded failed receipt for
  next-operation recovery. Controlled-session networking remains a later
  phase.
...
returns or the host reboots. It does not retry ownership mismatches, malformed
runtime results, filesystem failures, or other definitive failures while the
daemon is responsive; those produce the bounded failed receipt and retain
next-operation recovery. If both the attached operation and watchdog are
killed, durable labels and deployment-scoped live-run state let the next locked
Reploy operation reconcile the abandoned resources. This final fallback is
