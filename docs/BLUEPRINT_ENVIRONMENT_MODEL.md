...
Implementation status: `exclude` is accepted future syntax, not behavior in
the current implementation baseline. The strict current decoder rejects the
key as unknown. It becomes usable only after the local-source-exclusion
delivery tracked in the implementation plan; until then, the source-identity
rule above and every exclusion rule and example below specify the required
contract rather than supported behavior.
