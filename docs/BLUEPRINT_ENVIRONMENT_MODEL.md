...
`tool`, `version`, and `select` will be required. Because `python` is the only
binding, Reploy will infer it when `binding` is omitted; explicitly naming
`binding: python` as above will be equivalent. The embedded definition will
declare the complete Python requirement roots (`playwright==1.61.0`,
`pyee>=13,<14`, and `greenlet>=3.1.1,<4.0.0`), pin the exact Linux AMD64
Playwright wheel, and record its bundled Node.js 24.17.0 and `playwright-core`
