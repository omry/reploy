...
both `public` and `local` are allowed. Container-local loopback remains
available and cannot address host loopback through the container network
namespace. Docker's embedded resolver is the one loopback exception: it is
blocked unless `public` is allowed because it can forward queries outside the
container.
