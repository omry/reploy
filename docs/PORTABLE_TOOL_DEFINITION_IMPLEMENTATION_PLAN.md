...
observed canonical ABI to equal `cp<observed-major><observed-minor>` without a
free-threaded suffix. Any other Python/ABI form fails closed before acquisition.
Platform `any` needs no further runtime test. `linux_<arch>` is eligible after
the shared selected-target architecture check. Every manylinux policy requires
...
`abi3` tags, rejection of `abi3` on a free-threaded CPython ABI, and a
nonmatching implementation or ABI. Contract fixtures cover a matching
major/minor series, an exact matching patch release, and a nonmatching patch
release. Platform fixtures cover `any`, native Linux, every legacy manylinux
alias, and versioned
