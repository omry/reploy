...
existing pinned AMD64 and ARM64 targets is in scope, and v2 is retained as a
design comparison to ensure that
...
Scope: add plural binding contributions and named selection-dimension
contributions; scoped artifacts, packages, and
...
ordered dimensions and unique equal-length tuples whose cells normalize to
sets; a request must exactly match one advertised combination; Reploy does not
...
apply binding inference, validate the context and selection map against that
target, and traverse the selected references to build each candidate's
...
conflicting composition fails before embedding; generated output passes the
ordinary catalog validators.
...
Scope: add asciinema 3.2.1, its existing pinned AMD64 and ARM64 targets, and its
GitHub-hosted artifact sources, fixtures, shared validation profile, and probe
through the canonical catalog generator. Keep common records independent of
target variants.
...
evidence using the repository's existing pinned 3.2.1 metadata; the definition
builds and probes through Reploy. A non-published v2
...
Non-goals: asciinema v2 implementation or support advertisement.
...
fail during parsing, while unsupported dimensions or values fail during catalog
resolution before provider work. Identical same-scope requirements deduplicate
and an incompatible same-scope merge is an error; the binding set and normalized
selection map participate in request identity; parser and blueprint
tests cover Java, Playwright, and asciinema.
...
set and advertised selection combination; execute fixtures through ordinary
...
record current external evidence using the definition-supplied Java validation
profile.
...
record current external evidence using the definition-supplied Python/browser
validation profile.
...
Scope: run the manifest-derived asciinema cases on every advertised target and
record current external evidence using its definition-supplied profile probe.
...
CI; no result advertises asciinema v2.

Non-goals: asciinema v2 or additional targets and architectures.
