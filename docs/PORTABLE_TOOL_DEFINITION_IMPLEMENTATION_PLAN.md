...
- asciinema 3.2.1 using the existing pinned Linux AMD64 and ARM64 release
  assets;
...
and static validation succeed without acquisition, materialization, provider
integration, or probe execution. A non-published v2
...
Scope: run the manifest-derived asciinema cases on every advertised support
tuple and record current external evidence using its definition-supplied
profile probe.
...
CI; this is the first slice that claims end-to-end asciinema build and probe
acceptance; no result advertises asciinema v2.
