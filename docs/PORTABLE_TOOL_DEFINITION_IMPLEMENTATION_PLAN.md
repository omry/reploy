...
authorized release candidates for each normalized requirement, remove
candidates the running client cannot satisfy by version or primitive set, require exactly one matching target leaf, and try ordered-scheme
...
rather than reaching enumeration without an exact coordinate; request input
