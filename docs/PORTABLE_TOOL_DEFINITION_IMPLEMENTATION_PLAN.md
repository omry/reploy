...
the provider-owned pre-acquisition gate compares those facts plus Python and ABI
compatibility with the observed runtime. This remains one generic wheel
eligibility mechanism rather than tool-specific logic or a second parser.
...
the owning Python resolver has observed its interpreter and Linux libc,
...
needed before acquisition. Extend the existing fixed, isolated,
standard-library interpreter inspection, in the same invocation, to emit the
normalized Python implementation, canonical ABI tag, and normalized libc
implementation and canonical nonnegative major/minor release pair alongside
the complete Python version. An absent, malformed, or unrecognized libc
observation is represented explicitly as unknown rather than guessed. Replace
`python-interpreter-facts-v1` and every dependent provider profile and recipe
identity with their next versions in this slice; do not add a dual reader or
compatibility path for the unreleased format. The helper consumes those facts
and the already expanded and platform-policy-projected artifact tags; it
performs no subprocess, filesystem, network, or acquisition operation.

For ABI `none`, generic `py<major>` matches that observed Python major and
generic `py<major><minor>` matches the same major when the tagged minor is not
newer than the observed minor, following pip's generic compatibility ordering.
A `cp<major><minor>` tag requires CPython and the matching release; ABI `none`
is generic to that interpreter, an exact `cp<major><minor>` ABI must equal the
observed canonical ABI. `abi3` requires an encoded CPython release of at least
3.2, an observed CPython release at or above that encoded release, and the
observed canonical ABI to have the form
`cp<observed-major><observed-minor><flags>`, where `<flags>` is empty or omits
`t`; non-threading flags such as `d` remain eligible, while a `t` flag denotes
a free-threaded build and rejects ordinary `abi3`. Any other Python/ABI form
fails closed before acquisition. Platform `any` is eligible only with ABI
`none`; after that tuple constraint, compatible generic and CPython
`*-none-any` tags need no further runtime test. Any ABI-bearing `*-any` tag
fails closed even when its interpreter/ABI pair is otherwise compatible.
`linux_<arch>` is eligible after the shared selected-target architecture check.
Every manylinux policy requires
observed glibc with major exactly 2 and an observed release satisfying the
projected minimum; compare the canonical major/minor pairs using pip-compatible
release ordering. An unsupported versioned floor, unknown libc, a non-glibc
implementation, an observed glibc major other than 2, or an insufficient
release fails closed. The provider compares projected policy with runtime
facts but does not parse wheel filenames, infer architectures, or duplicate
the shared policy mapping.
...
release. Platform fixtures cover generic and CPython `*-none-any`, rejection of
exact-ABI and `abi3` `*-any` tuples, native Linux, every legacy manylinux alias,
and versioned
...
Differential fixtures prove that every supported Python/ABI/platform decision
