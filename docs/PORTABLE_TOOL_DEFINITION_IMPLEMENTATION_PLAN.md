...
The probe retains isolated mode and the controlled environment but does not
suppress system site initialization. It imports the selected interpreter's
installed `pip._vendor.packaging.tags` implementation—the same pip installation
used by the subsequent ordinary `python -m pip` resolver—enumerates `sys_tags()`
once, and emits only the tested tags present in that set. This read-only step
performs no network operation, artifact acquisition, or wheel parsing. The
...
an accepted tag outside the tested set before acquisition.
