...
observed canonical ABI, and `abi3` requires an encoded CPython release of at
least 3.2 plus an observed CPython release at or above that encoded release.
Any other Python/ABI form fails closed before acquisition.
...
`abi3` tags, and a nonmatching implementation or ABI. Platform fixtures
cover `any`, native Linux, every legacy manylinux alias, and versioned
