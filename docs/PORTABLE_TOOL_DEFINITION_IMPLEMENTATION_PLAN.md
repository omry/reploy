...
Scope: enumerate authorized release candidates for each normalized requirement,
remove candidates the running client cannot satisfy by version or primitive
set, require exactly one matching target leaf, and try ordered-scheme
candidates by descending scheme-native version and then descending definition
revision. Resolve the surviving scoped candidates as one bounded deterministic
constraint problem against the active provider graph, failing closed with a
diagnostic when the visited-state cap is exceeded. Then resolve version,
revision, context, target, binding, selections, typed parameters, all scoped
contributions, matching validation fixture, provenance, and order-independent
selected identity.
...
candidate falls back to an older compatible release instead of failing; request
input order cannot change the result; and exceeding the assignment cap fails
...
and their obsolete tests are removed; the flat Java definitions and their
aggregate digest behavior are removed in this same slice, because the design
requires that the flat and hierarchical formats never coexist as public
contracts.
...
user launches Chromium; conflicts fail before publication; the flat Playwright
definitions are removed in this same slice, so no tool retains both formats as
public contracts.
...
path survives, since each tool's flat format is removed by the slice that
activates its hierarchy; remove any remaining scaffolding and obsolete tests;
update ADR 0001, environment examples, maintaining docs, support presentation,
and release notes; run final scope and security review.
